...
# Certains environnements de build (binaire terra RSPM dans le sous-processus
# de `R CMD build` / pkgdown) ne savent pas construire de SpatRaster, alors que
# le même code passe dans les tests testthat. On sonde cette capacité une fois ;
# si elle manque, les chunks de calcul spatial sont rendus sans être évalués
# (pas de plantage du build). En local et sur un terra sain, tout s'exécute.
.nemeton_terra_ok <- tryCatch({
  tf <- tempfile(fileext = ".tif")
  terra::writeRaster(
    terra::rast(nrows = 2, ncols = 2, xmin = 0, xmax = 2, ymin = 0, ymax = 2,
                crs = "EPSG:32631", vals = 1),
    tf, filetype = "GTiff", overwrite = TRUE)
  unlink(tf)
  inherits(
    terra::rast(system.file("extdata", "massif_demo_dem.tif", package = "nemeton")),
    "SpatRaster")
...
```{r gate-spatial, include = FALSE}
# À partir d'ici, les chunks chargent réellement les rasters via terra.
# Si l'environnement de build ne peut pas construire de SpatRaster (cf. chunk
# `setup`), on n'évalue pas ces chunks pour éviter un échec de build.
knitr::opts_chunk$set(eval = .nemeton_terra_ok)
```
...
```{r, fig.cap = "Comparaison carbone vs eau", fig.width = 10}
plot_indicators_map(
  normalized,
  indicators = c("indicateur_c2_ndvi_norm", "indicateur_w3_humidite_norm"),
  palette = "viridis",
