...
> 🌍 **English version**: See `vignette("getting-started")` | **Version française** : Ce document
...
```{r, eval = FALSE, fig.cap = "Parcelles forestières par type"}
...
```{r, eval = FALSE, fig.cap = "Carte de l'indice de santé écosystémique"}
plot_indicators_map(
  composite,
...
```{r, eval = FALSE, fig.cap = "Comparaison carbone vs biodiversité", fig.width = 10}
plot_comparison_map(
  normalized,
  indicators = c("carbon_norm", "biodiversity_norm"),
  titles = c("Stock de carbone", "Biodiversité"),
  ncol = 2
)
```
...
```{r, eval = FALSE, fig.cap = "Profil écosystémique - Parcelle P01"}
nemeton_radar(
  normalized,
  unit_id = "P01",
  indicators = c("carbon_norm", "biodiversity_norm", "water_norm",
...
Pour les indicateurs où une valeur faible est souhaitable (ex: fragmentation) :

```{r, eval = FALSE}
# Inverser la fragmentation (moins fragmenté = mieux)
...
# La fragmentation inversée reflète maintenant la continuité
