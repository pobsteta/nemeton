---
title: "Analyse temporelle avec nemeton"
author: "Pascal Obstétar"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Analyse temporelle avec nemeton}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r setup, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.width = 8,
  fig.height = 6,
  warning = FALSE,
  message = FALSE
)
```

> 🌍 **English version**: See `vignette("temporal-analysis")` | **Version française** : Ce document

# Introduction

Le package `nemeton` fournit des outils complets pour **l'analyse temporelle** des indicateurs d'écosystèmes forestiers. Cette vignette démontre comment :

- Gérer des datasets multi-périodes avec `nemeton_temporal()`
- Calculer les taux de changement (absolu et relatif) avec `calculate_change_rate()`
- Visualiser les tendances temporelles avec des graphiques de séries chronologiques
- Créer des heatmaps d'évolution d'indicateurs
- Comparer des scénarios avant/après intervention

L'analyse temporelle permet aux gestionnaires forestiers de :

- **Détecter les tendances** : Identifier les changements à long terme dans les services écosystémiques
- **Évaluer les interventions** : Évaluer l'impact des actions de gestion
- **Surveiller la résilience** : Suivre la récupération après perturbations
- **Informer les décisions** : Baser les stratégies sur les dynamiques observées

# Installation

```{r, eval = FALSE}
# Depuis GitHub
remotes::install_github("pobsteta/nemeton")
```

```{r, eval = FALSE}
library(nemeton)
library(ggplot2)
library(dplyr)
```

# Créer des datasets multi-périodes

## Structure d'un objet temporel

Un objet `nemeton_temporal` contient plusieurs périodes temporelles, chacune avec les mêmes unités spatiales suivies dans le temps.

```{r, eval = FALSE}
# Charger les données de démo
data(massif_demo_units)

# Simuler trois périodes temporelles (2015, 2020, 2025)
# En pratique, ces données proviendraient de sources différentes

# Période 1 : 2015 baseline
units_2015 <- massif_demo_units[1:10, ]
units_2015$period <- "2015"
units_2015$parcel_id <- paste0("P", sprintf("%03d", 1:10))
units_2015$carbon <- rnorm(10, mean = 150, sd = 20)
units_2015$biodiversity <- rnorm(10, mean = 12, sd = 3)
units_2015$water <- rnorm(10, mean = 8, sd = 2)

# Période 2 : 2020 intermédiaire (croissance)
units_2020 <- massif_demo_units[1:10, ]
units_2020$period <- "2020"
units_2020$parcel_id <- paste0("P", sprintf("%03d", 1:10))
units_2020$carbon <- units_2015$carbon * runif(10, 1.05, 1.15)  # 5-15% croissance
units_2020$biodiversity <- units_2015$biodiversity + rnorm(10, mean = 1, sd = 0.5)
units_2020$water <- units_2015$water + rnorm(10, mean = 0, sd = 0.3)

# Période 3 : 2025 récent (croissance continue + effets intervention)
units_2025 <- massif_demo_units[1:10, ]
units_2025$period <- "2025"
units_2025$parcel_id <- paste0("P", sprintf("%03d", 1:10))
units_2025$carbon <- units_2020$carbon * runif(10, 1.08, 1.20)
units_2025$biodiversity <- units_2020$biodiversity + rnorm(10, mean = 1.5, sd = 0.8)
units_2025$water <- units_2020$water + rnorm(10, mean = 0.5, sd = 0.4)
```

## Créer l'objet temporel

```{r, eval = FALSE}
# Créer le dataset temporel
temporal_data <- nemeton_temporal(
  periods = list(
    "2015" = units_2015,
    "2020" = units_2020,
    "2025" = units_2025
  ),
  id_column = "parcel_id"
)

# Inspecter l'objet temporel
print(temporal_data)
```

```{r, eval = FALSE}
# Statistiques sommaires
summary(temporal_data)
```

L'objet temporel suit :

- **3 périodes** (2015, 2020, 2025)
- **10 unités spatiales** (parcelles P001-P010)
- **3 indicateurs** (carbon, biodiversity, water)
- **Identifiants cohérents** dans le temps (parcel_id)

# Calculer les taux de changement

## Taux de changement absolu

Le changement absolu quantifie la différence brute entre périodes (ex: Mg C/ha/an).

```{r, eval = FALSE}
# Calculer les taux de changement absolus pour tous les indicateurs
rates_absolute <- calculate_change_rate(
  temporal_data,
  indicators = c("carbon", "biodiversity", "water"),
  type = "absolute"
)

# Inspecter les résultats
head(rates_absolute)
```

```{r, eval = FALSE}
# Résumé du taux de changement du carbone
cat("\n=== Taux de changement du carbone (Mg C/parcelle/an) ===\n")
summary(rates_absolute$carbon_rate_abs)

# Identifier les parcelles avec l'accumulation de carbone la plus élevée
top_carbon <- rates_absolute %>%
  sf::st_drop_geometry() %>%
  arrange(desc(carbon_rate_abs)) %>%
  head(3)

cat("\nTop 3 des parcelles pour l'accumulation de carbone :\n")
print(top_carbon[, c("parcel_id", "carbon_rate_abs")])
```

## Taux de changement relatif

Le changement relatif exprime le changement en pourcentage de la baseline (ex: %/an).

```{r, eval = FALSE}
# Calculer les taux de changement relatifs
rates_relative <- calculate_change_rate(
  temporal_data,
  indicators = c("carbon", "biodiversity", "water"),
  type = "relative"
)

# Résumé du changement de biodiversité (%)
cat("\n=== Taux de changement de la biodiversité (%/an) ===\n")
summary(rates_relative$biodiversity_rate_rel)

# Identifier les parcelles avec l'augmentation de biodiversité la plus élevée
top_bio <- rates_relative %>%
  sf::st_drop_geometry() %>%
  arrange(desc(biodiversity_rate_rel)) %>%
  head(3)

cat("\nTop 3 des parcelles pour l'augmentation de biodiversité :\n")
print(top_bio[, c("parcel_id", "biodiversity_rate_rel")])
```

## Absolu et relatif simultanés

Calculer les deux types simultanément :

```{r, eval = FALSE}
# Les deux types de taux
rates_both <- calculate_change_rate(
  temporal_data,
  indicators = c("carbon", "biodiversity", "water"),
  type = "both"
)

# Vérifier les colonnes
cat("\nColonnes disponibles :\n")
grep("_rate_", names(rates_both), value = TRUE)
```

# Visualisations temporelles

## Graphiques de tendances temporelles

Visualiser l'évolution des indicateurs à travers toutes les périodes.

```{r fig.cap = "Évolution du stock de carbone (2015-2025)", eval = FALSE}
# Tendances du carbone dans le temps
plot_temporal_trend(
  temporal_data,
  indicator = "carbon",
  title = "Évolution du stock de carbone (2015-2025)"
)
```

```{r fig.cap = "Tendances multi-indicateurs", fig.height = 8, eval = FALSE}
# Indicateurs multiples
plot_temporal_trend(
  temporal_data,
  indicator = c("carbon", "biodiversity", "water"),
  title = "Tendances temporelles multi-indicateurs"
)
```

# Cas d'usage : Avant/Après intervention

## Simuler une intervention de gestion forestière

```{r, eval = FALSE}
# Scénario : Éclaircie sélective en 2020 affectant le carbone et la biodiversité

# Avant intervention (2015)
units_before <- massif_demo_units[1:5, ]
units_before$parcel_id <- paste0("P", 1:5)
units_before$carbon <- c(120, 135, 128, 142, 138)
units_before$biodiversity <- c(10, 11, 9, 12, 10)

# Après intervention (2025) - l'éclaircie augmente la biodiversité, réduit le carbone à court terme
units_after <- massif_demo_units[1:5, ]
units_after$parcel_id <- paste0("P", 1:5)
units_after$carbon <- c(110, 125, 118, 132, 128)       # -8% carbone
units_after$biodiversity <- c(14, 15, 13, 16, 14)     # +40% biodiversité

# Créer l'objet temporel
intervention <- nemeton_temporal(
  periods = list("2015" = units_before, "2025" = units_after),
  id_column = "parcel_id"
)

print(intervention)
```

## Évaluer les effets de l'intervention

```{r, eval = FALSE}
# Calculer les taux de changement
intervention_rates <- calculate_change_rate(
  intervention,
  indicators = c("carbon", "biodiversity"),
  type = "both"
)

# Résumé
cat("\n=== Résumé de l'impact de l'intervention ===\n")
cat("\nChangement du carbone (absolu) :\n")
summary(intervention_rates$carbon_rate_abs)

cat("\nChangement de la biodiversité (absolu) :\n")
summary(intervention_rates$biodiversity_rate_abs)

# Analyse des compromis
tradeoff <- intervention_rates %>%
  sf::st_drop_geometry() %>%
  select(parcel_id, carbon_rate_abs, biodiversity_rate_abs)

cat("\nAnalyse des compromis :\n")
print(tradeoff)
```

```{r fig.cap = "Impact de l'intervention sur la biodiversité", eval = FALSE}
# Visualiser l'augmentation de la biodiversité
plot_temporal_trend(
  intervention,
  indicator = "biodiversity",
  title = "Réponse de la biodiversité à l'intervention d'éclaircie"
)
```

# Workflows avancés

## Analyse temporelle multi-famille

Combiner l'analyse temporelle avec le système multi-famille (voir `vignette("indicator-families_fr")`).

```{r eval = FALSE}
# Exemple de workflow (nécessite des indicateurs de famille calculés)

# Calculer les indicateurs de famille pour chaque période
layers <- massif_demo_layers()

# Période 1
results_2015 <- nemeton_compute(
  units_2015,
  layers,
  indicators = c("C1", "C2", "W1", "W2", "W3")
)

# Période 2
results_2020 <- nemeton_compute(
  units_2020,
  layers,
  indicators = c("C1", "C2", "W1", "W2", "W3")
)

# Créer des indices de famille pour chaque période
family_2015 <- create_family_index(results_2015)
family_2020 <- create_family_index(results_2020)

# Analyse temporelle des scores de famille
temporal_families <- nemeton_temporal(
  periods = list("2015" = family_2015, "2020" = family_2020),
  id_column = "parcel_id"
)

# Taux de changement des scores de famille
family_rates <- calculate_change_rate(
  temporal_families,
  indicators = c("family_C", "family_W"),
  type = "both"
)
```

## Normalisation de période de référence

Normaliser toutes les périodes en utilisant la première période comme référence :

```{r eval = FALSE}
# Extraire les données de la première période
baseline <- temporal_data$periods[[1]]

# Normaliser chaque période en utilisant la référence baseline
temporal_normalized <- nemeton_temporal(
  periods = lapply(temporal_data$periods, function(period_data) {
    normalize_indicators(
      period_data,
      indicators = c("carbon", "biodiversity", "water"),
      method = "minmax",
      reference_data = baseline  # Utiliser min/max de la baseline
    )
  }),
  id_column = "parcel_id"
)

# Toutes les périodes sont maintenant normalisées par rapport à la baseline (échelle 0-100)
```

## Détection d'anomalies

Identifier les parcelles avec des patterns de changement inhabituels :

```{r, eval = FALSE}
# Calculer les z-scores des taux de changement
rates_zscore <- rates_absolute %>%
  sf::st_drop_geometry() %>%
  mutate(
    carbon_zscore = scale(carbon_rate_abs)[, 1],
    biodiversity_zscore = scale(biodiversity_rate_abs)[, 1]
  )

# Signaler les anomalies (|z| > 2)
anomalies <- rates_zscore %>%
  filter(abs(carbon_zscore) > 2 | abs(biodiversity_zscore) > 2) %>%
  select(parcel_id, carbon_zscore, biodiversity_zscore)

if (nrow(anomalies) > 0) {
  cat("\nAnomalies détectées (|z| > 2) :\n")
  print(anomalies)
} else {
  cat("\nAucune anomalie détectée (tous |z| < 2)\n")
}
```

# Export des résultats temporels

## Sauvegarder l'objet temporel

```{r eval = FALSE}
# Sauvegarder en RDS pour analyse future
saveRDS(temporal_data, "results/temporal_data_2015_2025.rds")

# Charger plus tard
temporal_data <- readRDS("results/temporal_data_2015_2025.rds")
```

## Exporter les taux de changement

```{r eval = FALSE}
# Exporter en GeoPackage
sf::st_write(
  rates_both,
  "results/change_rates_2015_2025.gpkg",
  delete_dsn = TRUE
)

# Exporter en CSV (sans géométrie)
rates_table <- rates_both %>%
  sf::st_drop_geometry()

write.csv(
  rates_table,
  "results/change_rates_2015_2025.csv",
  row.names = FALSE
)
```

## Sauvegarder les graphiques

```{r eval = FALSE}
# Graphique de série temporelle
p1 <- plot_temporal_trend(temporal_data, indicator = "carbon")
ggsave("figures/carbon_trend_2015_2025.png", p1, width = 10, height = 6, dpi = 300)
```

# Résumé

Le workflow d'analyse temporelle `nemeton` permet :

1. **Gestion multi-période** : Suivre les indicateurs sur 2+ périodes temporelles
2. **Quantification du changement** : Calculer les taux absolus et relatifs
3. **Visualisation des tendances** : Graphiques de séries temporelles
4. **Évaluation des interventions** : Évaluer les impacts de gestion
5. **Intégration** : Combiner avec le système multi-famille pour une analyse complète

## Fonctions clés

- `nemeton_temporal()` : Créer des datasets multi-périodes
- `calculate_change_rate()` : Calculer les taux de changement absolu/relatif
- `plot_temporal_trend()` : Graphiques de lignes de séries temporelles pour toutes les parcelles

## Prochaines étapes

- Explorer le système multi-famille : `vignette("indicator-families_fr")`
- Apprendre les workflows de base : `vignette("getting-started_fr")`
- Consulter la documentation : `help(package = "nemeton")`

# Session Info

```{r, eval = FALSE}
sessionInfo()
```
