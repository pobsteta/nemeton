...
**Interprétation** :
- R1, R2, R3 > 3 ou > 60% : Vulnérabilité élevée
...
La fonction `nemeton_radar()` avec `mode = "family"` permet de visualiser le profil complet d'une parcelle sur les 12 familles :
...
**Interprétation** :
- **Corrélation positive (rouge)** : Synergies (ex: Carbone × Production)
- **Corrélation négative (bleu)** : Conflits/trade-offs
...
- **Inverser** les indicateurs où faible = mieux (risques, fragmentation)
...
- Optimisation multi-critère : `vignette("multi-criteria-optimization_fr")`
