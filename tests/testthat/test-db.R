...
#   * SQLite local file (skipped without the RSQLite package)
...
test_that(".detect_driver rejects removed DuckDB URLs with guidance", {
  expect_error(nemeton:::.detect_driver("duckdb:///tmp/x.duckdb"),
               "DuckDB monitoring backend was removed")
  expect_error(nemeton:::.detect_driver("/tmp/x.duckdb"),
               "DuckDB monitoring backend was removed")
})
...
# ---- read_only argument validation -----------------------------------

test_that("db_connect read_only must be a single logical", {
  expect_error(db_connect("sqlite:///tmp/x.sqlite", read_only = NA),
               "single")
  expect_error(db_connect("sqlite:///tmp/x.sqlite", read_only = "yes"),
               "single")
})
