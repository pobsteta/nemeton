...
| Ancien nom | [`indicateur_l1_sylvosphere()`](https://pobsteta.github.io/nemeton/reference/indicateur_l1_sylvosphere.md) — alias conservé (spec 045) |
...
3.  **Le nom a changé, et les deux anciens noms étaient croisés**
    (spec 045) : l’alias de L2 est
    [`indicateur_l1_sylvosphere()`](https://pobsteta.github.io/nemeton/reference/indicateur_l1_sylvosphere.md)
    — celui qui *dit* L1 — et le slug `indicateur_l1_sylvosphere` reste
    reconnu par la normalisation, en passthrough comme L2. Ne pas le
    confondre avec `indicateur_l2_fragmentation`, qui porte les valeurs
    de **L1** et s’inverse depuis la 0.197.0 (spec 048 §9). Attention au
    sens, enfin : « fragmentation » suggère « haut = fragmenté », alors
    qu’un L2 élevé signifie **peu** morcelé. C’est précisément la raison
    du renommage.
