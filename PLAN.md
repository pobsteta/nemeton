...
> **Reste** : brief app `nemetonshiny` pour brancher `pai.tif` → `lai_max` et
> `source = "soilgrids"` dans `service_regeneration.R` (le garde-fou
> `is.null(lai_max) && is.null(grid)` y saute le LAI dès qu'un LiDAR existe).
