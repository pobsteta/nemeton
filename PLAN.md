...
**État au 2026-05-13** : `nemeton@main = 5f02449`, dernière release **v0.21.9** (tag poussé sur `origin`, DESCRIPTION + NEWS.md alignés). Working tree propre. Pas de chantier d'épaississement actif.

Série de patches **v0.21.1 → v0.21.9** livrée entre 2026-05-12 et 2026-05-13 (branche `claude/fix-duckdb-sql-error-cpXZ7`, mergée sur `main`). Le fil rouge est *« faire rentrer une ingestion S2 longue sans casse »* — les symptômes observés étaient des scènes silencieusement skippées, des hoquets DNS / 5xx fatals, et un cache COG qui ne servait pas à grand-chose parce que la branche cache-hit plantait.
