...
- chm:
  ...
  Optional `SpatRaster` of canopy heights in metres. When supplied, the
  CV(CHM) per unit is computed and blended into the final B2 score with
  weight `cv_chm_weight`. If strata/age fields are absent, the CV(CHM)
  becomes the primary score (spec 005 phase 4).

- cv_chm_weight:

  Numeric in `[0, 1]`. Additive weight of the CV(CHM) component in the
  final B2 score. Default `0.2`. Ignored when `chm` is `NULL`.
