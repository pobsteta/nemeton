...
1. **Option A (recommandée pour débuter)** : Utiliser la **zone d'étude prédéfinie** fournie avec le package nemeton (~450 ha, cluster Verc-02)

2. **Option B** : Créer votre propre zone à partir des données **Quatre Montagnes** de `lidaRtRee` (~1600 ha, groupe Centre)

Les données Quatre Montagnes sont situées dans le **Vercors** (Isère, France) et comprennent 96 placettes forestières avec des métriques LiDAR pré-calculées.
...
La première étape consiste à définir notre **zone d'étude**. Vous pouvez :

- **Option A** : Charger la zone prédéfinie du package nemeton (plus rapide)
- **Option B** : Créer votre propre zone à partir des données `quatre_montagnes`

---

### Option A : Zone d'étude prédéfinie (recommandé)

Le package nemeton fournit une zone d'étude prête à l'emploi dans `inst/extdata/zone_etude.gpkg`. Cette zone correspond au cluster **Verc-02** avec un buffer de 500m, couvrant environ **450 hectares**.
...
```{r ex-option-a, exercise=TRUE, exercise.eval=FALSE, exercise.lines=35, exercise.setup="ex-option-a-setup"}
# === OPTION A : CHARGER LA ZONE PRÉDÉFINIE ===
...
if (file.exists(zone_pkg)) {
  ...
  # Sauvegarder dans le répertoire de cache
  if (requireNamespace("rappdirs", quietly = TRUE)) {
    data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
  } else {
    data_dir <- file.path(path.expand("~"), "nemeton_tutorial_data")
  ...
  cat("Fichier non trouvé. Utilisez l'Option B ci-dessous.\n")
}
```

### Visualisation de la zone d'étude (Option A)

```{r viz-option-a-setup}
library(sf)
library(ggplot2)
...
```{r viz-option-a, exercise=TRUE, exercise.eval=FALSE, exercise.lines=25, exercise.setup="viz-option-a-setup"}
...
**Après avoir exécuté ces exercices, passez directement à la Section 2 (Cadastre).**

---

### Option B : Créer sa propre zone d'étude

Si vous préférez créer votre propre zone ou si le package nemeton n'est pas installé, suivez les exercices ci-dessous.
...
cat(
  "Dimensions:", nrow(quatre_montagnes), "placettes x",
...
st_write(zone_etude_sf, filepath,
  layer = "zone_etude",
  delete_layer = TRUE, quiet = TRUE
)
...
st_write(placettes_centre, filepath,
  layer = "placettes",
...
ggplot() +
  # Zone d'étude (buffer)
  geom_sf(
    data = zone_etude_sf, fill = "lightgreen", alpha = 0.3,
    color = "darkgreen", linewidth = 1
  ) +
  # Enveloppe convexe
  geom_sf(
    data = st_sf(geometry = enveloppe), fill = NA,
  ...
  labs(
    ...
    subtitle = paste(
      nrow(placettes_centre), "placettes -",
      round(as.numeric(st_area(zone_etude_sf)) / 10000), "ha"
    ),
    x = "X (m)", y = "Y (m)"
  ) +
...
if (!is.null(mnt)) {
  ...
  ggplot(mnt_df, aes(x = x, y = y, fill = altitude)) +
    ...
    coord_sf(datum = NA) +
    labs(
      title = "MNT RGE Alti 5m", subtitle = "Zone d'étude Quatre Montagnes",
      x = "X (m)", y = "Y (m)"
    ) +
    theme_minimal()
} else {
