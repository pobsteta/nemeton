...
Nous utiliserons les données **Quatre Montagnes** du package `lidaRtRee`, situées dans le **Vercors** (Isère, France). Cette zone comprend 96 placettes forestières avec des métriques LiDAR pré-calculées.

Pour ce tutoriel, nous travaillerons sur un sous-ensemble de **28 placettes** (groupe Centre) couvrant environ **1600 hectares**.
...
```{r show-data-dir, exercise=TRUE, exercise.lines=20}
...
La première étape consiste à définir notre **zone d'étude**. Nous allons :

1. Charger les placettes `quatre_montagnes` de lidaRtRee
2. Sélectionner un groupe de clusters (groupe Centre)
3. Créer un polygone englobant avec un buffer

### Les données quatre_montagnes

Le jeu de données `quatre_montagnes` contient **96 placettes forestières** inventoriées dans le massif du Vercors, avec plus de 70 variables incluant :

- **Coordonnées** : X, Y (Lambert-93)
- **Métriques dendrométriques** : G_m2_ha (surface terrière), N_ha (densité)
- **Métriques LiDAR** : zmax, zmean, zsd, percentiles (zq5 à zq95)
- **Topographie** : altitude, pente, exposition

### Exercice 1.1 : Explorer les données

Chargez et explorez les données `quatre_montagnes` :

```{r ex-1-1, exercise=TRUE, exercise.lines=15}
# Charger les données
data("quatre_montagnes", package = "lidaRtRee")

# Structure des données
cat("Dimensions:", nrow(quatre_montagnes), "placettes x",
   ncol(quatre_montagnes), "variables\n\n")

# Aperçu des premières colonnes
cat("Variables principales:\n")
cat(paste(names(quatre_montagnes)[1:10], collapse = ", "), "\n\n")

# Clusters disponibles
cat("Clusters:", length(unique(quatre_montagnes$cluster_id)), "\n")
cat(paste(unique(quatre_montagnes$cluster_id), collapse = ", "))
```
...
### Exercice 1.2 : Créer un objet sf

Convertissez les placettes en objet spatial `sf` :
...
```{r ex-1-2, exercise=TRUE, exercise.lines=12, exercise.setup="ex-1-2-setup"}
# Convertir en sf avec les colonnes X et Y
placettes_sf <- st_as_sf(
 quatre_montagnes,
 coords = c("X", "Y"),
 crs = 2154  # Lambert-93
)
...
# Vérifier
cat("Classe:", class(placettes_sf)[1], "\n")
cat("CRS:", st_crs(placettes_sf)$input, "\n")
cat("Nombre de points:", nrow(placettes_sf), "\n")
```
...
grade_this({
 if (!inherits(.result, "sf")) {
   fail("L'objet doit être de classe 'sf'. Utilisez st_as_sf().")
 }
 if (st_crs(.result)$epsg != 2154) {
   fail("Le CRS doit être EPSG:2154 (Lambert-93).")
 }
 pass("Excellent ! Vous avez créé un objet sf en Lambert-93.")
})
```

### Exercice 1.3 : Sélectionner le groupe Centre

Sélectionnez les clusters du **groupe Centre** pour créer une zone d'étude de taille raisonnable :

```{r ex-1-3-setup}
library(sf)
library(lidaRtRee)
data("quatre_montagnes", package = "lidaRtRee")
placettes_sf <- st_as_sf(quatre_montagnes, coords = c("X", "Y"), crs = 2154)
```

```{r ex-1-3, exercise=TRUE, exercise.lines=18, exercise.setup="ex-1-3-setup"}
# Clusters du groupe Centre (altitude intermédiaire)
clusters_centre <- c("Verc-C1", "Verc-S4", "Verc-02", "Verc-S7",
                    "Verc-C4", "Verc-C2", "Verc-05")

# Filtrer les placettes
placettes_centre <- placettes_sf[placettes_sf$cluster_id %in% clusters_centre, ]

cat("Placettes sélectionnées:", nrow(placettes_centre), "\n")

# Calculer l'emprise
bbox <- st_bbox(placettes_centre)
cat("\nEmprise (Lambert-93):\n")
cat(" xmin:", round(bbox["xmin"]), "\n")
cat(" ymin:", round(bbox["ymin"]), "\n")
cat(" xmax:", round(bbox["xmax"]), "\n")
cat(" ymax:", round(bbox["ymax"]), "\n")
cat("\nÉtendue:", round(bbox["xmax"] - bbox["xmin"]), "m x",
...
grade_this({
  ...
  if (nrow(.result) < 20 || nrow(.result) > 35) {
    fail(paste0("Nombre de placettes (", nrow(.result), ") inattendu. ",
               "Vérifiez les clusters sélectionnés."))
  }
  pass(paste0("Parfait ! Vous avez sélectionné ", nrow(.result), " placettes du groupe Centre."))
})
```

### Exercice 1.4 : Créer le contour de la zone d'étude

Créez un polygone englobant les placettes avec un **buffer de 500m** :
...
```{r ex-1-4, exercise=TRUE, exercise.lines=20, exercise.setup="ex-1-4-setup"}
# Créer l'enveloppe convexe des placettes
enveloppe <- st_convex_hull(st_union(placettes_centre))

# Ajouter un buffer de 500m
zone_etude <- st_buffer(enveloppe, dist = 500)

# Calculer la surface
surface_ha <- as.numeric(st_area(zone_etude)) / 10000

cat("Zone d'étude créée !\n")
cat("Surface:", round(surface_ha), "hectares\n")

# Convertir en sf pour manipulation ultérieure
zone_etude_sf <- st_sf(
 id = 1,
 nom = "Quatre Montagnes - Centre",
 geometry = zone_etude
)
...
grade_this({
 if (!inherits(.result, "sf")) {
   fail("L'objet doit être de classe 'sf'.")
 }
 area <- as.numeric(st_area(.result)) / 10000
 if (area < 1000 || area > 2500) {
   fail(paste0("La surface (", round(area), " ha) semble incorrecte. ",
              "Vérifiez le buffer de 500m."))
 }
 pass("Parfait ! Votre zone d'étude de ~1600 ha est prête.")
})
```

### Exercice 1.5 : Sauvegarder la zone d'étude

**Pattern de cache** : Pour éviter de recréer la zone à chaque exercice, nous allons la sauvegarder dans un répertoire local persistant. Ce pattern sera utilisé tout au long du tutoriel.
...
enveloppe <- st_convex_hull(st_union(placettes_centre))
zone_etude <- st_buffer(enveloppe, dist = 500)
zone_etude_sf <- st_sf(id = 1, nom = "Quatre Montagnes - Centre", geometry = zone_etude)
```

```{r ex-1-5, exercise=TRUE, exercise.lines=25, exercise.setup="ex-1-5-setup"}
...
cat("=== Sauvegarde de la zone d'étude ===\n")
cat("Répertoire:", data_dir, "\n")
cat("Fichier:", filepath, "\n\n")

# Sauvegarder la zone d'étude (couche 1)
st_write(zone_etude_sf, filepath, layer = "zone_etude",
         delete_layer = TRUE, quiet = TRUE)
cat("Zone d'étude sauvegardée (couche: zone_etude)\n")

# Sauvegarder les placettes (couche 2 dans le même fichier)
st_write(placettes_centre, filepath, layer = "placettes",
         append = TRUE, quiet = TRUE)
cat("Placettes sauvegardées (couche: placettes)\n")

# Vérifier le contenu du GeoPackage
cat("\n=== Contenu du fichier ===\n")
couches <- st_layers(filepath)
...
Chargeons les données depuis le cache pour visualiser :

```{r viz-zone-setup}
library(sf)
library(ggplot2)
# Définir le répertoire de cache (même code que dans l'exercice précédent)
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
} else {
  data_dir <- file.path(path.expand("~"), "nemeton_tutorial_data")
...
filepath <- file.path(data_dir, "zone_etude.gpkg")
zone_etude_sf <- st_read(filepath, layer = "zone_etude", quiet = TRUE)
placettes_centre <- st_read(filepath, layer = "placettes", quiet = TRUE)
enveloppe <- st_convex_hull(st_union(placettes_centre))
```

```{r viz-zone, exercise=TRUE, exercise.lines=20, exercise.setup="viz-zone-setup"}
# Visualiser la zone d'étude avec les placettes
ggplot() +
  # Zone d'étude (buffer)
  geom_sf(data = zone_etude_sf, fill = "lightgreen", alpha = 0.3,
          color = "darkgreen", linewidth = 1) +
  # Enveloppe convexe
  geom_sf(data = st_sf(geometry = enveloppe), fill = NA,
          color = "darkgreen", linetype = "dashed") +
  # Placettes
  geom_sf(data = placettes_centre, aes(color = cluster_id), size = 3) +
  # Forcer l'affichage en Lambert-93 (mètres)
  coord_sf(datum = NA) +
  # Mise en forme
  scale_color_viridis_d(name = "Cluster") +
  labs(
    title = "Zone d'étude : Quatre Montagnes (Centre)",
    subtitle = paste(nrow(placettes_centre), "placettes -",
...
quiz(
 ...
 question("Pourquoi ajoute-t-on un buffer autour des placettes ?",
   answer("Pour réduire la taille de la zone"),
   answer("Pour inclure le contexte spatial autour des points d'intérêt", correct = TRUE),
   answer("Pour changer le système de coordonnées"),
   answer("Pour fusionner les placettes"),
   allow_retry = TRUE
 ),
 question("Quelle fonction sf permet de créer une enveloppe convexe ?",
   answer("st_buffer()"),
   answer("st_union()"),
   answer("st_convex_hull()", correct = TRUE),
   answer("st_intersection()"),
   allow_retry = TRUE
 )
)
```
...
library(sf)
library(happign)
# Charger la zone d'étude depuis le cache
# Définir le répertoire de cache
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
} else {
...
```{r ex-2-1, exercise=TRUE, exercise.lines=35, exercise.setup="ex-2-1-setup"}
# === PATTERN DE CACHE ===
# Définir le répertoire de cache
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
...
cat(paste(names(parcelles)[1:min(8, ncol(parcelles))], collapse = ", "), "\n")
...
grade_this({
 ...
 if (nrow(.result) < 10) {
   fail("Trop peu de parcelles téléchargées. Vérifiez la zone d'étude.")
 }
 pass(paste0("Bravo ! Vous avez téléchargé ", nrow(.result), " parcelles cadastrales."))
})
```
...
```{r ex-2-2, exercise=TRUE, exercise.lines=22, exercise.setup="ex-2-2-setup"}
...
placettes_centre <- st_read(file.path(data_dir, "zone_etude.gpkg"), layer = "placettes", quiet = TRUE)
```

```{r viz-parcelles, exercise=TRUE, exercise.lines=16, exercise.setup="viz-parcelles-setup"}
# Les données sont chargées depuis le cache (setup)
parcelles$surface_ha <- as.numeric(st_area(parcelles)) / 10000

ggplot() +
  geom_sf(data = parcelles, aes(fill = surface_ha), color = "white", linewidth = 0.2) +
  geom_sf(data = placettes_centre, color = "red", size = 2) +
  coord_sf(datum = NA) +
...
dir.create(data_dir, showWarnings = FALSE, recursive = TRUE)
zone_etude_sf <- st_read(file.path(data_dir, "zone_etude.gpkg"), layer = "zone_etude", quiet = TRUE)
```
...
if (file.exists(mnt_filepath)) {
  ...
  mnt <- get_wms_raster(
    x = zone_etude_sf,
    layer = "ELEVATION.ELEVATIONGRIDCOVERAGE.HIGHRES",
    res = 5, crs = 2154, rgb = FALSE
  )
  writeRaster(mnt, mnt_filepath, overwrite = TRUE)
}

cat("Dimensions:", nrow(mnt), "x", ncol(mnt), "\n")
```
...
grade_this({
 ...
 pass("Excellent ! MNT téléchargé à 5m de résolution.")
})
```

### Exercice 3.2 : Calculer pente et exposition
...
```{r ex-3-2, exercise=TRUE, exercise.lines=22, exercise.setup="ex-3-2-setup"}
...
# Combiner dans un stack
topo <- c(mnt, pente, exposition)
names(topo) <- c("altitude", "pente", "exposition")
...
grade_this({
  ...
  if (nlyr(.result) != 3) {
    fail(paste0("Le stack doit contenir 3 couches (altitude, pente, exposition). ",
               "Vous avez ", nlyr(.result), " couche(s)."))
  }
  expected_names <- c("altitude", "pente", "exposition")
  if (!all(names(.result) == expected_names)) {
    fail("Les couches doivent être nommées 'altitude', 'pente', 'exposition'.")
  }
  pass("Excellent ! Stack topographique créé avec 3 couches.")
})
```
...
placettes_centre <- tryCatch(
  st_read(file.path(data_dir, "zone_etude.gpkg"), layer = "placettes", quiet = TRUE),
  error = function(e) NULL
)
```

```{r viz-mnt, exercise=TRUE, exercise.lines=15, exercise.setup="viz-mnt-setup"}
if (!is.null(mnt)) {
 ...
 ggplot(mnt_df, aes(x = x, y = y, fill = altitude)) +
   ...
   geom_sf(data = placettes_centre, color = "red", size = 1, inherit.aes = FALSE) +
   coord_sf(datum = NA) +
   labs(title = "MNT RGE Alti 5m", subtitle = "Zone d'étude Quatre Montagnes",
        x = "X (m)", y = "Y (m)") +
   theme_minimal()
} else {
...
```{r ex-4-1, exercise=TRUE, exercise.lines=35, exercise.setup="ex-4-1-setup"}
# === PATTERN DE CACHE ===
# Définir le répertoire de cache
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
...
cat("\nColonnes disponibles:\n")
cat(paste(names(bd_foret)[1:min(8, ncol(bd_foret))], collapse = ", "), "\n")
...
grade_this({
  ...
  if (nrow(.result) < 5) {
    fail("Trop peu de formations végétales. Vérifiez le téléchargement.")
  }
  if (!"tfv" %in% names(.result)) {
    fail("La colonne 'tfv' (type de formation végétale) est absente.")
  }
  pass(paste0("Bravo ! BD Forêt téléchargée avec ", nrow(.result), " formations végétales."))
})
```
...
```{r ex-4-2, exercise=TRUE, exercise.lines=30, exercise.setup="ex-4-2-setup"}
# === PATTERN DE CACHE ===
# Définir le répertoire de cache
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
...
grade_this({
  ...
  if (nrow(.result) < 1) {
    fail("Aucune route téléchargée. Vérifiez la zone d'étude.")
  }
  pass(paste0("Parfait ! ", nrow(.result), " tronçons routiers téléchargés."))
})
```
...
```{r ex-4-3, exercise=TRUE, exercise.lines=30, exercise.setup="ex-4-3-setup"}
# === PATTERN DE CACHE ===
# Définir le répertoire de cache
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
...
grade_this({
  ...
  # Note: 0 cours d'eau est acceptable selon la zone
  pass(paste0("Téléchargement terminé : ", nrow(.result), " cours d'eau."))
})
```
...
```{r viz-bdforet, exercise=TRUE, exercise.lines=15, exercise.setup="viz-bdforet-setup"}
if (!is.null(bd_foret) && nrow(bd_foret) > 0) {
  ggplot() +
    geom_sf(data = bd_foret, aes(fill = tfv), color = NA) +
    geom_sf(data = zone_etude_sf, fill = NA, color = "black", linewidth = 1) +
    geom_sf(data = placettes_centre, color = "red", size = 2) +
    coord_sf(datum = NA) +
...
```{r ex-5-1, exercise=TRUE, exercise.lines=30, exercise.timelimit=600, exercise.setup="ex-5-1-setup"}
# Définir le répertoire de cache
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
} else {
...
if (requireNamespace("lidarHD", quietly = TRUE)) {
  ...
  ta <- load_classified_ta(bbox = st_bbox(st_transform(zone_etude_sf, 4326)),
                           save_dir = data_dir)
  ...
  if (nrow(ta) > 0) {
    ...
    cat("Colonnes disponibles:", paste(names(ta)[1:min(5, ncol(ta))], collapse = ", "), "\n\n")
...
```{r ex-5-2, exercise=TRUE, exercise.lines=45, exercise.timelimit=600, exercise.setup="ex-5-2-setup"}
if (requireNamespace("lidarHD", quietly = TRUE)) {
  library(lidarHD)
  ...
  fichiers_existants <- list.files(lidar_dir, pattern = "\\.copc\\.laz$",
  ...
  if (length(fichiers_existants) >= nrow(ta)) {
    ...
    if (nrow(ta) > 0) {
      ...
      tryCatch({
        ta_result <- download_files(ta, roi = zone_etude_sf,
...
### Alternative : Utiliser les métriques pré-calculées

Si le téléchargement LiDAR est trop long, vous pouvez utiliser les **métriques pré-calculées** dans `quatre_montagnes` :

```{r ex-5-alt, exercise=TRUE, exercise.lines=20}
library(lidaRtRee)
data("quatre_montagnes", package = "lidaRtRee")

# Les métriques LiDAR sont déjà dans le jeu de données
metriques_lidar <- c("zmax", "zmean", "zsd", "zq50", "zq95",
                    "zentropy", "pzabove2", "Tree_density")

cat("=== Métriques LiDAR pré-calculées ===\n\n")

# Afficher les statistiques pour les clusters Centre
clusters_centre <- c("Verc-C1", "Verc-S4", "Verc-02", "Verc-S7",
                    "Verc-C4", "Verc-C2", "Verc-05")
data_centre <- quatre_montagnes[quatre_montagnes$cluster_id %in% clusters_centre, ]

for (m in metriques_lidar) {
 if (m %in% names(data_centre)) {
   vals <- data_centre[[m]]
   cat(sprintf("%s: min=%.1f, moy=%.1f, max=%.1f\n",
              m, min(vals), mean(vals), max(vals)))
 }
}
```
...
```{r ex-6-1, exercise=TRUE, exercise.lines=35, exercise.setup="ex-6-1-setup"}
# === INVENTAIRE DU CACHE ===
# Définir le répertoire de cache
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
...
```{r ex-6-2, exercise=TRUE, exercise.lines=55, exercise.setup="ex-6-2-setup"}
# === EXPORT CONSOLIDÉ ===
# Définir le répertoire de cache
if (requireNamespace("rappdirs", quietly = TRUE)) {
  data_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
...
Dans le **Tutorial 2**, vous apprendrez à :

- Charger et visualiser les nuages de points LiDAR
- Normaliser les hauteurs avec le MNT
- Extraire les métriques dendrométriques par parcelle
- Générer un Modèle Numérique de Hauteur (MNH)
