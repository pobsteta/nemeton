...
```{r ex-2-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=30, exercise.setup="ex-2-1-setup", fig.width=6, fig.height=6}
...
for (i in seq_len(nrow(synergies))) {
  cat(sprintf("  %s - %s : r = %.2f\n",
              synergies$Famille1[i], synergies$Famille2[i], synergies$Correlation[i]))
}
...
if (nrow(compromis) > 0) {
  for (i in seq_len(nrow(compromis))) {
    cat(sprintf("  %s - %s : r = %.2f\n",
                compromis$Famille1[i], compromis$Famille2[i], compromis$Correlation[i]))
  }
} else {
