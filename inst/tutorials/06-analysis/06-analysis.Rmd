...
cat("Parcelles chargées:", nrow(parcelles), "\n")
...
```{r ex-1-2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=35, exercise.setup="ex-1-2-setup"}
...
```{r ex-2-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=55, exercise.setup="ex-2-1-setup"}
# === DIAGRAMME RADAR AVEC fmsb ===

parcelles_path <- file.path(cache_dir, "parcelles.gpkg")
cat("Fichier:", parcelles_path, "\n")
cat("Existe:", file.exists(parcelles_path), "\n\n")

parcelles <- st_read(parcelles_path, quiet = TRUE)
cat("Parcelles chargées:", nrow(parcelles), "\n")
cat("Toutes les colonnes:\n")
print(names(parcelles))

# Vérifier les colonnes family_* disponibles
cols_family <- grep("^family_", names(parcelles), value = TRUE)
cat("\nColonnes family_* disponibles:", length(cols_family), "\n")
if (length(cols_family) > 0) {
  cat("  ", paste(cols_family, collapse = ", "), "\n\n")
}

# Sélectionner une parcelle
parcelle_id <- parcelles$id_parcelle[1]  # Prendre la première parcelle
parcelle_data <- parcelles[parcelles$id_parcelle == parcelle_id, ]
cat("Parcelle sélectionnée:", parcelle_id, "\n")
cat("Nombre de lignes dans parcelle_data:", nrow(parcelle_data), "\n\n")

# Extraire les scores par famille (uniquement celles disponibles)
familles <- c("C", "B", "W", "A", "F", "L", "T", "R", "S", "P", "E", "N")
noms_familles <- c("Carbone", "Biodiv", "Eau", "Air", "Sol", "Paysage",
                   "Temporel", "Risques", "Social", "Prod", "Énergie", "Natural")

# Extraire les données sans géométrie - utiliser directement parcelles (pas le subset)
parcelles_df <- as.data.frame(st_drop_geometry(parcelles))
cat("Colonnes family_* dans parcelles_df:\n")
print(grep("^family_", names(parcelles_df), value = TRUE))

# Extraire les scores avec purrr::map_dbl (garantit un vecteur numérique)
row_idx <- which(parcelles_df$id_parcelle == parcelle_id)[1]
cat("\nIndex de la ligne:", row_idx, "\n")

scores <- map_dbl(familles, ~{
  col_name <- paste0("family_", .x)
  if (col_name %in% names(parcelles_df) && !is.na(row_idx)) {
    as.numeric(parcelles_df[row_idx, col_name])
  } else {
    NA_real_
  }
})
...
# Créer le radar avec fmsb (si au moins quelques scores disponibles)
if (!all(is.na(scores)) && requireNamespace("fmsb", quietly = TRUE)) {
  # Remplacer NA par 0 pour le tracé
  scores_plot <- ifelse(is.na(scores), 0, scores)
...
} else if (all(is.na(scores))) {
  cat("\nAucune colonne family_* trouvée. Avez-vous exécuté le T05 ?\n")
} else {
  cat("\nPackage fmsb non installé. install.packages('fmsb')\n")
}
```
...
```{r ex-2-2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=50, exercise.setup="ex-2-2-setup"}
# === COMPARAISON MULTI-PARCELLES ===

parcelles_path <- file.path(cache_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

# Sélectionner 3 parcelles à comparer (premières disponibles)
parcelles_ids <- head(parcelles$id_parcelle, 3)

# Extraire les scores pour chaque parcelle
familles <- c("C", "B", "W", "A", "F", "L", "T", "R", "S", "P", "E", "N")

extract_scores <- function(data, id) {
  row_df <- as.data.frame(st_drop_geometry(data[data$id_parcelle == id, ]))
  map_dbl(familles, ~{
    col_name <- paste0("family_", .x)
    if (col_name %in% names(row_df)) as.numeric(row_df[1, col_name])
    else NA_real_
  })
}

scores_list <- map(parcelles_ids, ~extract_scores(parcelles, .x))
