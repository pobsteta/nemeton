...
# Extraire les données sans géométrie
parcelle_df <- as.data.frame(st_drop_geometry(parcelle_data))

# Extraire les scores avec purrr::map_dbl (garantit un vecteur numérique)
scores <- map_dbl(familles, ~{
  col_name <- paste0("family_", .x)
  if (col_name %in% names(parcelle_df)) {
    as.numeric(parcelle_df[1, col_name])
  } else {
    NA_real_
  }
})
