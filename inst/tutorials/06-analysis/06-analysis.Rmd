...
# Sélectionner une parcelle (utiliser la première ligne)
parcelle_id <- 1  # Index de la première parcelle
cat("Parcelle sélectionnée (ligne):", parcelle_id, "\n\n")
...
# Extraire les scores avec purrr::map_dbl
scores <- map_dbl(familles, ~{
  col_name <- paste0("family_", .x)
  if (col_name %in% names(parcelles_df)) {
    as.numeric(parcelles_df[parcelle_id, col_name])
  } else {
    NA_real_
  }
})
