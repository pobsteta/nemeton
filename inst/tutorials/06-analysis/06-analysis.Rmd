...
parcelles_sim <- st_sf(
  ...
  moy_C = runif(n, 0.3, 0.9),
  moy_B = runif(n, 0.2, 0.85),
  moy_W = runif(n, 0.25, 0.8),
  moy_A = runif(n, 0.4, 0.75),
  moy_F = runif(n, 0.35, 0.85),
  moy_L = runif(n, 0.3, 0.8),
  moy_T = runif(n, 0.4, 0.9),
  moy_R = runif(n, 0.2, 0.7),
  moy_S = runif(n, 0.3, 0.75),
  moy_P = runif(n, 0.35, 0.85),
  moy_E = runif(n, 0.4, 0.8),
  moy_N = runif(n, 0.25, 0.75),
  I_nemeton = runif(n, 0.35, 0.75),
  geometry = create_grid_polygons(n)
)
```

```{r ex-1-1, exercise=TRUE, exercise.lines=30, exercise.setup="ex-1-1-setup"}
# === CARTES THÉMATIQUES PAR FAMILLE ===

# Charger les données réelles ou utiliser simulation
indicateurs_path <- file.path(cache_dir, "indicateurs_complets.gpkg")

if (file.exists(indicateurs_path)) {
  parcelles <- st_read(indicateurs_path, quiet = TRUE)
...
# À compléter : créer une carte pour la famille Carbone (moy_C)
# Utilisez ggplot2 avec geom_sf() et scale_fill_viridis_c()

carte_carbone <- ggplot(parcelles) +
  geom_sf(aes(fill = ___), color = "white", linewidth = 0.2) +
...
# Carte pour la famille Carbone
carte_carbone <- ggplot(parcelles) +
  geom_sf(aes(fill = moy_C), color = "white", linewidth = 0.2) +
  scale_fill_viridis_c(
    name = "Score Carbone",
    option = "D",
    limits = c(0, 1)
...
```{r ex-1-1-check}
grade_this({
  pass("Carte thématique créée avec succès !")
})
```
...
parcelles_sim <- st_sf(
  id_parcelle = 1:n,
  moy_C = runif(n, 0.3, 0.9), moy_B = runif(n, 0.2, 0.85),
  moy_W = runif(n, 0.25, 0.8), moy_A = runif(n, 0.4, 0.75),
  moy_F = runif(n, 0.35, 0.85), moy_L = runif(n, 0.3, 0.8),
  moy_T = runif(n, 0.4, 0.9), moy_R = runif(n, 0.2, 0.7),
  moy_S = runif(n, 0.3, 0.75), moy_P = runif(n, 0.35, 0.85),
  moy_E = runif(n, 0.4, 0.8), moy_N = runif(n, 0.25, 0.75),
  I_nemeton = runif(n, 0.35, 0.75),
  geometry = create_grid_polygons(n)
)
```

```{r ex-1-2, exercise=TRUE, exercise.lines=40, exercise.setup="ex-1-2-setup"}
...
```{r ex-1-2-check}
grade_this({
  pass("Panneau multi-cartes créé !")
})
```
...
```{r ex-2-1, exercise=TRUE, exercise.lines=45, exercise.setup="ex-2-1-setup"}
...
```{r ex-2-2, exercise=TRUE, exercise.lines=55, exercise.setup="ex-2-2-setup"}
...
```{r ex-2-2-check}
grade_this({
  pass("Comparaison radar réussie !")
})
```
...
parcelles_sim <- st_sf(
  ...
  moy_C = base_eco + rnorm(n, 0, 0.1),
  moy_B = base_eco + rnorm(n, 0, 0.15),
  moy_N = base_eco + rnorm(n, 0, 0.12),
  # Corrélées positivement (production)
  moy_P = base_prod + rnorm(n, 0, 0.1),
  moy_E = base_prod + rnorm(n, 0, 0.12),
  # Indépendantes
  moy_W = runif(n, 0.25, 0.8),
  moy_A = runif(n, 0.4, 0.75),
  moy_F = runif(n, 0.35, 0.85),
  moy_L = runif(n, 0.3, 0.8),
  moy_T = runif(n, 0.4, 0.9),
  moy_R = 1 - base_eco + rnorm(n, 0, 0.1),  # Corrélé négativement
  moy_S = runif(n, 0.3, 0.75),
  I_nemeton = runif(n, 0.35, 0.75),
  geometry = create_grid_polygons(n)
)
...
for (col in grep("^moy_", names(parcelles_sim), value = TRUE)) {
...
cols_familles <- grep("^moy_", names(parcelles), value = TRUE)
data_familles <- st_drop_geometry(parcelles)[, cols_familles]

# Renommer les colonnes pour lisibilité
colnames(data_familles) <- gsub("moy_", "", colnames(data_familles))
...
```{r ex-3-1-check}
grade_this({
  pass("Matrice de corrélation calculée !")
})
```
...
parcelles_sim <- st_sf(
  ...
  moy_L = runif(n, 0.3, 0.8),
  moy_T = runif(n, 0.4, 0.9),
  moy_R = 1 - base_eco + rnorm(n, 0, 0.1),
  moy_S = runif(n, 0.3, 0.75),
  I_nemeton = runif(n, 0.35, 0.75),
  geometry = create_grid_polygons(n)
)
...
```{r ex-3-2, exercise=TRUE, exercise.lines=30, exercise.setup="ex-3-2-setup"}
...
```{r ex-3-2-check}
grade_this({
  pass("Visualisation corrplot créée !")
})
```
...
parcelles_sim <- st_sf(
  id_parcelle = 1:n,
  surface_ha = runif(n, 0.5, 10),
  moy_P = base + rnorm(n, 0, 0.15),  # Production
  moy_B = (1 - base) + rnorm(n, 0, 0.15),  # Biodiversité (trade-off)
  moy_C = runif(n, 0.3, 0.9),
  I_nemeton = runif(n, 0.35, 0.75),
  geometry = create_grid_polygons(n)
)
...
# Normaliser
parcelles_sim$moy_P <- pmax(0, pmin(1, parcelles_sim$moy_P))
parcelles_sim$moy_B <- pmax(0, pmin(1, parcelles_sim$moy_B))
```

```{r ex-5-1, exercise=TRUE, exercise.lines=30, exercise.setup="ex-5-1-setup"}
# === GRAPHIQUE TRADE-OFF ===
...
tradeoff_plot <- ggplot(data_plot, aes(x = moy_P, y = moy_B)) +
...
cor_PB <- cor(data_plot$moy_P, data_plot$moy_B, use = "complete.obs")
cat("\nCorrélation P-B:", round(cor_PB, 2))
```

```{r ex-5-1-check}
grade_this({
  pass("Trade-off visualisé !")
})
```
...
parcelles_sim <- st_sf(
  id_parcelle = 1:n,
  surface_ha = runif(n, 0.5, 10),
  moy_P = pmax(0, pmin(1, base + rnorm(n, 0, 0.15))),
  moy_B = pmax(0, pmin(1, (1 - base) + rnorm(n, 0, 0.15))),
  moy_C = runif(n, 0.3, 0.9),
  I_nemeton = runif(n, 0.35, 0.75),
  geometry = create_grid_polygons(n)
)
...
```{r ex-5-2, exercise=TRUE, exercise.lines=45, exercise.setup="ex-5-2-setup"}
# === FRONT DE PARETO ===
...
data_plot$is_pareto <- identify_pareto(data_plot$moy_P, data_plot$moy_B)

cat("Points sur le front de Pareto:", sum(data_plot$is_pareto), "/", nrow(data_plot), "\n")

# Visualiser
pareto_plot <- ggplot(data_plot, aes(x = moy_P, y = moy_B)) +
...
```{r ex-5-2-check}
grade_this({
  pass("Front de Pareto identifié !")
})
```
...
```{r ex-6-1, exercise=TRUE, exercise.lines=40, exercise.setup="ex-6-1-setup"}
...
# Ajouter clusters
cols_familles <- grep("^moy_", names(parcelles_sim), value = TRUE)
...
```{r ex-6-2, exercise=TRUE, exercise.lines=25, exercise.setup="ex-6-2-setup"}
# === CARTE DES CLUSTERS ===
...
```{r ex-7-1, exercise=TRUE, exercise.lines=35, exercise.setup="ex-7-1-setup"}
...
```{r ex-7-1-check}
grade_this({
  pass("Export GeoPackage préparé !")
})
```
...
parcelles_sim <- data.frame(
  ...
  surface_ha = runif(n, 0.5, 10),
  moy_C = runif(n, 0.3, 0.9), moy_B = runif(n, 0.2, 0.85),
  moy_W = runif(n, 0.25, 0.8), moy_P = runif(n, 0.35, 0.85),
  I_nemeton = runif(n, 0.35, 0.75)
)
parcelles_sim$classe <- cut(parcelles_sim$I_nemeton,
  breaks = c(0, 0.4, 0.55, 0.7, 1),
...
```{r ex-7-2, exercise=TRUE, exercise.lines=25, exercise.setup="ex-7-2-setup"}
# === EXPORT CSV ===
...
cols_export <- c("id_parcelle", "surface_ha",
                 grep("^moy_", names(data_export), value = TRUE),
                 "I_nemeton", "classe")
...
parcelles_sim <- st_sf(
  ...
  moy_C = round(runif(n, 0.3, 0.9), 2),
  moy_B = round(runif(n, 0.2, 0.85), 2),
...
```{r ex-8-1, exercise=TRUE, exercise.lines=45, exercise.setup="ex-8-1-setup"}
