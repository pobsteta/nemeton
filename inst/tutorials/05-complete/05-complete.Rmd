...
Parcelles (base) — clé: idu
     │
     ├── + Métriques LiDAR (T02) → C1, P1, P3, E1, A1
     │
...
# Charger les données des tutoriels précédents
parcelles <- st_read(file.path(cache_dir, "parcelles.gpkg"), quiet = TRUE)
metriques_lidar <- st_read(file.path(cache_dir, "metriques_lidar.gpkg"), quiet = TRUE)
indic_terrain <- st_read(file.path(cache_dir, "indicateurs_terrain.gpkg"), quiet = TRUE)
indic_eco <- st_read(file.path(cache_dir, "indicateurs_ecologiques.gpkg"), quiet = TRUE)
...
```{r assemble-indicators, exercise=TRUE, exercise.lines=50, exercise.timelimit=300, exercise.setup="assemble-indicators-setup"}
# ============================================================
# ASSEMBLAGE DES 32 INDICATEURS NEMETON
# Clé de jointure : idu (identifiant cadastral unique)
# ============================================================
...
lidar_data <- st_drop_geometry(metriques_lidar)[, intersect(cols_lidar, names(metriques_lidar))]
indicateurs <- merge(indicateurs, lidar_data, by = "idu", all.x = TRUE)
cat("✓ LiDAR: 5 indicateurs\n")

# 3. Joindre les indicateurs terrain (T03) → W1-3, R1-4, S1-3, P2, F1
cols_terrain <- c("idu", "W1", "W2", "W3", "R1", "R2", "R3", "R4", "S1", "S2", "S3", "P2", "F1")
terrain_data <- st_drop_geometry(indic_terrain)[, intersect(cols_terrain, names(indic_terrain))]
indicateurs <- merge(indicateurs, terrain_data, by = "idu", all.x = TRUE)
cat("✓ Terrain: 12 indicateurs\n")

# 4. Joindre les indicateurs écologiques (T04) → B1-3, L1-3, C2, T1-2, A2, F2, N1-3
cols_eco <- c("idu", "B1", "B2", "B3", "L1", "L2", "L3", "C2", "T1", "T2", "A2", "F2", "N1", "N2", "N3")
eco_data <- st_drop_geometry(indic_eco)[, intersect(cols_eco, names(indic_eco))]
indicateurs <- merge(indicateurs, eco_data, by = "idu", all.x = TRUE)
cat("✓ Écologique: 14 indicateurs\n")

# ============================================================
# BILAN
# ============================================================
indicateurs_attendus <- c("C1", "P1", "P3", "E1", "A1",
...
cat("Indicateurs:", presents, "/ 31 (+ E2 à calculer = 32)\n")
```

```{r assemble-indicators-solution}
# Assemblage simplifié avec idu comme clé de jointure
indicateurs <- parcelles

# Joindre LiDAR
cols_lidar <- c("idu", "C1", "P1", "P3", "E1", "A1")
indicateurs <- merge(indicateurs, st_drop_geometry(metriques_lidar)[, intersect(cols_lidar, names(metriques_lidar))], by = "idu", all.x = TRUE)

# Joindre Terrain
cols_terrain <- c("idu", "W1", "W2", "W3", "R1", "R2", "R3", "R4", "S1", "S2", "S3", "P2", "F1")
indicateurs <- merge(indicateurs, st_drop_geometry(indic_terrain)[, intersect(cols_terrain, names(indic_terrain))], by = "idu", all.x = TRUE)

# Joindre Écologique
cols_eco <- c("idu", "B1", "B2", "B3", "L1", "L2", "L3", "C2", "T1", "T2", "A2", "F2", "N1", "N2", "N3")
indicateurs <- merge(indicateurs, st_drop_geometry(indic_eco)[, intersect(cols_eco, names(indic_eco))], by = "idu", all.x = TRUE)

cat("Assemblage terminé:", ncol(indicateurs), "colonnes\n")
```
...
grade_this({
  ...
  pass(paste0("Assemblage réussi ! ", ncol(.result), " colonnes."))
})
```
