...
description: >
  Assemblage des 31 indicateurs, calcul E2 (évitement carbone), normalisation
...
- **T02** : Métriques LiDAR (C1, P1, P3, B2, E1, A1)
- **T03** : Indicateurs terrain (W1-W3, R1-R3, S1-S3, P2, F1)
- **T04** : Indicateurs écologiques (B1-B3, L1-L3, C2, T1-T2, A2, F2, N1-N3)
...
familles <- data.frame(
  ...
  Nb_Indicateurs = c(3, 3, 3, 2, 2, 2, 2, 3, 3, 3, 2, 3),
...
Parcelles (base)
     │
     ├── + Métriques LiDAR (T02)
     │       → C1, P1, P3, B2_lidar, E1, A1
     │
     ├── + Indicateurs Terrain (T03)
     │       → W1, W2, W3, R1, R2, R3, S1, S2, S3, P2, F1
     │
     ├── + Indicateurs Écologiques (T04)
     │       → B1, B2, B3, L1, L2, L3, C2, T1, T2, A2, F2, N1, N2, N3
     │
     └── + Calcul E2 (T05)
             → E2 (évitement carbone = E1 × facteur substitution)
")
```
...
```{r assemble-indicators, exercise=TRUE, exercise.lines=25}
# À compléter : assembler les indicateurs
# Commencer par les parcelles de base
indicateurs_complet <- parcelles

# Joindre les métriques LiDAR (conserver uniquement les colonnes d'indicateurs)
# Utiliser st_join() ou merge() selon la structure

# Joindre les indicateurs terrain

# Joindre les indicateurs écologiques
...
# Assembler les indicateurs par jointure attributaire
# Supposons que toutes les couches ont un identifiant 'id_parcelle'

indicateurs_complet <- parcelles

# Joindre métriques LiDAR
if (exists("metriques_lidar")) {
  ...
  if (length(cols_present) > 1) {
    indicateurs_complet <- merge(
      indicateurs_complet,
      st_drop_geometry(metriques_lidar)[, cols_present],
      by = "id_parcelle",
      all.x = TRUE
    )
  }
...
# Joindre indicateurs terrain
if (exists("indic_terrain")) {
  cols_terrain <- c("id_parcelle", "W1", "W2", "W3", "R1", "R2", "R3",
                    "S1", "S2", "S3", "P2", "F1")
  cols_present <- intersect(cols_terrain, names(indic_terrain))
  if (length(cols_present) > 1) {
    indicateurs_complet <- merge(
      indicateurs_complet,
      st_drop_geometry(indic_terrain)[, cols_present],
      by = "id_parcelle",
      all.x = TRUE
    )
  }
...
# Joindre indicateurs écologiques
if (exists("indic_eco")) {
  cols_eco <- c("id_parcelle", "B1", "B2", "B3", "L1", "L2", "T1", "T2",
                "A2", "F2", "N1", "N2", "N3")
  cols_present <- intersect(cols_eco, names(indic_eco))
  if (length(cols_present) > 1) {
    indicateurs_complet <- merge(
      indicateurs_complet,
      st_drop_geometry(indic_eco)[, cols_present],
      by = "id_parcelle",
      all.x = TRUE
    )
  }
...
indicateurs_negatifs <- c("R1", "R2", "R3", "F1", "L1")  # L1 = fragmentation
...
indic_norm <- data.frame(
  ...
  R1 = runif(n), R2 = runif(n), R3 = runif(n)
)
...
indic_norm$moy_R <- rowMeans(indic_norm[, c("R1", "R2", "R3")])
