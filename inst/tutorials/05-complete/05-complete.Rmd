...
if (file.exists(file.path(cache_dir, "parcelles.gpkg"))) {
  parcelles <- st_read(file.path(cache_dir, "parcelles.gpkg"), quiet = TRUE)
}
...
```{r assemble-indicators, exercise=TRUE, exercise.lines=120, exercise.timelimit=300, exercise.setup="assemble-indicators-setup"}
# ============================================================
# ASSEMBLAGE DES 32 INDICATEURS NEMETON
# ============================================================

# Fonction pour trouver la colonne ID commune
find_id_col <- function(df) {
  id_cols <- c("id_parcelle", "ID", "id", "fid", "FID", "OBJECTID")
  for (col in id_cols) {
...
# Ajouter id_parcelle si absent
if (!"id_parcelle" %in% names(parcelles)) {
  parcelles$id_parcelle <- seq_len(nrow(parcelles))
}

indicateurs_complet <- parcelles
cat("Parcelles:", nrow(indicateurs_complet), "lignes\n")

# ============================================================
# 1. MÉTRIQUES LIDAR (T02) → C1, P1, P3, E1, A1
# ============================================================
if (exists("metriques_lidar") && nrow(metriques_lidar) > 0) {
  # Ajouter id_parcelle si absent
  if (!"id_parcelle" %in% names(metriques_lidar)) {
    metriques_lidar$id_parcelle <- seq_len(nrow(metriques_lidar))
  }
...
# ============================================================
if (exists("indic_terrain") && nrow(indic_terrain) > 0) {
  if (!"id_parcelle" %in% names(indic_terrain)) {
    indic_terrain$id_parcelle <- seq_len(nrow(indic_terrain))
  }
  ...
  if (length(cols_present) > 1) {
    ...
    indicateurs_complet <- merge(indicateurs_complet, terrain_data,
                                  by = "id_parcelle", all.x = TRUE)
    cat("✓ Terrain:", paste(setdiff(cols_present, "id_parcelle"), collapse = ", "), "\n")
  }
} else {
  cat("⚠ Pas de données Terrain\n")
}

# ============================================================
# 3. INDICATEURS ÉCOLOGIQUES (T04) → B1-3, L1-3, C2, T1-2, A2, F2, N1-3
# ============================================================
if (exists("indic_eco") && nrow(indic_eco) > 0) {
  if (!"id_parcelle" %in% names(indic_eco)) {
    indic_eco$id_parcelle <- seq_len(nrow(indic_eco))
  }
  ...
  if (length(cols_present) > 1) {
    ...
    indicateurs_complet <- merge(indicateurs_complet, eco_data,
                                  by = "id_parcelle", all.x = TRUE)
    cat("✓ Écologique:", paste(setdiff(cols_present, "id_parcelle"), collapse = ", "), "\n")
  }
} else {
  cat("⚠ Pas de données Écologiques\n")
}
...
cat("\n=== BILAN ===\n")
cat("Indicateurs:", presents, "/ 31 (+ E2 = 32)\n")
cat("Colonnes:", ncol(indicateurs_complet), "\n")
```

```{r assemble-indicators-solution}
# Solution identique à l'exercice - voir code ci-dessus
```
