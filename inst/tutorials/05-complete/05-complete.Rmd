...
# Assemblage avec idu comme clé de jointure
indicateurs <- parcelles

# Joindre LiDAR
cols_lidar <- c("idu", "C1", "P1", "P3", "E1", "A1")
cols_present <- intersect(cols_lidar, names(metriques_lidar))
if ("idu" %in% cols_present) {
  indicateurs <- merge(indicateurs, st_drop_geometry(metriques_lidar)[, cols_present, drop = FALSE], by = "idu", all.x = TRUE)
}
...
cols_present <- intersect(cols_terrain, names(indic_terrain))
if ("idu" %in% cols_present) {
  indicateurs <- merge(indicateurs, st_drop_geometry(indic_terrain)[, cols_present, drop = FALSE], by = "idu", all.x = TRUE)
}
...
cols_present <- intersect(cols_eco, names(indic_eco))
if ("idu" %in% cols_present) {
  indicateurs <- merge(indicateurs, st_drop_geometry(indic_eco)[, cols_present, drop = FALSE], by = "idu", all.x = TRUE)
}
...
```{r calc-e2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=30}
...
```{r e2-by-type, exercise=TRUE, exercise.eval=FALSE, exercise.eval=FALSE}
...
```{r e2-export, exercise=TRUE, exercise.eval=FALSE, exercise.eval=FALSE}
