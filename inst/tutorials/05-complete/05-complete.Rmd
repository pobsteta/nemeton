...
- **T03** : Indicateurs terrain → W1, W2, W3, R1, R2, R3, R4, S1, S2, S3, F1 (11 indicateurs)
- **T04** : Indicateurs écologiques → B1, B2, B3, L1, L2, L3, T1, T2, F2, N1, N2, N3 (12 indicateurs)
...
indics_tuto03 <- c("W1", "W2", "W3", "R1", "R2", "R3", "R4", "S1", "S2", "S3", "F1")  # 11 indicateurs terrain
indics_tuto04 <- c("B1", "B2", "B3", "L1", "L2", "L3", "T1", "T2", "F2", "N1", "N2", "N3")  # 12 indicateurs écologie
```
...
indics_tuto03 <- c("W1", "W2", "W3", "R1", "R2", "R3", "R4", "S1", "S2", "S3", "F1")  # Terrain (11)
indics_tuto04 <- c("B1", "B2", "B3", "L1", "L2", "L3", "T1", "T2", "F2", "N1", "N2", "N3")  # Écologie (12)
...
```{r verif-indicateurs-check}
grade_this({
  pass("Vérification effectuée !")
})
```
...
```{r invert-indicators-check}
grade_this({
  pass("Inversion des indicateurs négatifs effectuée !")
})
```
...
```{r family-means-check}
grade_this({
  pass("Moyennes par famille calculées !")
})
```
...
grade_this({
  pass("Indice composite I_nemeton calculé avec succès !")
})
```
...
# 6. Exporter
output_path <- file.path(cache_dir, "parcelles.gpkg")
...
```{r export-final-check}
grade_this({
  pass("Export réussi ! Vos données sont prêtes pour l'analyse multicritères.")
})
```
