...
> **Note** : OSO doit être téléchargé depuis [Recherche Data Gouv](https://entrepot.recherche.data.gouv.fr/api/access/datafile/:persistentId?persistentId=doi:10.57745/8M1AN1) et placé dans `~/.local/share/nemeton/tutorial_data/oso.tif`
...
parcelles_path <- file.path(data_dir, "parcelles.gpkg")
oso_path <- file.path(data_dir, "oso.tif")
...
if (!file.exists(oso_path)) {
  ...
  oso_tar_files <- list.files(data_dir, pattern = "^OSO_.*\\.tar\\.gz$", full.names = TRUE)
  if (length(oso_tar_files) > 0) {
    oso_tar <- oso_tar_files[1]  # Utiliser la première archive trouvée
  } else {
    oso_tar <- file.path(data_dir, "OSO_RASTER.tar.gz")  # Nom par défaut pour le téléchargement
  }
  ...
  tryCatch({
    ...
    if (length(tif_files) > 0) {
      ...
      if (copy_ok && file.exists(oso_path)) {
        ...
        # Nettoyer: supprimer le répertoire extrait
        oso_dirs <- list.dirs(data_dir, full.names = TRUE, recursive = FALSE)
        oso_dirs <- oso_dirs[grepl("^OSO_", basename(oso_dirs))]
        if (length(oso_dirs) > 0) {
          unlink(oso_dirs, recursive = TRUE)
          cat("   Répertoire temporaire supprimé\n")
        }
        ...
        cat("   Erreur lors de la copie - archive conservée\n\n")
      }
    } else {
      cat("   Aucun fichier OCS_*.tif trouvé dans l'archive - archive conservée\n\n")
    }

  }, error = function(e) {
    cat("   Erreur:", e$message, "\n")
    if (file.exists(oso_tar)) {
      cat("   Archive partielle conservée - relancez pour reprendre le téléchargement\n")
    }
...
if (ndvi_available && exists("ndvi_raster") && !is.null(ndvi_raster)) {
  ...
  if (requireNamespace("exactextractr", quietly = TRUE)) {
    ...
    if (requireNamespace("furrr", quietly = TRUE) && requireNamespace("future", quietly = TRUE)) {
      ...
      cat("Mode parallèle (furrr)...\n")
      parcelles$C2 <- future_map_dbl(seq_len(nrow(parcelles)), extract_ndvi_i, .progress = TRUE)
      plan(sequential)
    } else {
      cat("Mode séquentiel...\n")
  ...
  if (file.exists(bd_foret_path)) {
    ...
    if (requireNamespace("furrr", quietly = TRUE) && requireNamespace("future", quietly = TRUE)) {
      ...
      parcelles$C2 <- future_map_dbl(seq_len(nrow(parcelles)), calc_ndvi_bdforet_i,
        parcelles_geom = parcelles_geom, bd_foret = bd_foret, ndvi_by_type = ndvi_by_type,
...
cat("=== Consolidation Famille C (Carbone) ===\n\n")
...
# Import des indicateurs C
c1 <- import_indicator("metriques_stock_carbone_c1.gpkg", "C1")
if (!is.null(c1)) parcelles$C1 <- c1

c2 <- import_indicator("metriques_vitalite_c2.gpkg", "C2")
if (!is.null(c2)) parcelles$C2 <- c2

# Statistiques
cat("\n=== Statistiques Famille C ===\n")
for (ind in c("C1", "C2")) {
...
# Sauvegarder
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille C consolidée (2 indicateurs)\n")
```
...
```{r ex-10-2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=50, exercise.setup="ex-10-2-setup"}
# === FAMILLE P (Production) : P1, P2, P3 ===

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== Consolidation Famille P (Production) ===\n\n")
...
# Import des indicateurs P
p1 <- import_indicator("metriques_volume_bois_p1.gpkg", "P1")
if (!is.null(p1)) parcelles$P1 <- p1

p2 <- import_indicator("metriques_productivite_p2.gpkg", "P2")
if (!is.null(p2)) parcelles$P2 <- p2

p3 <- import_indicator("metriques_qualite_structurale_p3.gpkg", "P3")
if (!is.null(p3)) parcelles$P3 <- p3

# Statistiques
cat("\n=== Statistiques Famille P ===\n")
for (ind in c("P1", "P2", "P3")) {
  if (ind %in% names(parcelles)) {
    vals <- parcelles[[ind]]
    cat(sprintf("%s: min=%.2f, moy=%.2f, max=%.2f\n",
      ind, min(vals, na.rm=TRUE), mean(vals, na.rm=TRUE), max(vals, na.rm=TRUE)))
...
# Sauvegarder
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille P consolidée (3 indicateurs)\n")
```
...
cat("=== Consolidation Famille E (Énergie) ===\n\n")
...
# Import des indicateurs E
e1 <- import_indicator("metriques_bois_energie_e1.gpkg", "E1")
if (!is.null(e1)) parcelles$E1 <- e1

e2 <- import_indicator("metriques_evitement_carbone_e2.gpkg", "E2")
if (!is.null(e2)) parcelles$E2 <- e2

# Statistiques
cat("\n=== Statistiques Famille E ===\n")
for (ind in c("E1", "E2")) {
  if (ind %in% names(parcelles)) {
    vals <- parcelles[[ind]]
    cat(sprintf("%s: min=%.2f, moy=%.2f, max=%.2f\n",
...
# Sauvegarder
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille E consolidée (2 indicateurs)\n")
```
...
cat("=== Consolidation Famille A (Air) ===\n\n")
...
# Import des indicateurs A
a1 <- import_indicator("metriques_couverture_forestiere_a1.gpkg", "A1")
if (!is.null(a1)) parcelles$A1 <- a1

a2 <- import_indicator("metriques_qualite_air_a2.gpkg", "A2")
if (!is.null(a2)) parcelles$A2 <- a2

# Statistiques
cat("\n=== Statistiques Famille A ===\n")
for (ind in c("A1", "A2")) {
  if (ind %in% names(parcelles)) {
    vals <- parcelles[[ind]]
    cat(sprintf("%s: min=%.2f, moy=%.2f, max=%.2f\n",
...
# Sauvegarder
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille A consolidée (2 indicateurs)\n")
```
...
```{r ex-10-5, exercise=TRUE, exercise.eval=FALSE, exercise.lines=50, exercise.setup="ex-10-5-setup"}
# === SYNTHÈSE FINALE ===

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== Synthèse des indicateurs LiDAR ===\n\n")
...
cat("\nTotal:", total, "/9 indicateurs consolidés\n")
