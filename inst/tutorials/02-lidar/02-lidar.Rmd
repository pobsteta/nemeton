...
# === SYNTHÈSE FINALE ===
# Consolidation de tous les indicateurs LiDAR dans parcelles.gpkg

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== CONSOLIDATION INDICATEURS LIDAR ===\n\n")
...
for (f in metriques_files) {
  tryCatch({
    ...
    # Colonnes à ajouter (exclure géométrie et colonnes déjà présentes)
    new_cols <- setdiff(names(st_drop_geometry(metrics)), names(st_drop_geometry(parcelles)))
    if (length(new_cols) > 0) {
      # Joindre par correspondance spatiale (même géométrie = même parcelle)
...
# Sauvegarder indicateurs_lidar.gpkg (fichier spécifique)
output_path <- file.path(data_dir, "indicateurs_lidar.gpkg")
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)

# Sauvegarder parcelles.gpkg avec tous les indicateurs consolidés
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Indicateurs LiDAR consolidés dans parcelles.gpkg\n")
```
