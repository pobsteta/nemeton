...
if (!file.exists(oso_path)) {
  ...
  # Créer un répertoire cache dédié pour OSO
  ...
  tryCatch({
    ...
    if (length(tif_files) > 0) {
      # Copier le fichier principal (OCS_*.tif) vers oso.tif dans data_dir
      copy_ok <- file.copy(tif_files[1], oso_path)
      if (copy_ok && file.exists(oso_path)) {
        cat("   Fichier copié vers oso.tif\n")

        # Nettoyer: supprimer tout le répertoire cache
        unlink(oso_cache, recursive = TRUE)
        cat("   Répertoire cache supprimé\n\n")
      } else {
        cat("   Erreur lors de la copie - cache conservé\n\n")
      }
    } else {
      cat("   Aucun fichier OCS_*.tif trouvé dans l'archive - cache conservé\n\n")
    }
