...
```{r ex-2-1, exercise=TRUE, exercise.lines=25, exercise.setup="ex-2-1-setup"}
...
grade_this({
  if (!exists(".result")) {
    fail("Le code n'a pas produit de résultat.")
  }
  pass("Excellent ! Vous avez exploré les fichiers LiDAR disponibles.")
})
```
...
```{r ex-2-2, exercise=TRUE, exercise.lines=30, exercise.setup="ex-2-2-setup"}
if (requireNamespace("lidR", quietly = TRUE)) {
  library(lidR)
...
```{r ex-3-1, exercise=TRUE, exercise.lines=45, exercise.setup="ex-3-1-setup", exercise.timelimit=300}
if (requireNamespace("lidR", quietly = TRUE)) {
  library(lidR)
...
```{r ex-3-1-check}
grade_this({
  pass("Normalisation effectuée ! Les hauteurs sont maintenant relatives au sol.")
})
```
...
```{r ex-4-1, exercise=TRUE, exercise.lines=50, exercise.setup="ex-4-1-setup", exercise.timelimit=300}
if (requireNamespace("lidR", quietly = TRUE)) {
  library(lidR)
...
```{r ex-4-1-check}
grade_this({
  pass("MNH généré ! Vous pouvez maintenant extraire des métriques par parcelle.")
})
```
...
```{r ex-5-1, exercise=TRUE, exercise.lines=60, exercise.setup="ex-5-1-setup", exercise.timelimit=300}
if (requireNamespace("lidR", quietly = TRUE)) {
  library(lidR)
...
```{r ex-5-1-check}
grade_this({
  pass("Métriques LiDAR calculées par parcelle !")
})
```
...
```{r ex-6-1, exercise=TRUE, exercise.lines=30, exercise.setup="ex-6-1-setup"}
# === EXPORT DES MÉTRIQUES LIDAR ===

# Simuler des métriques si lidR non disponible
parcelles_path <- file.path(data_dir, "parcelles.gpkg")

if (file.exists(parcelles_path)) {
  parcelles <- st_read(parcelles_path, quiet = TRUE)
  
  # Ajouter des métriques simulées (remplacer par vraies valeurs si lidR disponible)
  n <- nrow(parcelles)
  parcelles$zmax <- runif(n, 15, 35)
  parcelles$zmean <- parcelles$zmax * runif(n, 0.5, 0.7)
  parcelles$zsd <- runif(n, 3, 8)
  parcelles$zq95 <- parcelles$zmax * runif(n, 0.85, 0.95)
  parcelles$pzabove2 <- runif(n, 60, 95)
  
  # Sauvegarder
  output_path <- file.path(data_dir, "metriques_lidar.gpkg")
  st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
  
  cat("=== Métriques LiDAR exportées ===\n")
  cat("Fichier:", output_path, "\n")
  cat("Parcelles:", nrow(parcelles), "\n")
  cat("Colonnes:", paste(names(parcelles), collapse = ", "), "\n")
  
  # Résumé
  cat("\n=== Résumé des métriques ===\n")
  for (col in c("zmax", "zmean", "zsd", "zq95", "pzabove2")) {
    cat(sprintf("%s: %.1f - %.1f (moy: %.1f)\n", 
                col, min(parcelles[[col]]), max(parcelles[[col]]), mean(parcelles[[col]])))
  }
} else {
...
```{r ex-6-1-check}
grade_this({
  pass("Métriques LiDAR exportées en GeoPackage !")
})
```
...
Le fichier `metriques_lidar.gpkg` contient désormais vos parcelles avec les métriques :
