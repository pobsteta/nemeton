...
```{r ex-4-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=55, exercise.setup="ex-4-1-setup", exercise.timelimit=300}
if (requireNamespace("lidR", quietly = TRUE)) {
  library(lidR)
  ...
  if (length(fichiers_laz) > 0 && file.exists(mnt_path)) {
    # Charger le MNT pour définir la zone valide
    mnt <- rast(mnt_path)
    mnt_bbox <- ext(mnt)

    # Charger LiDAR (premier fichier)
    las_full <- readLAS(fichiers_laz[1], filter = "-drop_z_below 0")

    # === RÉDUCTION À UN CARRÉ DE 50x50m DANS LA ZONE DU MNT ===
    # Trouver l'intersection entre le LiDAR et le MNT
    las_bbox <- st_bbox(las_full)
    clip_xmin <- max(las_bbox["xmin"], mnt_bbox$xmin)
    clip_xmax <- min(las_bbox["xmax"], mnt_bbox$xmax)
    clip_ymin <- max(las_bbox["ymin"], mnt_bbox$ymin)
    clip_ymax <- min(las_bbox["ymax"], mnt_bbox$ymax)

    # Prendre un carré de 50x50m au centre de l'intersection
    center_x <- (clip_xmin + clip_xmax) / 2
    center_y <- (clip_ymin + clip_ymax) / 2

    las <- clip_rectangle(las_full,
      xleft = center_x - 25, ybottom = center_y - 25,
      xright = center_x + 25, ytop = center_y + 25
    )

    cat("Zone 50x50m:", nrow(las@data), "points\n")
    ...
    if (nrow(las@data) > 0) {
      ...
      cat("Aucun point dans l'intersection LiDAR/MNT.\n")
    }
  } else {
...
```{r ex-5-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=70, exercise.setup="ex-5-1-setup", exercise.timelimit=300}
if (requireNamespace("lidR", quietly = TRUE)) {
  library(lidR)
  ...
  if (length(fichiers_laz) > 0 && file.exists(mnt_path)) {
    ...
    if (nrow(las@data) > 0) {
      ...
      cat("\n=== Statistiques sur la zone 50x50m ===\n")
...
if (file.exists(parcelles_path)) {
  ...
  # Sauvegarder aussi dans parcelles.gpkg pour les exercices suivants (C1, P1, etc.)
...
if (file.exists(parcelles_path)) {
  ...
  if (all(c("pzabove2", "zmean") %in% names(parcelles))) {
    ...
    # Sauvegarder aussi dans parcelles.gpkg pour E2
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
# === SYNTHÈSE FINALE ===
# Consolidation des indicateurs LiDAR (T02 uniquement)

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== CONSOLIDATION INDICATEURS LIDAR (T02) ===\n\n")

# Liste des indicateurs LiDAR par famille (9 indicateurs T02)
familles <- list(
  "C (Carbone)" = c("C1", "C2"),
...
indicateurs_t02 <- unlist(familles)
...
if (length(manquants) > 0) {
  ...
  refs <- c(C1 = "7.1", C2 = "10.2", P1 = "8.1", P2 = "8.1", P3 = "8.1",
            E1 = "9.1", E2 = "9.1", A1 = "10.1", A2 = "10.2")
  for (m in manquants) {
    cat(sprintf("  - %s → Exercice %s\n", m, refs[m]))
  }
}

# Sauvegarder indicateurs_lidar.gpkg (uniquement indicateurs T02)
cols_base <- c("id", "code_parcelle", "surface_ha")  # colonnes de base si présentes
cols_base <- cols_base[cols_base %in% names(parcelles)]
cols_t02 <- indicateurs_t02[indicateurs_t02 %in% names(parcelles)]
parcelles_t02 <- parcelles[, c(cols_base, cols_t02)]

output_path <- file.path(data_dir, "indicateurs_lidar.gpkg")
st_write(parcelles_t02, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ indicateurs_lidar.gpkg:", length(cols_t02), "indicateurs T02\n")
