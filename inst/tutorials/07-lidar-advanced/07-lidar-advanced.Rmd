...
cache_segments <- file.path(output_dir_rasters, "segmentation.rds")
cache_trees_gpkg <- file.path(output_dir_rasters, "trees_detected.gpkg")
...
```{r ex-8-3, exercise=TRUE, exercise.timelimit=2800, exercise.setup="ex-8-3-setup"}
...
if (file.exists(cache_trees)) {
  ...
  # 2. Segmentation des arbres (avec cache)
  cat("2. Segmentation des arbres...\n")

  if (file.exists(cache_segments) && file.exists(cache_trees_gpkg)) {
    cat("   >>> Cache trouvé, chargement segmentation et arbres\n")
    segms <- readRDS(cache_segments)
    trees <- sf::st_read(cache_trees_gpkg, quiet = TRUE)
    cat("   Arbres chargés:", nrow(trees), "\n\n")
  } else {
    cat("   >>> Calcul de la segmentation (tree_segmentation)...\n")
    segms <- lidaRtRee::tree_segmentation(chm)

    cat("   >>> Extraction des arbres (tree_extraction)...\n")
    trees <- lidaRtRee::tree_extraction(segms)

    # Sauvegarder en cache
    saveRDS(segms, cache_segments)
    sf::st_write(trees, cache_trees_gpkg, delete_dsn = TRUE, quiet = TRUE)
    cat("   Arbres détectés:", nrow(trees), "\n")
    cat("   Cache sauvegardé:", basename(cache_segments), ",", basename(cache_trees_gpkg), "\n\n")
  }
