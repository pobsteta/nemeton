...
```{r ex-2-2, exercise=TRUE, exercise.timelimit=1200, exercise.setup="ex-2-2-setup"}
# =============================================================================
...
```{r ex-3-1, exercise=TRUE, exercise.timelimit=600, exercise.setup="ex-3-1-setup"}
# =============================================================================
# GESTION MÉMOIRE - Important pour les gros jeux de données
# =============================================================================
gc()  # Forcer le garbage collection avant traitement
...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  if (length(chm_tiles) > 0) {
    chm_vrt <- vrt(data_dir, file.path(data_dir, "chm_complet.vrt"), overwrite = TRUE)
  ...
  # Visualisation du CHM avec segments
  ...
  if (exists("chm_vrt") && exists("crowns_vrt")) {
    par(mfrow = c(1, 2))
    plot(chm_vrt, main = "CHM (0.5m)", col = viridis::viridis(50))
    plot(crowns_vrt %% 8, main = "Segments houppiers", col = rainbow(8), legend = FALSE)
  }
...
```{r ex-3-3, exercise=TRUE, exercise.timelimit=180, exercise.setup="ex-3-3-setup"}
