...
plots <- dir(field_dir, pattern = "_PiquetsTerrain\\.csv$", full.names = TRUE) |>
  ...
  map_dfr(~ {
    ...
    df <- df[, c("Id", "X", "Y")]
    df$cluster_id <- sub("_PiquetsTerrain\\.csv$", "", basename(.x))
    df
  }) |>
  dplyr::filter(Id %in% c("p1", "p2", "p3", "p4")) |>
  dplyr::mutate(plot_id = paste0(cluster_id, "-", substr(Id, 2, 2)))

cat("Clusters chargés:", dplyr::n_distinct(plots$cluster_id), "\n")
...
trees <- dir(field_dir, pattern = "_ArbresTerrain\\.csv$", full.names = TRUE) |>
  ...
  dplyr::filter(!is.na(Diam), Diam >= dbh_min, Aspect %in% c(1, 2))
...
plots_sf <- plots |>
  dplyr::left_join(stand_params, by = "plot_id") |>
...
stand_params <- dir(field_dir, pattern = "_ArbresTerrain\\.csv$", full.names = TRUE) |>
  ...
  dplyr::filter(!is.na(Diam), Diam >= 7.5, Aspect %in% c(1, 2)) |>
  dplyr::group_by(plot_id) |>
  dplyr::summarise(G_m2_ha = sum(pi * (Diam / 200)^2) / plot_area_ha, .groups = "drop")
