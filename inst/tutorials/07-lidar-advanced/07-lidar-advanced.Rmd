...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  if (!file.exists(chm_filled_vrt)) {
    ...
    # Appliquer focal mean avec fenêtre 30×30 pixels (= 30m sur CHM 1m)
    t0 <- Sys.time()
    focal_window <- matrix(1, nrow = 31, ncol = 31)  # 31x31 pour centrer sur pixel
    focal_result <- terra::focal(chm_crop, w = focal_window, fun = "mean", na.rm = TRUE)
    names(focal_result) <- "zmean_focal"

    # Rééchantillonner à 10m pour comparaison
    focal_10m <- terra::aggregate(focal_result, fact = 10, fun = "mean", na.rm = TRUE)
    t_focal <- round(difftime(Sys.time(), t0, units = "secs"), 1)
    cat("Focal mean calculé en", t_focal, "sec\n\n")
