...
- **Sauvegarde incrémentale** : chaque tuile est sauvegardée au fur et à mesure
- **Reprise possible** : les tuiles existantes sont conservées en cas d'interruption
...
# Répertoire pour les tuiles de métriques arbres
tiles_dir_tree_metrics <- file.path(result_itd, "tiles_tree_metrics")
if (!dir.exists(tiles_dir_tree_metrics)) dir.create(tiles_dir_tree_metrics, recursive = TRUE)
...
cat("Tuiles source:\n")
cat("  - CHM filled:", length(chm_tiles), "\n")
cat("  - Crowns:", length(crown_tiles), "\n")
cat("  - Seeds:", length(seed_files), "\n\n")
...
if (length(crown_tiles) == 0) {
  ...
  # Vérifier les tuiles de métriques déjà calculées
  existing_metrics <- list.files(tiles_dir_tree_metrics, pattern = "\\.gpkg$", full.names = TRUE)
  cat("Tuiles métriques existantes:", length(existing_metrics), "\n\n")

  # ==========================================================================
  # FONCTION DE TRAITEMENT D'UNE TUILE (avec sauvegarde)
  # ==========================================================================

  process_tile <- function(i, chm_tiles, crown_tiles, seed_files, tiles_dir_tree_metrics) {
    # Nom du fichier de sortie basé sur le nom de la tuile crown
    crown_basename <- tools::file_path_sans_ext(basename(crown_tiles[i]))
    output_file <- file.path(tiles_dir_tree_metrics,
                              paste0("metrics_", crown_basename, ".gpkg"))

    # Si le fichier existe déjà, le charger et retourner
    ...
    # Filtrer les seeds sans correspondance valide (id NA ou 0)
    metrics <- metrics[!is.na(metrics$id) & metrics$id > 0, ]
    ...
    metrics <- merge(metrics, crown_data, by = "id", all.x = FALSE)

    # 5. Ajouter géométrie WKT des houppiers
    ...
    # Convertir en sf et sauvegarder
    metrics_sf <- sf::st_as_sf(metrics, coords = c("x", "y"), crs = terra::crs(chm))
    sf::st_write(metrics_sf, output_file, delete_dsn = TRUE, quiet = TRUE)
  ...
  if (requireNamespace("future.apply", quietly = TRUE) && n_tiles > 1) {
    ...
    cat("Méthode: future.apply (", N_CORES, " workers)\n", sep = "")
    cat("Tuiles à traiter:", n_tiles, "\n")
    cat("Sortie:", basename(tiles_dir_tree_metrics), "/\n\n")

    t0 <- Sys.time()

    results <- future.apply::future_lapply(
      seq_len(n_tiles),
      function(i) process_tile(i, chm_tiles, crown_tiles, seed_files, tiles_dir_tree_metrics),
      future.seed = TRUE
    )
    ...
    results <- lapply(
      ...
      function(i) {
        ...
        process_tile(i, chm_tiles, crown_tiles, seed_files, tiles_dir_tree_metrics)
      }
    )
  }
  ...
  # ==========================================================================
  # FUSION DES RÉSULTATS (depuis les fichiers GPKG sauvegardés)
  # ==========================================================================
  cat("=== Fusion des résultats ===\n")

  # Lister tous les fichiers GPKG générés
  metric_files <- list.files(tiles_dir_tree_metrics, pattern = "\\.gpkg$", full.names = TRUE)
  cat("Tuiles générées:", length(metric_files), "\n")

  if (length(metric_files) > 0) {
    # Charger et fusionner tous les fichiers
    cat("Chargement et fusion des tuiles...\n")
    trees_list <- lapply(metric_files, function(f) sf::st_read(f, quiet = TRUE))
    trees_all <- do.call(rbind, trees_list)

    cat("Tuiles fusionnées:", length(metric_files), "/", n_tiles, "\n\n")
    ...
    cat("Fichier complet:", basename(output_file), "\n\n")
    ...
    cat("Colonnes:", paste(names(trees_all), collapse = ", "), "\n")
    ...
    cat("\n=== Avantages de la sauvegarde incrémentale ===\n")
    cat("- Chaque tuile sauvegardée immédiatement\n")
    cat("- Reprise possible en cas d'interruption\n")
    cat("- Parallélisation efficace (", N_CORES, " workers)\n", sep = "")
  } else {
    cat("Aucune tuile générée. Vérifiez les données d'entrée.\n")
  }
}
```
...
Les métriques basées sur la détection d'arbres **réutilisent les résultats de l'exercice 3.1** (segmentation lasR) :

1. **Chargement des arbres** depuis `tree_metrics_complet.gpkg` (exercice 3.3)
2. **Découpage par emprise de tuile** (mêmes emprises que les fichiers LAZ)
3. **Traitement parallèle** tuile par tuile avec `future.apply`
4. **Assemblage en VRT** comme dans l'exercice 8.2

> **Note** : Cette approche évite de recalculer la segmentation et réutilise le travail
> effectué dans la section 3 avec lasR, qui est plus performant pour le traitement
> parallèle des grandes zones.
...
# Répertoire pour les tuiles de métriques arbres
tiles_dir_trees <- file.path(output_dir_rasters, "tiles_metrics_trees")
if (!dir.exists(tiles_dir_trees)) dir.create(tiles_dir_trees, recursive = TRUE)

# Répertoire pour les tuiles de métriques canopée
tiles_dir_canopy <- file.path(output_dir_rasters, "tiles_metrics_canopy")
if (!dir.exists(tiles_dir_canopy)) dir.create(tiles_dir_canopy, recursive = TRUE)

# Fichier VRT pour les métriques canopée
vrt_canopy <- file.path(output_dir_rasters, "metrics_canopy.vrt")
...
# Fichiers produits par les exercices 3.1/3.2/3.3
trees_itd_gpkg <- file.path(data_dir, "tree_metrics_complet.gpkg")
chm_filled_vrt <- file.path(data_dir, "c_filled_complet.vrt")
crowns_vrt <- file.path(data_dir, "crowns_complet.vrt")

# Fichier VRT pour les métriques arbres
vrt_trees <- file.path(output_dir_rasters, "metrics_trees.vrt")

# Configuration parallélisation
N_CORES <- 4L
BUFFER <- 30  # Buffer en mètres pour éviter les effets de bord (comme dans 8.2)
```

```{r ex-8-3, exercise=TRUE, exercise.timelimit=1200, exercise.setup="ex-8-3-setup"}
# === 8.3 : Métriques raster basées sur les arbres (parallélisé par tuiles) ===

cat("=== Métriques arbres - Réutilisation des résultats ITD (Section 3) ===\n\n")
cat("Résolution:", resolution, "m\n")
cat("Surface pixel:", resolution^2, "m²\n\n")

# Vérifier que les fichiers de l'exercice 3 existent
if (!file.exists(trees_itd_gpkg)) {
  stop("Fichier tree_metrics_complet.gpkg non trouvé.\n",
       "Exécutez d'abord les exercices 3.1, 3.2 et 3.3.")
}

# Compter les fichiers LAZ source (pour définir les emprises) et tuiles TIF existantes
laz_files <- list.files(laz_dir_normalized, pattern = "\\.laz$", full.names = TRUE)
n_laz <- length(laz_files)
existing_tiles <- list.files(tiles_dir_trees, pattern = "\\.tif$", full.names = TRUE)
n_existing <- length(existing_tiles)

cat("Tuiles de référence (LAZ):", n_laz, "\n")
cat("Tuiles existantes:", n_existing, "TIF\n\n")

# Vérifier si le traitement est complet
if (file.exists(vrt_trees) && n_existing >= n_laz) {
  cat(">>> Traitement complet, chargement du VRT existant\n\n")
  metrics_trees_raster <- terra::rast(vrt_trees)
  # Restaurer les noms de couches depuis une tuile TIF
  tile_names <- names(terra::rast(existing_tiles[1]))
  names(metrics_trees_raster) <- tile_names
} else {
  # Nettoyer les tuiles existantes pour éviter les conflits
  if (n_existing > 0) {
    cat(">>> Nettoyage des", n_existing, "tuiles existantes (traitement incomplet)\n")
    file.remove(existing_tiles)
    if (file.exists(vrt_trees)) file.remove(vrt_trees)
    cat(">>> Recalcul complet de toutes les tuiles\n\n")
  } else {
    cat(">>> Nouveau calcul de toutes les tuiles\n\n")
  }

  # 1. Charger les arbres détectés (exercice 3.3)
  cat("1. Chargement des arbres depuis tree_metrics_complet.gpkg...\n")
  trees_all <- sf::st_read(trees_itd_gpkg, quiet = TRUE)
  
  cat("   Arbres chargés:", nrow(trees_all), "\n")

  # Adapter les noms de colonnes pour std_tree_metrics()
  # std_tree_metrics attend : h (hauteur), s (surface), v (volume)
  if (!"h" %in% names(trees_all) && "height" %in% names(trees_all)) {
    trees_all$h <- trees_all$height
  }
  if (!"s" %in% names(trees_all) && "surface" %in% names(trees_all)) {
    trees_all$s <- trees_all$surface
  }
  if (!"v" %in% names(trees_all) && "volume" %in% names(trees_all)) {
    trees_all$v <- trees_all$volume
  }

  # Filtrer les arbres avec des NA (std_tree_metrics n'accepte pas les NA)
  n_before <- nrow(trees_all)
  trees_all <- trees_all[!is.na(trees_all$h) & !is.na(trees_all$s) & !is.na(trees_all$v), ]
  n_removed <- n_before - nrow(trees_all)

  cat("   Colonnes adaptées: h, s, v\n")
  if (n_removed > 0) {
    cat("   Arbres avec NA retirés:", n_removed, "\n")
  }
  cat("   Arbres valides:", nrow(trees_all), "\n")
  cat("   Hauteur moyenne:", round(mean(trees_all$h, na.rm = TRUE), 1), "m\n\n")

  # 2. Obtenir les emprises des tuiles LAZ (vecteurs simples pour sérialisation future)
  cat("2. Extraction des emprises des tuiles LAZ...\n")
  tile_extents <- lapply(laz_files, function(f) {
    hdr <- lidR::readLASheader(f)
    # Vecteur nommé (sérialisable, contrairement à terra::ext)
    c(xmin = hdr@PHB$`Min X`, xmax = hdr@PHB$`Max X`,
      ymin = hdr@PHB$`Min Y`, ymax = hdr@PHB$`Max Y`)
  })
  cat("   Emprises extraites:", length(tile_extents), "\n\n")

  # Récupérer le CRS comme EPSG (sérialisable)
  crs_epsg <- sf::st_crs(trees_all)$epsg
  if (is.na(crs_epsg)) crs_epsg <- 2154  # Lambert 93 par défaut

  # 3. Fonction de traitement d'une tuile (avec buffer)
  process_tree_tile <- function(i, trees_all, tile_extents, laz_files,
                                 tiles_dir_trees, resolution, buffer, crs_epsg) {
    ext_i <- tile_extents[[i]]

    # Emprise avec buffer pour inclure les arbres de bordure
    bbox_buffered <- sf::st_as_sfc(sf::st_bbox(c(
      xmin = ext_i[["xmin"]] - buffer, xmax = ext_i[["xmax"]] + buffer,
      ymin = ext_i[["ymin"]] - buffer, ymax = ext_i[["ymax"]] + buffer
    ), crs = sf::st_crs(crs_epsg)))

    # Filtrer les arbres dans l'emprise avec buffer
    trees_tile <- sf::st_filter(trees_all, bbox_buffered)

    if (nrow(trees_tile) == 0) {
      return(NULL)
    }

    # Filtrer les arbres avec NA dans h, s, v (std_tree_metrics n'accepte pas les NA)
    trees_tile <- trees_tile[!is.na(trees_tile$h) & !is.na(trees_tile$s) & !is.na(trees_tile$v), ]

    if (nrow(trees_tile) == 0) {
      return(NULL)
    }

    # Calculer les métriques sur l'emprise avec buffer
    metrics_tile <- lidaRtRee::raster_metrics(
      trees_tile,
      res = resolution,
      fun = function(x) {
    ...
    metrics_tile[!is.finite(metrics_tile)] <- 0

    # Cropper à l'emprise sans buffer (évite les doublons entre tuiles)
    ext_crop <- terra::ext(ext_i["xmin"], ext_i["xmax"], ext_i["ymin"], ext_i["ymax"])
    metrics_tile <- terra::crop(metrics_tile, ext_crop)

    # Nom de fichier basé sur le nom de la dalle LAZ source
    laz_basename <- tools::file_path_sans_ext(basename(laz_files[i]))
    output_file <- file.path(tiles_dir_trees, paste0("trees_", laz_basename, ".tif"))

    # Sauvegarder la tuile
    terra::writeRaster(metrics_tile, output_file, overwrite = TRUE)

    return(output_file)
  }

  # 4. Traitement parallèle des tuiles
  cat("3. Traitement parallèle des tuiles...\n")
  ...
  results <- future_lapply(
    seq_len(n_laz),
    function(i) {
      process_tree_tile(i, trees_all, tile_extents, laz_files,
                        tiles_dir_trees, resolution, BUFFER, crs_epsg)
    },
    future.seed = TRUE
  )
  ...
  elapsed <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
  cat("Temps de calcul:", round(elapsed, 1), "s\n\n")

  # 5. Assembler les tuiles en VRT
  cat("4. Assemblage des tuiles en VRT...\n")
  metric_tiles <- list.files(tiles_dir_trees, pattern = "\\.tif$", full.names = TRUE)
  cat("   Tuiles générées:", length(metric_tiles), "\n")

  if (length(metric_tiles) == 0) {
    stop("Aucune tuile générée. Vérifiez les données d'entrée.")
  }

  # Récupérer les noms de couches depuis une tuile
  tile_names <- names(terra::rast(metric_tiles[1]))

  # Créer le VRT
  metrics_trees_raster <- terra::vrt(metric_tiles, vrt_trees, overwrite = TRUE)
  names(metrics_trees_raster) <- tile_names
  cat("   VRT créé:", basename(vrt_trees), "\n\n")

  cat(">>> Métriques arbres calculées et assemblées en VRT\n")
}

# 6. Métriques supplémentaires (couverture canopée) - traitement par tuiles
cat("5. Calcul couverture canopée (parallélisé par tuiles)...\n")

# S'assurer que tile_extents existe (nécessaire si cache arbres était complet)
if (!exists("tile_extents")) {
  tile_extents <- lapply(laz_files, function(f) {
    hdr <- lidR::readLASheader(f)
    c(xmin = hdr@PHB$`Min X`, xmax = hdr@PHB$`Max X`,
      ymin = hdr@PHB$`Min Y`, ymax = hdr@PHB$`Max Y`)
...
if (file.exists(chm_filled_vrt) && file.exists(crowns_vrt)) {
  # Identifier les tuiles canopée existantes et manquantes
  expected_canopy_tiles <- sapply(laz_files, function(f) {
    laz_basename <- tools::file_path_sans_ext(basename(f))
    file.path(tiles_dir_canopy, paste0("canopy_", laz_basename, ".tif"))
  })
  existing_canopy_tiles <- list.files(tiles_dir_canopy, pattern = "\\.tif$", full.names = TRUE)
  n_existing_canopy <- length(existing_canopy_tiles)
  missing_canopy_mask <- !file.exists(expected_canopy_tiles)
  n_missing_canopy <- sum(missing_canopy_mask)

  cat("   Tuiles canopée existantes:", n_existing_canopy, "\n")
  cat("   Tuiles canopée manquantes:", n_missing_canopy, "\n")

  # Vérifier si le traitement canopée est complet
  if (file.exists(vrt_canopy) && n_missing_canopy == 0) {
    cat("   >>> Cache complet, chargement du VRT canopée existant\n")
    metrics_canopy <- terra::rast(vrt_canopy)
    canopy_names <- names(terra::rast(existing_canopy_tiles[1]))
    names(metrics_canopy) <- canopy_names
  } else {
    # Supprimer le VRT obsolète si présent
    if (file.exists(vrt_canopy)) file.remove(vrt_canopy)

    # Charger les VRT sources
    chm_filled <- terra::rast(chm_filled_vrt)
    crowns <- terra::rast(crowns_vrt)
    chm_res <- terra::res(chm_filled)[1]
    cat("   CHM filled chargé, résolution:", chm_res, "m\n")

    # Récupérer le CRS
    crs_epsg <- terra::crs(chm_filled, describe = TRUE)$code
    if (is.na(crs_epsg)) crs_epsg <- 2154

    # Fonction de traitement d'une tuile canopée
    process_canopy_tile <- function(i, tile_extents, laz_files, tiles_dir_canopy,
                                     chm_filled_vrt, crowns_vrt, resolution, chm_res, crs_epsg) {
      ext_i <- tile_extents[[i]]

      # Emprise de la tuile
      ext_terra <- terra::ext(ext_i["xmin"], ext_i["xmax"], ext_i["ymin"], ext_i["ymax"])

      # Charger les rasters (dans le worker)
      chm_tile <- terra::crop(terra::rast(chm_filled_vrt), ext_terra)
      crowns_tile <- terra::crop(terra::rast(crowns_vrt), ext_terra)

      # Créer le raster canopée (pixels avec arbre uniquement)
      r_treechm <- chm_tile
      r_treechm[crowns_tile == 0 | is.na(crowns_tile)] <- NA
      ...
      metrics_tile <- lidaRtRee::raster_metrics(
        ...
        fun = function(x) {
          vals <- x[[1]]
          c(
            sum(!is.na(vals)) / (resolution / chm_res)^2,
            mean(vals, na.rm = TRUE)
          )
        },
        output = "raster"
      )
      names(metrics_tile) <- c("TreeCanopy_cover_in_plot", "TreeCanopy_meanH_in_plot")

      # Nom de fichier basé sur le nom de la dalle LAZ source
      laz_basename <- tools::file_path_sans_ext(basename(laz_files[i]))
      output_file <- file.path(tiles_dir_canopy, paste0("canopy_", laz_basename, ".tif"))
    ...
    # Traitement parallèle des tuiles MANQUANTES
    indices_canopy <- which(missing_canopy_mask)

    if (length(indices_canopy) > 0) {
      cat("   Traitement parallèle de", length(indices_canopy), "tuiles...\n")

      library(future)
      library(future.apply)
      plan(multisession, workers = N_CORES)
      start_time_canopy <- Sys.time()

      results_canopy <- future_lapply(
        indices_canopy,
        function(i) {
          process_canopy_tile(i, tile_extents, laz_files, tiles_dir_canopy,
                               chm_filled_vrt, crowns_vrt, resolution, chm_res, crs_epsg)
        },
        future.seed = TRUE
      )
      ...
      elapsed_canopy <- as.numeric(difftime(Sys.time(), start_time_canopy, units = "secs"))
      cat("   Temps de calcul canopée:", round(elapsed_canopy, 1), "s\n")
    }

    # Assembler les tuiles canopée en VRT
    canopy_tiles <- list.files(tiles_dir_canopy, pattern = "\\.tif$", full.names = TRUE)
    cat("   Tuiles canopée totales:", length(canopy_tiles), "\n")

    canopy_names <- names(terra::rast(canopy_tiles[1]))
    metrics_canopy <- terra::vrt(canopy_tiles, vrt_canopy, overwrite = TRUE)
    names(metrics_canopy) <- canopy_names
    cat("   VRT canopée créé:", basename(vrt_canopy), "\n")
  }

  # Harmoniser les emprises et combiner avec metrics_trees_raster
  ...
  cat("   Couverture canopée ajoutée\n\n")
} else {
  cat("   VRT non trouvés, couverture canopée ignorée\n")
  cat("   (Exécutez l'exercice 3.2 pour les créer)\n\n")
}

cat(">>> Métriques arbres et canopée calculées\n")
...
Les métriques de terrain (altitude, pente, aspect) sont calculées à partir du **DTM créé dans l'exercice 3.1** (lasR). Cette approche est plus efficace que de recalculer depuis les points LAZ :

1. **Étape 1** : Calcul de la pente et de l'aspect à 1m depuis le DTM (`terra::terrain`)
2. **Étape 2** : Agrégation à 25m pour les métriques finales (`terra::aggregate`)

Les tuiles intermédiaires (pente/aspect à 1m) sont sauvegardées pour réutilisation future.
...
# Répertoire pour les tuiles de métriques terrain à 25m
...
# Répertoires pour les tuiles pente et aspect à 1m (dérivés du DTM)
tiles_dir_slope <- file.path(data_dir, "tiles_slope_1m")
tiles_dir_aspect <- file.path(data_dir, "tiles_aspect_1m")
if (!dir.exists(tiles_dir_slope)) dir.create(tiles_dir_slope, recursive = TRUE)
if (!dir.exists(tiles_dir_aspect)) dir.create(tiles_dir_aspect, recursive = TRUE)
...
# DTM créé dans l'exercice 3.1 (lasR)
dtm_vrt <- file.path(data_dir, "dtm_complet.vrt")

# VRT pour altitude, pente et aspect à 1m
vrt_altitude_1m <- file.path(data_dir, "altitude_1m.vrt")
vrt_slope_1m <- file.path(data_dir, "slope_1m.vrt")
vrt_aspect_1m <- file.path(data_dir, "aspect_1m.vrt")

# Fichier VRT pour les métriques terrain à 25m
vrt_terrain <- file.path(output_dir_rasters, "metrics_terrain.vrt")

# Répertoire des tuiles LAZ (pour les emprises)
laz_dir_normalized <- file.path(data_dir, "result_laz_normalized")
...
# === 8.4 : Métriques terrain depuis DTM (Section 3) ===

cat("=== Calcul des métriques terrain depuis le DTM existant ===\n\n")

# Vérifier que le DTM existe
if (!file.exists(dtm_vrt)) {
  stop("DTM non trouvé: ", dtm_vrt, "\n",
       "Exécutez d'abord l'exercice 3.1 pour créer le DTM.")
}

# Lister les fichiers LAZ pour obtenir les emprises
laz_files <- list.files(laz_dir_normalized, pattern = "\\.laz$", full.names = TRUE)
n_laz <- length(laz_files)

# Compter les tuiles existantes
existing_terrain_tiles <- list.files(tiles_dir_terrain, pattern = "\\.tif$", full.names = TRUE)
existing_slope_tiles <- list.files(tiles_dir_slope, pattern = "\\.tif$", full.names = TRUE)
existing_aspect_tiles <- list.files(tiles_dir_aspect, pattern = "\\.tif$", full.names = TRUE)

cat("DTM source:", basename(dtm_vrt), "\n")
cat("Tuiles de référence (LAZ):", n_laz, "\n")
cat("Tuiles existantes - Pente 1m:", length(existing_slope_tiles), "\n")
cat("Tuiles existantes - Aspect 1m:", length(existing_aspect_tiles), "\n")
cat("Tuiles existantes - Terrain 25m:", length(existing_terrain_tiles), "\n\n")

# Vérifier si le traitement terrain 25m est complet
if (file.exists(vrt_terrain) && length(existing_terrain_tiles) >= n_laz) {
  cat(">>> Métriques terrain 25m complètes, chargement du VRT existant\n\n")
  metrics_terrain_raster <- terra::rast(vrt_terrain)
  tile_names <- names(terra::rast(existing_terrain_tiles[1]))
  names(metrics_terrain_raster) <- tile_names
} else {
  # ============================================================
  # ÉTAPE 1 : Calcul pente et aspect à 1m par tuiles (parallèle)
  # ============================================================

  cat("--- Étape 1 : Calcul pente/aspect à 1m par tuiles ---\n\n")

  # Obtenir les emprises des tuiles LAZ
  tile_extents <- lapply(laz_files, function(f) {
    hdr <- lidR::readLASheader(f)
    c(xmin = hdr@PHB$`Min X`, xmax = hdr@PHB$`Max X`,
      ymin = hdr@PHB$`Min Y`, ymax = hdr@PHB$`Max Y`)
  ...
  # Identifier les tuiles pente/aspect manquantes
  expected_slope <- sapply(laz_files, function(f) {
    file.path(tiles_dir_slope, paste0("slope_", tools::file_path_sans_ext(basename(f)), ".tif"))
  })
  expected_aspect <- sapply(laz_files, function(f) {
    file.path(tiles_dir_aspect, paste0("aspect_", tools::file_path_sans_ext(basename(f)), ".tif"))
  })

  missing_slope <- !file.exists(expected_slope)
  missing_aspect <- !file.exists(expected_aspect)
  missing_deriv <- missing_slope | missing_aspect
  indices_deriv <- which(missing_deriv)

  cat("Tuiles pente/aspect à calculer:", length(indices_deriv), "/", n_laz, "\n")

  if (length(indices_deriv) > 0) {
    # Fonction pour calculer pente et aspect d'une tuile
    process_terrain_derivatives <- function(i, tile_extents, laz_files,
                                             tiles_dir_slope, tiles_dir_aspect, dtm_vrt) {
      ext_i <- tile_extents[[i]]
      laz_basename <- tools::file_path_sans_ext(basename(laz_files[i]))

      # Fichiers de sortie
      slope_file <- file.path(tiles_dir_slope, paste0("slope_", laz_basename, ".tif"))
      aspect_file <- file.path(tiles_dir_aspect, paste0("aspect_", laz_basename, ".tif"))

      # Emprise avec buffer pour éviter les artefacts de bord
      buffer <- 10  # 10m de buffer pour terra::terrain
      ext_buffered <- terra::ext(
        ext_i["xmin"] - buffer, ext_i["xmax"] + buffer,
        ext_i["ymin"] - buffer, ext_i["ymax"] + buffer
      )
      ext_crop <- terra::ext(ext_i["xmin"], ext_i["xmax"], ext_i["ymin"], ext_i["ymax"])

      # Charger le DTM pour cette tuile
      dtm_tile <- terra::crop(terra::rast(dtm_vrt), ext_buffered)

      # Calculer pente et aspect
      slope_tile <- terra::terrain(dtm_tile, v = "slope", unit = "degrees")
      aspect_tile <- terra::terrain(dtm_tile, v = "aspect", unit = "degrees")

      # Cropper à l'emprise sans buffer
      slope_tile <- terra::crop(slope_tile, ext_crop)
      aspect_tile <- terra::crop(aspect_tile, ext_crop)

      # Sauvegarder
      terra::writeRaster(slope_tile, slope_file, overwrite = TRUE)
      terra::writeRaster(aspect_tile, aspect_file, overwrite = TRUE)

      return(c(slope_file, aspect_file))
    }
    ...
    cat("Traitement parallèle avec", N_CORES, "workers...\n")
    start_time <- Sys.time()

    results_deriv <- future_lapply(
      indices_deriv,
      function(i) {
        process_terrain_derivatives(i, tile_extents, laz_files,
                                     tiles_dir_slope, tiles_dir_aspect, dtm_vrt)
      },
      future.seed = TRUE
    )
    ...
    elapsed <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
    cat("Temps de calcul pente/aspect:", round(elapsed, 1), "s\n\n")
  } else {
    cat(">>> Tuiles pente/aspect déjà complètes\n\n")
  }

  # Créer/mettre à jour les VRT altitude, pente et aspect à 1m
  slope_tiles <- list.files(tiles_dir_slope, pattern = "\\.tif$", full.names = TRUE)
  aspect_tiles <- list.files(tiles_dir_aspect, pattern = "\\.tif$", full.names = TRUE)

  # Copier le DTM vers altitude_1m.vrt pour cohérence de nomenclature
  if (!file.exists(vrt_altitude_1m) || file.mtime(dtm_vrt) > file.mtime(vrt_altitude_1m)) {
    file.copy(dtm_vrt, vrt_altitude_1m, overwrite = TRUE)
    cat("VRT altitude 1m créé:", basename(vrt_altitude_1m), "\n")
  }

  if (length(slope_tiles) > 0) {
    slope_1m <- terra::vrt(slope_tiles, vrt_slope_1m, overwrite = TRUE)
    cat("VRT pente 1m créé:", basename(vrt_slope_1m), "\n")
  }
  if (length(aspect_tiles) > 0) {
    aspect_1m <- terra::vrt(aspect_tiles, vrt_aspect_1m, overwrite = TRUE)
    cat("VRT aspect 1m créé:", basename(vrt_aspect_1m), "\n")
  }

  # ============================================================
  # ÉTAPE 2 : Agrégation à 25m par tuiles (parallèle)
  # ============================================================

  cat("\n--- Étape 2 : Agrégation à 25m par tuiles ---\n\n")

  # Identifier les tuiles terrain 25m manquantes
  expected_terrain <- sapply(laz_files, function(f) {
    file.path(tiles_dir_terrain, paste0("terrain_", tools::file_path_sans_ext(basename(f)), ".tif"))
  })
  missing_terrain <- !file.exists(expected_terrain)
  indices_terrain <- which(missing_terrain)

  cat("Tuiles terrain 25m à calculer:", length(indices_terrain), "/", n_laz, "\n")

  # Supprimer le VRT obsolète
  if (file.exists(vrt_terrain)) file.remove(vrt_terrain)

  if (length(indices_terrain) > 0) {
    # Fonction pour agréger une tuile à 25m
    process_terrain_aggregate <- function(i, tile_extents, laz_files, tiles_dir_terrain,
                                           vrt_altitude_1m, vrt_slope_1m, vrt_aspect_1m, resolution) {
      ext_i <- tile_extents[[i]]
      laz_basename <- tools::file_path_sans_ext(basename(laz_files[i]))

      # Fichier de sortie
      terrain_file <- file.path(tiles_dir_terrain, paste0("terrain_", laz_basename, ".tif"))

      # Emprise de la tuile
      ext_terra <- terra::ext(ext_i["xmin"], ext_i["xmax"], ext_i["ymin"], ext_i["ymax"])

      # Charger les rasters 1m pour cette tuile
      altitude_tile <- terra::crop(terra::rast(vrt_altitude_1m), ext_terra)
      slope_tile <- terra::crop(terra::rast(vrt_slope_1m), ext_terra)
      aspect_tile <- terra::crop(terra::rast(vrt_aspect_1m), ext_terra)

      # Facteur d'agrégation
      fact <- as.integer(resolution / terra::res(altitude_tile)[1])

      # Agréger à 25m
      altitude_mean <- terra::aggregate(altitude_tile, fact = fact, fun = "mean", na.rm = TRUE)
      slope_deg <- terra::aggregate(slope_tile, fact = fact, fun = "mean", na.rm = TRUE)
      # Pour l'aspect, utiliser la moyenne (simplifié - idéalement moyenne circulaire)
      aspect_deg <- terra::aggregate(aspect_tile, fact = fact, fun = "mean", na.rm = TRUE)

      # Combiner en stack
      terrain_stack <- c(altitude_mean, slope_deg, aspect_deg)
      names(terrain_stack) <- c("altitude_mean", "slope_deg", "aspect_deg")

      # Sauvegarder
      terra::writeRaster(terrain_stack, terrain_file, overwrite = TRUE)
    ...
    results_terrain <- future_lapply(
      indices_terrain,
      function(i) {
        process_terrain_aggregate(i, tile_extents, laz_files, tiles_dir_terrain,
                                   vrt_altitude_1m, vrt_slope_1m, vrt_aspect_1m, resolution)
      },
      future.seed = TRUE
    )
    ...
    elapsed <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
    cat("Temps de calcul agrégation:", round(elapsed, 1), "s\n\n")
  } else {
    cat(">>> Tuiles terrain 25m déjà complètes\n\n")
  }

  # Assembler les tuiles terrain en VRT
  terrain_tiles <- list.files(tiles_dir_terrain, pattern = "\\.tif$", full.names = TRUE)
  cat("Tuiles terrain totales:", length(terrain_tiles), "\n")

  tile_names <- names(terra::rast(terrain_tiles[1]))
  metrics_terrain_raster <- terra::vrt(terrain_tiles, vrt_terrain, overwrite = TRUE)
  names(metrics_terrain_raster) <- tile_names
  cat("VRT terrain 25m créé:", basename(vrt_terrain), "\n")
}
...
# Répertoire pour les tuiles de prédiction
tiles_dir_prediction <- file.path(output_dir_rasters, "tiles_prediction")
if (!dir.exists(tiles_dir_prediction)) dir.create(tiles_dir_prediction, recursive = TRUE)
...
# VRT pour la prédiction
vrt_prediction <- file.path(output_dir_rasters, "prediction.vrt")
...
```{r ex-8-5, exercise=TRUE, exercise.timelimit=300, exercise.setup="ex-8-5-setup"}
# === 8.5 : Prédiction wall-to-wall avec aba_predict() (parallélisé) ===
...
# Compter les tuiles de prédiction existantes
existing_pred_tiles <- list.files(tiles_dir_prediction, pattern = "\\.tif$", full.names = TRUE)
n_existing <- length(existing_pred_tiles)

cat("Tuiles de référence (LAZ):", n_laz, "\n")
cat("Tuiles prédiction existantes:", n_existing, "\n\n")
...
predictors <- all.vars(model_aba$model$terms)[-1]
cat("Prédicteurs requis:", paste(predictors, collapse = ", "), "\n\n")

# Vérifier si le traitement est complet
if (file.exists(vrt_prediction) && n_existing >= n_laz) {
  cat(">>> Prédiction complète, chargement du VRT existant\n\n")
  prediction_map <- terra::rast(vrt_prediction)
} else {
  # Identifier les tuiles manquantes
  expected_pred_tiles <- sapply(laz_files, function(f) {
    laz_basename <- tools::file_path_sans_ext(basename(f))
    file.path(tiles_dir_prediction, paste0("pred_", laz_basename, ".tif"))
  })
  missing_mask <- !file.exists(expected_pred_tiles)
  n_missing <- sum(missing_mask)
  indices_to_process <- which(missing_mask)

  cat("Tuiles à calculer:", n_missing, "/", n_laz, "\n")

  # Supprimer le VRT obsolète
  if (file.exists(vrt_prediction)) file.remove(vrt_prediction)
  ...
  if (n_missing > 0) {
    ...
    # Récupérer les noms des couches depuis les tuiles TIF existantes
    # (les VRT perdent parfois les noms)
    tiles_points <- list.files(file.path(output_dir_rasters, "tiles_metrics_points"),
                                pattern = "\\.tif$", full.names = TRUE)
    tiles_trees <- list.files(file.path(output_dir_rasters, "tiles_metrics_trees"),
                               pattern = "\\.tif$", full.names = TRUE)
    tiles_terrain <- list.files(file.path(output_dir_rasters, "tiles_metrics_terrain"),
                                 pattern = "\\.tif$", full.names = TRUE)

    names_points <- names(terra::rast(tiles_points[1]))
    names_trees <- names(terra::rast(tiles_trees[1]))
    names_terrain <- names(terra::rast(tiles_terrain[1]))

    cat("Couches - Points:", length(names_points),
        "| Arbres:", length(names_trees),
        "| Terrain:", length(names_terrain), "\n")

    # Fonction de prédiction pour une tuile
    process_prediction_tile <- function(i, tile_extents, laz_files, tiles_dir_prediction,
                                         vrt_points, vrt_trees, vrt_terrain,
                                         names_points, names_trees, names_terrain,
                                         model_aba) {
      ext_i <- tile_extents[[i]]
      laz_basename <- tools::file_path_sans_ext(basename(laz_files[i]))

      # Fichier de sortie
      pred_file <- file.path(tiles_dir_prediction, paste0("pred_", laz_basename, ".tif"))

      # Emprise de la tuile
      ext_terra <- terra::ext(ext_i["xmin"], ext_i["xmax"], ext_i["ymin"], ext_i["ymax"])

      # Charger les métriques pour cette tuile
      metrics_points <- terra::crop(terra::rast(vrt_points), ext_terra)
      metrics_trees <- terra::crop(terra::rast(vrt_trees), ext_terra)
      metrics_terrain <- terra::crop(terra::rast(vrt_terrain), ext_terra)

      # Restaurer les noms des couches (perdus par les VRT)
      names(metrics_points) <- names_points
      names(metrics_trees) <- names_trees
      names(metrics_terrain) <- names_terrain

      # Harmoniser les emprises
      metrics_terrain <- terra::extend(metrics_terrain, metrics_points)
      metrics_terrain <- terra::crop(metrics_terrain, metrics_points)
      metrics_trees <- terra::extend(metrics_trees, metrics_points)
      metrics_trees <- terra::crop(metrics_trees, metrics_points)

      # Combiner
      metrics_tile <- c(metrics_points, metrics_trees, metrics_terrain)

      # Prédiction
      pred_tile <- lidaRtRee::aba_predict(model_aba, metrics_tile)

      # Filtrer les valeurs hors plage plausible (G entre 0 et 80 m²/ha)
      pred_tile[pred_tile < 0] <- NA
      pred_tile[pred_tile > 80] <- NA

      # Sauvegarder
      terra::writeRaster(pred_tile, pred_file, overwrite = TRUE)
    ...
    cat("\nTraitement parallèle avec", N_CORES, "workers...\n")
    start_time <- Sys.time()

    results <- future_lapply(
      indices_to_process,
      function(i) {
        process_prediction_tile(i, tile_extents, laz_files, tiles_dir_prediction,
                                 vrt_points, vrt_trees, vrt_terrain,
                                 names_points, names_trees, names_terrain,
                                 model_aba)
      },
  ...
  # Assembler les tuiles en VRT
  pred_tiles <- list.files(tiles_dir_prediction, pattern = "\\.tif$", full.names = TRUE)
  cat("Tuiles prédiction totales:", length(pred_tiles), "\n")

  prediction_map <- terra::vrt(pred_tiles, vrt_prediction, overwrite = TRUE)
  cat("VRT prédiction créé:", basename(vrt_prediction), "\n\n")
}
...
n_valid <- sum(!is.na(terra::values(prediction_map)))
surface_ha <- n_valid * (resolution^2) / 10000
...
### 8.6 Nettoyage, export et synthèse

La fonction `clean_raster()` permet d'appliquer des seuils de validité et un masque forêt pour filtrer les valeurs aberrantes et les zones non forestières. Le masque forêt est créé à partir de la carte d'occupation des sols **OSO** (CESBIO) en sélectionnant les classes forestières (17=Feuillus, 18=Conifères, 19=Mixte). La carte nettoyée est ensuite exportée au format GeoTIFF avec les statistiques de synthèse.
...
# Fichiers cache (exercices 8.2-8.5)
vrt_prediction <- file.path(output_dir_rasters, "prediction.vrt")
vrt_points <- file.path(output_dir_rasters, "metrics_points.vrt")

# Carte d'occupation des sols OSO (CESBIO)
oso_file <- file.path(data_dir, "oso_cache", "oso.tif")

# Charger la prédiction depuis le cache (exercice 8.5)
prediction_map <- terra::rast(vrt_prediction)

# Charger les métriques points pour zmean (hauteur de Lorey)
metrics_points <- terra::rast(vrt_points)
# Restaurer les noms depuis un fichier TIF
tiles_points <- list.files(file.path(output_dir_rasters, "tiles_metrics_points"),
                           pattern = "\\.tif$", full.names = TRUE)
if (length(tiles_points) > 0) {
  names(metrics_points) <- names(terra::rast(tiles_points[1]))
}
```

```{r ex-8-6, exercise=TRUE, exercise.timelimit=60, exercise.setup="ex-8-6-setup"}
# === 8.6 : Nettoyage avec clean_raster() et masque OSO ===
...
# Créer un masque forêt depuis OSO (CESBIO)
# Classes forestières OSO : 17=Feuillus, 18=Conifères, 19=Forêt mixte
if (file.exists(oso_file)) {
  cat("Chargement du masque OSO...\n")
  oso <- terra::rast(oso_file)

  # Reprojeter et rééchantillonner à la résolution de la prédiction
  if (!terra::same.crs(oso, prediction_map)) {
    cat("  - Reprojection vers CRS de la prédiction\n")
  }
  oso_aligned <- terra::project(oso, prediction_map, method = "near")

  # Créer le masque forêt (classes 17, 18, 19)
  forest_mask <- (oso_aligned == 17) | (oso_aligned == 18) | (oso_aligned == 19)

  # Statistiques du masque
  n_forest <- sum(terra::values(forest_mask), na.rm = TRUE)
  n_total <- sum(!is.na(terra::values(forest_mask)))

  cat("Masque forêt OSO créé:\n")
  cat("  - Classes: 17 (Feuillus), 18 (Conifères), 19 (Mixte)\n")
  cat("  - Pixels forêt:", n_forest, "(", round(100 * n_forest / n_total, 1), "%)\n")
  cat("  - Pixels total:", n_total, "\n\n")
} else {
  forest_mask <- NULL
  cat("Fichier OSO non trouvé:", oso_file, "\n")
...
# Sauvegarder la carte nettoyée avec nom explicite
output_file <- file.path(data_dir, "prediction_G_m2_ha.tif")
terra::writeRaster(prediction_clean, filename = output_file, overwrite = TRUE)
cat("Carte exportée:", basename(output_file), "\n\n")

# Visualisation comparaison
par(mfrow = c(1, 2), mar = c(2, 2, 3, 1))
terra::plot(prediction_map, main = "Avant nettoyage",
            col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
terra::plot(prediction_clean, main = "Après nettoyage (masque OSO)",
            col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
par(mfrow = c(1, 1))
...
# Hauteur de Lorey (hauteur pondérée par la surface terrière)
# H_Lorey = Σ(H × G) / Σ(G)
zmean_layer <- metrics_points[["zmean"]]
zmean_masked <- terra::mask(zmean_layer, prediction_clean)
zmean_values <- terra::values(zmean_masked)
zmean_values <- zmean_values[!is.na(zmean_values) & !is.na(g_values)]
g_for_lorey <- g_values[!is.na(terra::values(zmean_masked))]

# Filtrer les valeurs valides (G > 0 pour éviter division par 0)
valid_idx <- g_for_lorey > 0 & zmean_values > 0
zmean_valid <- zmean_values[valid_idx]
g_valid <- g_for_lorey[valid_idx]

h_lorey <- sum(zmean_valid * g_valid) / sum(g_valid)

cat("HAUTEUR DE LOREY:\n")
cat("  - H_Lorey:", round(h_lorey, 1), "m\n")
cat("  - H_min:", round(min(zmean_valid), 1), "m\n")
cat("  - H_max:", round(max(zmean_valid), 1), "m\n")
cat("  - H_mean:", round(mean(zmean_valid), 1), "m\n\n")
...
## Section 9 : Approche BABA - Cartographie Exploratoire

*Cartographie rapide des métriques LiDAR brutes, sans calibration terrain*

### Principe du BABA (Buffered Area-Based Approach)

Le **BABA** est une approche de cartographie **exploratoire** qui produit des cartes de métriques LiDAR brutes (hauteurs, couverture, etc.) sans nécessiter de placettes terrain.

**ATTENTION : BABA ne calcule PAS les variables forestières (G, V, N)**

Contrairement à l'ABA classique (sections 6-8), le BABA :

- **Ne prédit PAS** la surface terrière (G), le volume (V), ou la densité (N)
- **Produit uniquement** des métriques dérivées du nuage de points LiDAR
- **Ne nécessite pas** de calibration car il n'y a pas de modèle prédictif

**Pourquoi utiliser BABA ?**

- **Exploration rapide** : visualiser la structure forestière sans données terrain
- **Données limitées** : quand on n'a pas assez de placettes pour calibrer un modèle ABA
- **Haute résolution** : possibilité de pixels fins avec fenêtres de calcul plus grandes
- **Analyse qualitative** : identifier des zones d'intérêt avant un inventaire terrain

**Comparaison ABA vs BABA :**

| Aspect | ABA classique (sections 6-8) | BABA |
|--------|------------------------------|------|
| Données terrain | **Requises** (placettes) | Non requises |
| Calibration | Modèles statistiques | Aucune |
| Sortie | **Variables forestières** (G, V, N) | **Métriques LiDAR** (zmax, zmean, p95...) |
| Interprétation | Quantitative (m²/ha, m³/ha) | Qualitative (hauteurs en m, % couverture) |
| Résolution | Dépend de la densité de placettes | Libre (pixels fins possibles) |

### BABA vs Focal : quelle différence ?

On pourrait penser que BABA est équivalent à appliquer `focal()` sur un raster (CHM). En réalité, il y a une différence fondamentale :

| Aspect | BABA (lasR rasterize) | Focal sur raster (CHM) |
|--------|----------------------|------------------------|
| Données source | Nuage de points 3D | Raster 2D déjà agrégé |
| Calcul | Sur TOUS les points dans la fenêtre | Sur les valeurs de pixels voisins |
| Information | Accès aux points bruts | Perte d'info par agrégation préalable |

**Exemple :**

- **BABA `p95`** : percentile 95 calculé sur ~50 000 points LiDAR dans la fenêtre 30×30m
- **Focal `p95` sur CHM 1m** : percentile 95 calculé sur 900 valeurs de pixels (déjà des max)

**En pratique :**

- `zmax` : quasi identique (le max reste le max)
- `zmean` : différent (BABA = moyenne de tous les points, Focal = moyenne des max par cellule)
- `p95` : très différent (BABA = vrai percentile sur points, Focal = percentile de valeurs déjà agrégées)
- `above2` (couverture) : impossible avec Focal sur CHM (information perdue lors de la rastérisation)

**Conclusion :** Si vous avez déjà un CHM et voulez juste le lisser, `focal()` suffit. BABA est utile pour les métriques de distribution (percentiles, couverture, strates) calculées directement sur les points bruts.
...
# CHM filled existant (créé dans section 3)
chm_filled_vrt <- file.path(data_dir, "c_filled_complet.vrt")

# Répertoire de sortie BABA
output_dir_baba <- file.path(data_dir, "result_baba")
dir.create(output_dir_baba, recursive = TRUE, showWarnings = FALSE)
...
cat("=== BABA vs Focal : Démonstration sur 2 dalles ===\n\n")

cat("OBJECTIF: Montrer la différence entre BABA et Focal sans buffer\n")
cat("─────────────────────────────────────────────────────────────\n\n")
...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  # Sélectionner 4 dalles LAZ pour la démonstration
  all_laz <- dir(laz_dir, pattern = "\\.laz$", full.names = TRUE)
  if (length(all_laz) == 0) {
    stop("Aucun fichier LAZ trouvé dans: ", laz_dir)
  }
  laz_files <- head(all_laz, 2)
  cat("Dalles sélectionnées:", length(laz_files), "\n")
  cat(paste("  -", basename(laz_files), collapse = "\n"), "\n\n")

  # Calculer l'emprise des 2 dalles pour le crop
  headers <- lapply(laz_files, function(f) {
    lidR::readLASheader(f)
  })
  xmin <- min(sapply(headers, function(h) h@PHB$`Min X`))
  xmax <- max(sapply(headers, function(h) h@PHB$`Max X`))
  ymin <- min(sapply(headers, function(h) h@PHB$`Min Y`))
  ymax <- max(sapply(headers, function(h) h@PHB$`Max Y`))
  demo_extent <- terra::ext(xmin, xmax, ymin, ymax)
  cat("Emprise de démonstration:", round(xmax - xmin), "x", round(ymax - ymin), "m\n\n")

  # ==========================================================================
  # 1. MÉTHODE BABA : calcul direct sur points avec fenêtre glissante
  # ==========================================================================
  cat("=== 1. BABA (fenêtre 30m sur points bruts) ===\n\n")

  # Filtrage des points aberrants (z < 0 ou z > 60m)
  cat("Filtrage des points: 0m <= z <= 60m\n\n")

  # Répertoire pour les tuiles BABA
  tiles_dir_baba <- file.path(output_dir_baba, "tiles_zmean")
  dir.create(tiles_dir_baba, recursive = TRUE, showWarnings = FALSE)

  # Joker '*' pour créer une tuile par fichier LAZ
  output_baba_pattern <- file.path(tiles_dir_baba, "zmean_*.tif")

  # Pipeline avec filtre sur Z (exclut points aberrants)
  pipeline_baba <- reader_las(filter = "-drop_z_below 0 -drop_z_above 60") +
    rasterize(
      res = c(10, 30),  # 10m sortie, 30m fenêtre
      operators = "mean",
      ofile = output_baba_pattern
    )

  t0 <- Sys.time()
  exec(pipeline_baba, on = laz_files, ncores = N_CORES, progress = TRUE)
  t_baba <- round(difftime(Sys.time(), t0, units = "secs"), 1)

  # Assembler les tuiles en VRT
  baba_tiles <- list.files(tiles_dir_baba, pattern = "\\.tif$", full.names = TRUE)
  baba_vrt <- file.path(output_dir_baba, "zmean_baba.vrt")
  baba_result <- terra::vrt(baba_tiles, baba_vrt, overwrite = TRUE)
  names(baba_result) <- "zmean_baba"

  cat("BABA terminé en", t_baba, "sec\n")
  cat("  Tuiles créées:", length(baba_tiles), "\n")
  cat("  VRT:", basename(baba_vrt), "\n")
  cat("  Résolution:", terra::res(baba_result)[1], "m\n\n")

  # ==========================================================================
  # 2. MÉTHODE FOCAL : CHM existant puis focal mean (SANS buffer = artefacts)
  # ==========================================================================
  cat("=== 2. Focal sur CHM existant (section 3) ===\n\n")

  if (!file.exists(chm_filled_vrt)) {
    cat("ERREUR: CHM non trouvé:", chm_filled_vrt, "\n")
    cat("Exécutez d'abord la section 3.\n")
  } else {
    # Charger le CHM filled et cropper sur l'emprise des 2 dalles
    chm_full <- terra::rast(chm_filled_vrt)
    chm_crop <- terra::crop(chm_full, demo_extent)

    # Filtrer les valeurs aberrantes (z < 0 ou z > 60m)
    chm_crop <- terra::clamp(chm_crop, lower = 0, upper = 60, values = TRUE)
    cat("CHM chargé depuis section 3 (résolution:", terra::res(chm_crop)[1], "m)\n")
    cat("Filtrage appliqué: 0m <= z <= 60m\n")

    # Appliquer focal mean avec fenêtre ~30m (CHM à 0.5m → 61×61 pixels = 30.5m)
    t0 <- Sys.time()
    focal_window <- matrix(1, nrow = 61, ncol = 61)  # 61 × 0.5m = 30.5m
    focal_result <- terra::focal(chm_crop, w = focal_window, fun = "mean", na.rm = TRUE)
    names(focal_result) <- "zmean_focal"

    # Rééchantillonner à 10m pour comparaison (0.5m × 20 = 10m)
    focal_10m <- terra::aggregate(focal_result, fact = 20, fun = "mean", na.rm = TRUE)
    t_focal <- round(difftime(Sys.time(), t0, units = "secs"), 1)
    cat("Focal mean calculé en", t_focal, "sec (fenêtre 30m, sortie 10m)\n\n")

    # ========================================================================
    # 3. COMPARAISON VISUELLE
    # ========================================================================
    cat("=== 3. Comparaison des résultats ===\n\n")

    # Aligner les deux rasters
    focal_aligned <- terra::resample(focal_10m, baba_result, method = "bilinear")

    # Calculer la différence
    diff_raster <- baba_result - focal_aligned
    names(diff_raster) <- "difference"

    cat("Statistiques BABA (zmean sur points):\n")
    cat("  Min:", round(terra::minmax(baba_result)[1], 2), "m\n")
    cat("  Max:", round(terra::minmax(baba_result)[2], 2), "m\n")
    cat("  Mean:", round(mean(terra::values(baba_result), na.rm = TRUE), 2), "m\n\n")

    cat("Statistiques Focal (mean sur CHM max):\n")
    cat("  Min:", round(terra::minmax(focal_aligned)[1], 2), "m\n")
    cat("  Max:", round(terra::minmax(focal_aligned)[2], 2), "m\n")
    cat("  Mean:", round(mean(terra::values(focal_aligned), na.rm = TRUE), 2), "m\n\n")

    cat("Différence (BABA - Focal):\n")
    diff_vals <- terra::values(diff_raster)
    diff_vals <- diff_vals[!is.na(diff_vals)]
    cat("  Min:", round(min(diff_vals), 2), "m\n")
    cat("  Max:", round(max(diff_vals), 2), "m\n")
    cat("  Mean:", round(mean(diff_vals), 2), "m\n")
    cat("  RMSE:", round(sqrt(mean(diff_vals^2)), 2), "m\n\n")

    # Visualisation
    par(mfrow = c(2, 2), mar = c(2, 2, 3, 1))
    terra::plot(baba_result, main = "BABA: zmean sur points",
                col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
    terra::plot(focal_aligned, main = "Focal: mean sur CHM",
                col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
    terra::plot(diff_raster, main = "Différence (BABA - Focal)",
                col = grDevices::hcl.colors(50, "RdBu"))
    hist(diff_vals, breaks = 50, main = "Distribution des différences",
         xlab = "Différence (m)", col = "steelblue")
    par(mfrow = c(1, 1))

    # Sauvegarder les résultats
    terra::writeRaster(diff_raster, file.path(output_dir_baba, "diff_baba_focal.tif"),
                       overwrite = TRUE)

    cat("\n=== CONCLUSION ===\n")
    cat("BABA calcule zmean sur TOUS les points dans la fenêtre.\n")
    cat("Focal calcule la moyenne des HAUTEURS MAX (CHM) des pixels.\n")
    cat("→ zmean BABA < zmean Focal car Focal utilise les max (CHM).\n")
    cat("→ La différence reflète l'écart entre moyenne des points et moyenne des max.\n")
  }
}
```
...
quiz(
  ...
  question("Quelle différence entre BABA et focal() sur un CHM ?",
    answer("Aucune, c'est équivalent"),
    answer("BABA calcule sur les points bruts, focal sur un raster déjà agrégé", correct = TRUE),
    answer("Focal est plus précis car il utilise plus de données"),
    answer("BABA ne fonctionne qu'avec des fenêtres carrées"),
    allow_retry = TRUE
  ),
...
```
  ...
  │   - Parallélisation par tuiles avec cache incrémental   │
  │   → Produits : result_itd/ (CHM, crowns, seeds, arbres) │
  └─────────────────────────────────────────────────────────┘
                          │
  ┌─────────────────────────────────────────────────────────┐
  │ Section 4 : Trouées et Lisières                         │
  ...
  │   → Produits : models_aba.rda (modèles calibrés G)      │
  └─────────────────────────────────────────────────────────┘
                          │
  ┌─────────────────────────────────────────────────────────┐
  │ Section 8 : Cartographie et Inférence                   │
  │   - raster_metrics() : métriques points/arbres/terrain  │
  │   - aba_predict() : prédiction wall-to-wall             │
  │   - clean_raster() : seuils + masque OSO forêt          │
  │   - Parallélisation par tuiles (future_lapply)          │
  │   - Réutilisation du DTM section 3                      │
  │   - Hauteur de Lorey (H pondérée par G)                 │
  │   → Produits : prediction_G_m2_ha.tif, rasters_aba/     │
  └─────────────────────────────────────────────────────────┘
                          │
  ┌─────────────────────────────────────────────────────────┐
  │ Section 9 : Approche BABA (Cartographie Exploratoire)   │
  │   - lasR : rasterize avec res = c(pixel, fenêtre)       │
  │   - ATTENTION : ne calcule PAS G, V, N                  │
  │   - Comparaison BABA vs focal() sur CHM                 │
  │   → Produits : métriques LiDAR brutes (zmax, zmean...)  │
  └─────────────────────────────────────────────────────────┘
```

### Produits pour indicateurs nemeton

| Produit                  | Fonction / Source        | Indicateurs       |
|--------------------------|--------------------------|-------------------|
| result_itd/arbres_*.gpkg | tree_extraction()        | P1, P3, E1        |
| gaps.gpkg                | gap_detection()          | B2, L1            |
| edges.gpkg               | edge_detection()         | L1                |
| prediction_G_m2_ha.tif   | aba_predict() + OSO mask | P1, C1            |
| rasters_aba/*.vrt        | métriques intermédiaires | (cache)           |
| result_baba/zmean_*.tif  | lasR BABA (exploration)  | —                 |

**Note importante :** Les produits BABA (zmean, zmax, p95...) sont des métriques LiDAR brutes, pas des variables forestières. Pour obtenir G, V, N, il faut des placettes terrain et une calibration ABA (sections 6-8).

### Optimisation avec lasR et parallélisation

Pour les gros volumes de données, ce tutorial utilise :

- **Parallélisation** : `future_lapply()` pour traiter les tuiles en parallèle
- **Cache incrémental** : sauvegarde par tuile, reprise en cas d'interruption
- **VRT** : assemblage virtuel sans copie de données
- **lasR** : traitement C++ ultra-rapide avec `ncores`
- **Réutilisation** : DTM section 3 → section 8, CHM section 3 → section 9
