...
if (dir.exists(laz_dir_normalized)) {
  ...
  if (length(laz_files) > 0) {
    ...
    # Configuration du traitement par tuiles
    lidR::opt_chunk_buffer(cata_height) <- 30  # Buffer de 30m pour éviter effets de bord
    lidR::opt_progress(cata_height) <- TRUE

    # Statistiques du LAScatalog
    bbox <- sf::st_bbox(cata_height)
    area_km2 <- (bbox["xmax"] - bbox["xmin"]) * (bbox["ymax"] - bbox["ymin"]) / 1e6

    cat("LAScatalog créé:\n")
    cat("  - Nombre de tuiles:", length(cata_height$filename), "\n")
    ...
    plot(cata_height, main = "LAScatalog - 18 tuiles normalisées")
...
Les métriques du nuage de points sont calculées sur l'ensemble de la zone d'étude via le LAScatalog. Le résultat est sauvegardé en cache pour réutilisation dans les exercices suivants.
...
# Fichier cache pour les métriques points
cache_points <- file.path(output_dir_rasters, "metrics_points_raster.tif")
```

```{r ex-8-2, exercise=TRUE, exercise.timelimit=600, exercise.setup="ex-8-2-setup"}
...
if (file.exists(cache_points)) {
  cat(">>> Cache trouvé, chargement depuis:", basename(cache_points), "\n\n")
  metrics_points_raster <- terra::rast(cache_points)
} else {
  cat("Calcul des métriques points (ceci peut prendre plusieurs minutes)...\n\n")
  ...
  cat("LAScatalog:", length(cata_height$filename), "tuiles\n")

  # Calcul des métriques via pixel_metrics sur le catalog
  cat("Calcul pixel_metrics avec aba_metrics()...\n")
  metrics_points_raster <- lidR::pixel_metrics(cata_height,
                                                aba_point_metrics_fun,
                                                res = resolution)

  # Remplacer les hauteurs négatives par 0
  zmax <- metrics_points_raster[["zmax"]]
  zmax[zmax < 0] <- 0
  metrics_points_raster[["zmax"]] <- zmax

  # Sauvegarder en cache
  terra::writeRaster(metrics_points_raster, cache_points, overwrite = TRUE)
  cat("\n>>> Métriques sauvegardées:", basename(cache_points), "\n")
}
...
cat("  - Premières couches:", paste(head(names(metrics_points_raster), 5),
                                    collapse = ", "), "...\n")
...
Les métriques basées sur la détection d'arbres sont calculées sur l'ensemble de la zone via création du MNC (Modèle Numérique de Canopée), segmentation et agrégation par pixel.
...
# Fichiers cache
cache_chm <- file.path(output_dir_rasters, "chm.tif")
...
if (file.exists(cache_trees)) {
  ...
  # 1. Créer le MNC via LAScatalog
  cat("1. Création du MNC (résolution:", aba_res_chm, "m)...\n")

  if (!file.exists(cache_chm)) {
    # Créer le LAScatalog
    filter_cata_height <- paste(c("-keep_class", class_points,
                                  "-drop_z_above", h_points), collapse = " ")
    ...
    # Créer le MNC
    chm <- lidR::rasterize_canopy(cata_height, res = aba_res_chm,
                                   algorithm = lidR::p2r())
    chm[is.na(chm) | chm < 0] <- 0

    # Sauvegarder le CHM
    terra::writeRaster(chm, cache_chm, overwrite = TRUE)
    cat("   MNC sauvegardé:", basename(cache_chm), "\n")
  } else {
    cat("   MNC chargé depuis cache\n")
    chm <- terra::rast(cache_chm)
  }
...
Les métriques de terrain (altitude, pente, aspect) sont calculées à partir des points classifiés sol sur l'ensemble de la zone, en utilisant les fichiers LAZ **originaux** (non normalisés).
...
# Fichier cache
cache_terrain <- file.path(output_dir_rasters, "metrics_terrain_raster.tif")
```
...
# Vérifier si le cache existe déjà
if (file.exists(cache_terrain)) {
  cat(">>> Cache trouvé, chargement depuis:", basename(cache_terrain), "\n\n")
  metrics_terrain_raster <- terra::rast(cache_terrain)
} else {
  cat("Calcul des métriques terrain (ceci peut prendre plusieurs minutes)...\n\n")
  ...
  lidR::projection(cata_ground) <- 2154
  lidR::opt_chunk_buffer(cata_ground) <- 30
  lidR::opt_progress(cata_ground) <- TRUE

  cat("LAScatalog (tuiles originales):", length(cata_ground$filename), "tuiles\n")
  cat("Filtre appliqué:", filter_ground, "(points sol uniquement)\n\n")

  # Calcul des métriques terrain via pixel_metrics
  cat("Calcul pixel_metrics terrain...\n")
  metrics_terrain_raster <- lidR::pixel_metrics(cata_ground,
                                                 ~ f_terrain(X, Y, Z),
                                                 res = resolution)
  ...
  # Sauvegarder en cache
  terra::writeRaster(metrics_terrain_raster, cache_terrain, overwrite = TRUE)
  cat("\n>>> Métriques terrain sauvegardées:", basename(cache_terrain), "\n")
}
