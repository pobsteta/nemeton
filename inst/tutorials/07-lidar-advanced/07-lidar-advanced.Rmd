...
for (i in 1:nrow(plots)) {
  tryCatch({
    ...
    if (!lidR::is.empty(las)) {
      ...
      llas_height[[plots$plot_id[i]]] <- las
    }
  }, error = function(e) NULL)
}
```
...
for (i in 1:nrow(plots)) {
  tryCatch({
    ...
    if (!lidR::is.empty(las)) {
      llas_original[[plots$plot_id[i]]] <- las
    }
  }, error = function(e) NULL)
}
```
...
for (i in 1:nrow(plots)) {
  tryCatch({
    ...
    if (!lidR::is.empty(las_orig)) {
      ...
      metrics_terrain[[plots$plot_id[i]]] <- lidaRtRee::terrain_points_metrics(las_orig, center_xy, p_radius)
    }
  }, error = function(e) NULL)
}
...
plots <- quatre_montagnes[grepl(paste0("^", pattern_zone, "-"), quatre_montagnes$plot_id), ]
# plots$plot_id already exists in quatre_montagnes
...
for (i in 1:nrow(plots)) {
  tryCatch({
    ...
    if (!lidR::is.empty(las_orig)) {
      center_xy <- as.numeric(plots[i, c("X", "Y")])
      metrics_terrain[[plots$plot_id[i]]] <- lidaRtRee::terrain_points_metrics(las_orig, center_xy, plot_radius)
    }
  }, error = function(e) NULL)
}
metrics_terrain <- as.data.frame(do.call(rbind, metrics_terrain))
