...
# Vérifier si le VRT existe déjà
if (file.exists(vrt_points)) {
  cat(">>> VRT trouvé, chargement depuis:", basename(vrt_points), "\n\n")
  metrics_points_raster <- terra::rast(vrt_points)
} else {
  cat("Calcul des métriques points tuile par tuile...\n\n")

  # Configuration parallélisation avec future
  library(future)
  plan(multisession, workers = N_CORES)
  cat("Parallélisation: future avec", N_CORES, "workers\n\n")
  ...
  cat("Configuration LAScatalog:\n")
  cat("  - Tuiles:", nrow(cata_height), "\n")
  cat("  - Buffer:", lidR::opt_chunk_buffer(cata_height), "m\n")
  cat("  - Sortie:", basename(tiles_dir_points), "/\n\n")

  # Calcul des métriques (lidR utilise future automatiquement)
...
# Vérifier si le VRT existe déjà
if (file.exists(vrt_terrain)) {
  cat(">>> VRT trouvé, chargement depuis:", basename(vrt_terrain), "\n\n")
  metrics_terrain_raster <- terra::rast(vrt_terrain)
} else {
  cat("Calcul des métriques terrain tuile par tuile...\n\n")
  ...
  # Calcul des métriques terrain (lidR utilise future automatiquement)
  ...
  cat("Tuiles générées:", length(terrain_tiles), "\n")
