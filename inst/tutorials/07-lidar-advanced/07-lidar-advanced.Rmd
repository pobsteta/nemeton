...
laz_raw_dir <- file.path(data_dir, "laz")
laz_raw_files <- list.files(laz_raw_dir, pattern = "\\.la[sz]$", full.names = TRUE)

# Répertoire de sortie pour les fichiers normalisés
laz_norm_dir <- file.path(data_dir, "laz_normalized_lasr")
dir.create(laz_norm_dir, showWarnings = FALSE, recursive = TRUE)
```

```{r ex-5-1a, exercise=TRUE, exercise.timelimit=600, exercise.setup="ex-5-1a-setup"}
library(lasR)
...
existing_norm <- list.files(laz_norm_dir, pattern = "\\.laz$", full.names = TRUE)
if (length(existing_norm) >= length(laz_raw_files)) {
  cat("Fichiers déjà normalisés:", length(existing_norm), "\n")
  cat("Utilisation du cache existant.\n\n")
} else {
  ...
  pipeline <- reader_las() +
    triangulate(filter = keep_ground()) +
    transform_with(stage(triangulate), operator = "-") +
    write_las(ofile = paste0(laz_norm_dir, "/*_norm.laz"))

  # Exécution parallélisée
  ans <- exec(pipeline, on = laz_raw_files, ncores = n_cores, progress = TRUE)

  elapsed <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
  cat("\nTemps de traitement:", round(elapsed, 1), "s\n")
  cat("Vitesse:", round(length(laz_raw_files) / elapsed * 60, 1), "fichiers/min\n\n")
}
...
# Priorité aux fichiers normalisés par lasR, sinon ceux de la section 3
laz_norm_lasr <- file.path(data_dir, "laz_normalized_lasr")
laz_norm_lidr <- file.path(data_dir, "laz_normalized")

if (length(list.files(laz_norm_lasr, pattern = "\\.laz$")) > 0) {
  laz_dir <- laz_norm_lasr
} else {
  laz_dir <- laz_norm_lidr
}
...
```{r ex-5-1b, exercise=TRUE, exercise.timelimit=600, exercise.setup="ex-5-1b-setup"}
library(lidaRtRee)
library(lidR)
library(sf)
library(future)
...
if (length(laz_files) == 0) {
  stop("Aucun fichier LAZ normalisé trouvé. Exécutez d'abord l'exercice 5.1a ou la section 3.")
}
...
# Indice de Simpson sur les classes de hauteur
# Simpson = 1 - Σ(pi²) où pi = proportion dans chaque classe
# Mesure la diversité : 0 = une seule classe, 1 = distribution uniforme
classes_hauteur <- cut(trees$height,
                        breaks = c(0, 5, 10, 15, 20, 25, 30, Inf),
                        labels = c("0-5", "5-10", "10-15", "15-20", "20-25", "25-30", ">30"))
freq_classes <- table(classes_hauteur) / length(trees$height)
simpson <- 1 - sum(freq_classes^2)

cat("Indice de diversité:\n")
cat("  Simpson:", round(simpson, 3), "(diversité des classes de hauteur)\n")
cat("  Interprétation: 0 = homogène, 1 = très diversifié\n\n")

cat("Distribution par classe de hauteur:\n")
print(table(classes_hauteur))
