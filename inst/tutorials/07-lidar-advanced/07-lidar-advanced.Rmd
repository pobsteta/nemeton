...
```{r ex-3-1, exercise=TRUE, exercise.timelimit=1800, exercise.setup="ex-3-1-setup"}
# =============================================================================
# GESTION MÉMOIRE - Important pour les gros jeux de données
# =============================================================================
gc()  # Forcer le garbage collection avant traitement
...
cat("2. triangulate()         - Maillage TIN des premiers retours\n")
cat("3. rasterize()           - CHM à 0.5m de résolution\n")
cat("4. pit_fill()            - Remplissage des trous\n")
cat("5. local_maximum_raster()- Détection des cimes\n")
cat("6. region_growing()      - Segmentation des houppiers\n\n")
...
if (length(fichiers_laz) == 0) {
  ...
  # ==========================================================================
  # PIPELINE ITD lasR COMPLET
  # ==========================================================================

  # 1. Créer l'étape de triangulation (référencée par rasterize)
  del <- triangulate(filter = keep_first())

  # 2. Créer l'étape CHM (référencée par pit_fill)
  chm <- lasR::rasterize(0.5, del,
                   ofile = file.path(result_itd, "chm_*.tif"))

  # 3. Créer l'étape de remplissage des puits (référencée par local_maximum et region_growing)
  chm_filled <- pit_fill(chm,
                         ofile = file.path(result_itd, "c_filled_*.tif"))

  # 4. Créer l'étape de détection des maxima locaux (référencée par region_growing)
  # Fenêtre de 3m - ajuster selon la densité du peuplement
  # Un arbre fait minimum 5 m
  seeds <- local_maximum_raster(chm_filled, ws = 3, min_height = 5,
                                ofile = file.path(result_itd, "seeds_*.gpkg"))

  # 5. Créer l'étape de segmentation par croissance de région
  tree <- region_growing(chm_filled, seeds,
                         ofile = file.path(result_itd, "crowns_*.tif"))

  # Pipeline ITD complet pour nemeton
  ...
  chm_tiles <- list.files(file.path(data_dir, "result_itd"), pattern = "^chm_.*\\.tif$", full.names = TRUE)
  c_filled_tiles <- list.files(file.path(data_dir, "result_itd"), pattern = "^c_filled_.*\\.tif$", full.names = TRUE)
  crown_tiles <- list.files(file.path(data_dir, "result_itd"), pattern = "^crowns_.*\\.tif$", full.names = TRUE)
  seed_files <- list.files(file.path(data_dir, "result_itd"), pattern = "^seeds_.*\\.gpkg$", full.names = TRUE)

  cat("Tuiles CHM:", length(chm_tiles), "\n")
  cat("Tuiles CHM filled:", length(c_filled_tiles), "\n")
  ...
  # Création des tif complet
  cat("\n=== Fusion physique du CHM (optionnel) ===\n")
  chm_merge <- terra::merge(sprc(chm_tiles))
  writeRaster(chm_merge, file.path(data_dir, "chm_complet.tif"), overwrite = TRUE)
  cat("CHM complet écrit:", file.path(data_dir, "chm_complet.tif"), "\n")
  
  cat("\n=== Fusion physique des CROWNS (optionnel) ===\n")
  crown_merge <- terra::merge(sprc(crown_tiles))
  writeRaster(crown_merge, file.path(data_dir, "crown_complet.tif"), overwrite = TRUE)
  cat("CROWNS complet écrit:", file.path(data_dir, "crown_complet.tif"), "\n")

  # Visualisation du CHM avec segments (zoom 100m x 100m centré)
  ...
  if (exists("chm_vrt") && exists("crowns_vrt") && exists("c_filled_vrt")) {
    # Calculer le centre de l'emprise
    ext <- terra::ext(chm_vrt)
    ...
    # Filtrer les cimes dans l'emprise de zoom
    if (exists("seeds_all")) {
      seeds_zoom <- sf::st_crop(seeds_all, sf::st_bbox(c(
        xmin = center_x - 50, xmax = center_x + 50,
        ymin = center_y - 50, ymax = center_y + 50
      ), crs = sf::st_crs(seeds_all)))
    }

    cat("\n=== Visualisation : zoom 100m x 100m centré ===\n")
    cat("Centre:", round(center_x), ",", round(center_y), "\n")
    if (exists("seeds_zoom")) {
      cat("Arbres dans la zone:", nrow(seeds_zoom), "\n")
    }

    par(mfrow = c(1, 3))
    plot(chm_zoom, main = "CHM (0.5m)", col = col, mar = c(1, 1, 1, 3))
    plot(chm_filled_zoom, main = "CHM filled (0.5m)", col = col, mar = c(1, 1, 1, 3))
    plot(crowns_zoom %% 8, main = "Segments + cimes",
         col = col2[sample.int(50, 1000, TRUE)], mar = c(1, 1, 1, 3), legend = FALSE)
    # Ajouter les cimes des houppiers
    if (exists("seeds_zoom") && nrow(seeds_zoom) > 0) {
  ...
  # NETTOYAGE MÉMOIRE
  # ==========================================================================
  # Supprimer les objets volumineux intermédiaires
  # rm(del, chm, chm_filled, seeds, tree, pipeline_itd, ans)
  invisible(gc())  # Forcer la libération mémoire
  cat("\n=== Mémoire libérée ===\n")
  cat("Mémoire utilisée après nettoyage:", round(sum(gc()[, 2]), 1), "MB\n")
}
```

### Exercice 3.3 : Extraction des attributs d'arbres avec lasR (haute performance)

Pour les gros jeux de données, lasR offre un pipeline optimisé qui peut également
calculer toutes les métriques d'arbres équivalentes à `tree_extraction()` de lidaRtRee
...
```{r ex-3-3, exercise=TRUE, exercise.timelimit=1800, exercise.setup="ex-3-3-setup"}
# Segmentation d'arbres ET extraction des métriques avec lasR
...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  # ==========================================================================
  # CALLBACK : Extraction des métriques d'arbres (équivalent tree_extraction)
  # ==========================================================================
  # Ce callback réplique exactement les métriques de lidaRtRee::tree_extraction()
  # - id : identifiant unique de l'arbre
  # - x, y : coordonnées de la cime
  # - height : hauteur de la cime (m)
  # - dom_radius : rayon de dominance (m)
  # - surface : surface du houppier (m²)
  # - volume : volume du houppier (m³)
  # - crown_geom : géométrie WKT du polygone du houppier

  tree_metrics_callback <- function(chm, crowns, seeds) {
    if (is.null(chm) || is.null(crowns) || is.null(seeds)) return(NULL)

    # Résolution du raster
    res_x <- terra::xres(chm)
    res_y <- terra::yres(chm)
    pixel_area <- res_x * res_y

    # Extraire les valeurs des rasters
    chm_vals <- terra::values(chm)[, 1]
    crown_vals <- terra::values(crowns)[, 1]
    seed_vals <- terra::values(seeds)[, 1]

    # Identifier les cellules des maxima locaux (cimes)
    cells_maxi <- which(!is.na(seed_vals) & seed_vals > 0)
    if (length(cells_maxi) == 0) return(NULL)

    # Coordonnées et attributs de base des cimes
    coords <- terra::xyFromCell(chm, cells_maxi)
    metrics <- data.frame(
      id = crown_vals[cells_maxi],
      x = coords[, 1],
      y = coords[, 2],
      height = chm_vals[cells_maxi],
      dom_radius = seed_vals[cells_maxi]
    )

    # Masque des pixels valides pour les calculs de surface/volume
    valid <- !is.na(crown_vals) & crown_vals > 0 & !is.na(chm_vals)

    # Surface : comptage de pixels par segment × surface pixel
    surf_table <- table(crown_vals[valid])
    surf_df <- data.frame(
      id = as.integer(names(surf_table)),
      surface = as.numeric(surf_table) * pixel_area
    )

    # Volume : somme des hauteurs par segment × surface pixel
    vol_agg <- aggregate(chm_vals[valid],
                         by = list(id = crown_vals[valid]),
                         FUN = sum)
    vol_df <- data.frame(id = vol_agg$id, volume = vol_agg$x * pixel_area)

    # Fusion des métriques
    metrics <- merge(metrics, surf_df, by = "id", all.x = TRUE)
    metrics <- merge(metrics, vol_df, by = "id", all.x = TRUE)

    # Géométrie des couronnes (polygones vectorisés)
    crown_polys <- terra::as.polygons(crowns)
    crown_sf <- sf::st_as_sf(crown_polys)
    names(crown_sf)[1] <- "id"

    # Calcul de la précision pour WKT (comme lidaRtRee)
    bbox_max <- max(abs(sf::st_bbox(crown_sf)))
    n_digits <- ceiling(log10(bbox_max)) +
                abs(min(floor(log10(min(terra::res(crowns))/10)), 0))
    crown_sf$crown_geom <- sf::st_as_text(sf::st_geometry(crown_sf),
                                           digits = n_digits)
    ...
    # Convertir en objet sf avec géométrie ponctuelle à la cime
    sf::st_as_sf(metrics, coords = c("x", "y"), crs = terra::crs(chm))
  }

  # ==========================================================================
  # PIPELINE lasR COMPLET AVEC EXTRACTION DES MÉTRIQUES
  # ==========================================================================

  # Dossier de sortie
  result_dir <- file.path(data_dir, "lasR_itd_metrics")
  ...
  seeds <- local_maximum_raster(chm_filled, ws = 3)

  # 5. Segmentation par croissance de région
  crowns <- region_growing(chm_filled, seeds)

  # 6. Callback pour extraction des métriques
  tree_metrics <- callback(
    fun = tree_metrics_callback,
    expose = "rst",
    ofile = file.path(result_dir, "tree_metrics_*.gpkg")
  )

  # Pipeline complet
  pipeline_itd <- reader_las() + del + chm + chm_filled + seeds + crowns + tree_metrics

  cat("=== Pipeline lasR avec extraction des métriques ===\n")
  print(pipeline_itd)

  cat("\n=== Avantages de cette approche ===\n")
  cat("- Traitement 5-10x plus rapide que lidR/lidaRtRee\n")
  cat("- Toutes les métriques calculées en un seul passage\n")
  cat("- Métriques identiques à tree_extraction() de lidaRtRee\n")
  cat("- Idéal pour gros jeux de données (>100 dalles)\n")

  # Exécution sur les données (si disponibles)
  if (length(fichiers_laz) > 0) {
    cat("\n=== Exécution sur", length(fichiers_laz), "fichier(s) ===\n")

    ans <- exec(pipeline_itd, on = fichiers_laz[1],
                with = list(tree_metrics = list(
                  chm = chm_filled,
                  crowns = crowns,
                  seeds = seeds
                )))
    ...
    gpkg_files <- list.files(result_dir, pattern = "tree_metrics.*\\.gpkg$",
                             full.names = TRUE)
    if (length(gpkg_files) > 0) {
      trees <- sf::st_read(gpkg_files[1], quiet = TRUE)

      cat("\n=== Résultats : Métriques des arbres ===\n")
      cat("Nombre d'arbres détectés:", nrow(trees), "\n")
      cat("Hauteur moyenne:", round(mean(trees$height, na.rm = TRUE), 1), "m\n")
      cat("Hauteur max:", round(max(trees$height, na.rm = TRUE), 1), "m\n")
      cat("Surface moyenne houppier:", round(mean(trees$surface, na.rm = TRUE), 1), "m²\n")
      cat("Volume moyen houppier:", round(mean(trees$volume, na.rm = TRUE), 1), "m³\n")

      cat("\n=== Premiers arbres ===\n")
      print(head(sf::st_drop_geometry(trees[, c("id", "height", "dom_radius",
                                                 "surface", "volume")])))
    }
  } else {
    cat("\nAucun fichier LAZ trouvé. Testez avec vos propres données :\n")
    cat("ans <- exec(pipeline_itd, on = 'votre_fichier.laz',\n")
    cat("            with = list(tree_metrics = list(\n")
    cat("              chm = chm_filled, crowns = crowns, seeds = seeds)))\n")
  }
}
```
...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  # Exécution avec parallélisation (décommenter avec vos données)
  # N_CORES <- 6L
  # result <- exec(pipeline_baba, on = fichiers_laz,
  #                ncores = concurrent_files(N_CORES))
}
```
