...
if (file.exists(vrt_points) && n_existing >= n_laz) {
  ...
  # Déterminer si c'est une reprise ou un nouveau calcul
  if (n_existing > 0 && n_existing < n_laz) {
    cat(">>> Reprise du traitement (", n_existing, "/", n_laz, " tuiles déjà calculées)\n\n")
  ...
  cat("  - Note: lidR saute automatiquement les tuiles existantes\n\n")
...
if (file.exists(cache_trees)) {
  ...
  if (file.exists(vrt_chm) && n_existing_chm >= n_laz) {
    ...
    # Déterminer si c'est une reprise
    if (n_existing_chm > 0 && n_existing_chm < n_laz) {
      cat("   >>> Reprise (", n_existing_chm, "/", n_laz, " tuiles existantes)\n")
    }
    ...
    cat("   Tuiles à traiter:", nrow(cata_height), "\n")
    cat("   Buffer:", lidR::opt_chunk_buffer(cata_height), "m\n")
    cat("   Note: lidR saute les tuiles existantes\n")
