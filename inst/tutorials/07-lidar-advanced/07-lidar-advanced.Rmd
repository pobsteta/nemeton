...
```{r ex-4-1, exercise=TRUE, exercise.timelimit=600, exercise.setup="ex-4-1-setup"}
library(lidaRtRee)
library(terra)
library(sf)
library(future)
...
process_gaps_tile <- function(chm_path, tile_id, output_dir) {
  ...
  if (all(is.na(values(gap_mask)))) {
    return(list(sf = NULL, mask_file = mask_file))
  }
  ...
  list(sf = gap_sf, mask_file = mask_file)
}
...
gap_mask_vrt <- file.path(data_dir, "gap_mask_complet.vrt")
terra::vrt(mask_files, gap_mask_vrt, overwrite = TRUE)

cat("\n=== Résultats ===\n")
cat("Temps de traitement:", round(elapsed, 1), "s\n")
cat("Tuiles de masques créées:", length(mask_files), "\n")
...
### Exercice 4.2 : Détection des lisières sur le catalogue (parallélisé)
...
```{r ex-4-2, exercise=TRUE, exercise.timelimit=600, exercise.setup="ex-4-2-setup"}
library(lidaRtRee)
library(terra)
library(sf)
library(future)
