...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  if (length(mnt_tiles) > 0) {
    mnt_vrt <- vrt(mnt_tiles, file.path(result, "mnt_complet.vrt"))
    cat("VRT MNT créé:", file.path(result, "mnt_complet.vrt"), "\n")
    cat("  Dimensions:", nrow(mnt_vrt), "x", ncol(mnt_vrt), "pixels\n")
    cat("  Résolution:", res(mnt_vrt)[1], "m\n")
  }

  if (length(mnh_tiles) > 0) {
    mnh_vrt <- vrt(mnh_tiles, file.path(result, "mnh_complet.vrt"))
    cat("VRT MNH créé:", file.path(result, "mnh_complet.vrt"), "\n")
  }

  if (length(met_tiles) > 0) {
    met_vrt <- vrt(met_tiles, file.path(result, "metriques_complet.vrt"))
    cat("VRT Métriques créé:", file.path(result, "metriques_complet.vrt"), "\n")
  }
