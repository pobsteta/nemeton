...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  # 1. Créer l'étape de triangulation (référencée par rasterize et pit_fill)
  tri <- triangulate(filter = keep_first())

  # 2. Créer l'étape CHM (référencée par pit_fill)
  chm <- rasterize(0.5, tri,
  ...
  pipeline_itd <- reader_las() +
    ...
    # Triangulation TIN des premiers retours (base pour le CHM)
    tri +

    # CHM à 0.5m de résolution (haute précision pour ITD)
    chm +

    # Remplissage des puits et NA (améliore la qualité du CHM)
    chm_filled +

    # Détection des maxima locaux (cimes des arbres)
    seeds +

    # Segmentation par croissance de région (délimitation des houppiers)
    region_growing(chm_filled, seeds,
  ...
  if (length(chm_tiles) > 0) {
    chm_vrt <- vrt(chm_tiles, file.path(result_itd, "chm_complet.vrt"), overwrite = TRUE)
    cat("VRT CHM créé:", file.path(result_itd, "chm_complet.vrt"), "\n")
    cat("  Dimensions:", nrow(chm_vrt), "x", ncol(chm_vrt), "pixels\n")
    cat("  Résolution:", res(chm_vrt)[1], "m\n")
  }

  if (length(crown_tiles) > 0) {
    crowns_vrt <- vrt(crown_tiles, file.path(result_itd, "crowns_complet.vrt"), overwrite = TRUE)
    cat("VRT Crowns créé:", file.path(result_itd, "crowns_complet.vrt"), "\n")
  }
