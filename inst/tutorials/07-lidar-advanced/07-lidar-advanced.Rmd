...
laz_dir_normalized <- file.path(data_dir, "laz_normalized")
```

```{r ex-8-1, exercise=TRUE, exercise.timelimit=60, exercise.setup="ex-8-1-setup"}
# === 8.1 : Chargement des modèles et configuration ===
...
# Vérifier le répertoire des tuiles
cat("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
cat("RÉPERTOIRE DES TUILES:\n")
cat(laz_dir_normalized, "\n")
...
if (dir.exists(laz_dir_normalized)) {
  ...
  if (length(laz_files) > 0) {
    ...
    cat("\nFiltre LAScatalog:", filter_cata_height, "\n")
    ...
    lidR::projection(cata_height) <- 2154

    cat("\nLAScatalog créé avec", length(cata_height$filename), "fichiers\n")
    cat("Emprise:", paste(round(sf::st_bbox(cata_height)), collapse = ", "), "\n")
  }
} else {
  cat("  Répertoire non trouvé.\n")
  cat("  NOTE: Pour la cartographie wall-to-wall, vous devez avoir\n")
  cat("        des tuiles LAZ normalisées couvrant la zone d'étude.\n")
}
```
...
# Charger LAZ NORMALISÉS pré-extraits
plots_extracted_norm <- file.path(data_dir, "plots_extracted_norm")
laz_norm_files <- list.files(plots_extracted_norm, pattern = "\\.laz$", full.names = TRUE)
llas_height <- purrr::map(laz_norm_files, lidR::readLAS)
names(llas_height) <- gsub("\\.laz$", "", basename(laz_norm_files))
