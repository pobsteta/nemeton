...
# Répertoire des données du package (inst/extdata après installation)
field_dir <- system.file("extdata", "aba.model", "field", package = "nemeton")
if (field_dir == "") {
  # Fallback pour développement local
  field_dir <- "/home/pascal/Téléchargements/nemeton/inst/extdata/aba.model/field"
}
```
...
# Paramètres d'inventaire (comme dans le tutoriel lidaRtRee)
p_radius <- 15
dbh_min <- 7.5
...
# ==========================================================================
# 1. LISTER LES FICHIERS ARBRES PAR PATTERN MATCHING (comme tutoriel)
# ==========================================================================
cat("--- 1. Chargement des arbres ---\n")

# Pattern: Verc-XX-Y_ArbresTerrain.csv où XX = cluster, Y = placette
# Filtrer sur nos 7 clusters: 02, 05, 06, C1, C2, S4, S7
files_t <- dir(field_dir,
               pattern = "Verc-(02|05|06|C1|C2|S4|S7)-[1-4]_ArbresTerrain\\.csv$",
               full.names = TRUE)

cat("Fichiers trouvés:", length(files_t), "\n")

# Charger avec purrr::map_dfr (équivalent lapply + do.call(rbind))
trees <- files_t |>
  map_dfr(~ {
    df <- read.table(.x, sep = ";", header = TRUE, stringsAsFactors = FALSE)
    # Convertir Rem en character (types mixtes entre fichiers)
    if ("Rem" %in% names(df)) df$Rem <- as.character(df$Rem)
    # Extraire plot_id du nom de fichier (ex: Verc-02-1)
    df$plot_id <- sub("_ArbresTerrain\\.csv$", "", basename(.x))
    df
  }) |>
  dplyr::filter(!is.na(Dia_cm), Dia_cm >= dbh_min)

cat("Arbres chargés:", nrow(trees), "\n")
cat("Placettes:", dplyr::n_distinct(trees$plot_id), "\n")
cat("Colonnes:", paste(names(trees), collapse = ", "), "\n\n")

# ==========================================================================
# 2. CALCULER G, N, D PAR PLACETTE (dplyr)
# ==========================================================================
...
stand_params <- trees |>
  ...
  dplyr::summarise(
    G_m2_ha = sum(pi * (Dia_cm / 200)^2) / plot_area_ha,
    N_ha = dplyr::n() / plot_area_ha,
    D_mean_cm = mean(Dia_cm),
    .groups = "drop"
  )

cat("Placettes avec mesures:", nrow(stand_params), "\n\n")

# ==========================================================================
# 3. AJOUTER LES COORDONNÉES DEPUIS *_PiquetsTerrain.csv
# ==========================================================================
cat("--- 3. Ajout des coordonnées ---\n")

# Pattern pour les fichiers piquets (centres de placettes)
files_p <- dir(field_dir,
               pattern = "Verc-(02|05|06|C1|C2|S4|S7)_PiquetsTerrain\\.csv$",
               full.names = TRUE)

# Charger les coordonnées avec purrr::map_dfr
coords <- files_p |>
  map_dfr(~ {
    df <- read.table(.x, sep = ";", header = TRUE, stringsAsFactors = FALSE)
    df <- df[df$Id %in% c("p1", "p2", "p3", "p4"), c("Id", "X", "Y")]
    cluster_id <- sub("_PiquetsTerrain\\.csv$", "", basename(.x))
    df$plot_id <- paste0(cluster_id, "-", substr(df$Id, 2, 2))
    df[, c("plot_id", "X", "Y")]
  })

# Fusionner et créer objet sf
plots_sf <- stand_params |>
  dplyr::left_join(coords, by = "plot_id") |>
  sf::st_as_sf(coords = c("X", "Y"), crs = 2154)

cat("Placettes géolocalisées:", nrow(plots_sf), "\n\n")

# ==========================================================================
# 4. RÉSUMÉ
# ==========================================================================
cat("--- 4. Résumé des données terrain ---\n\n")

plots_sf |>
  st_drop_geometry() |>
  summarise(
    G_min = min(G_m2_ha, na.rm = TRUE),
    G_max = max(G_m2_ha, na.rm = TRUE),
    G_moy = mean(G_m2_ha, na.rm = TRUE),
    N_min = min(N_ha, na.rm = TRUE),
    N_max = max(N_ha, na.rm = TRUE),
    N_moy = mean(N_ha, na.rm = TRUE),
    D_min = min(D_mean_cm, na.rm = TRUE),
    D_max = max(D_mean_cm, na.rm = TRUE),
    D_moy = mean(D_mean_cm, na.rm = TRUE)
  ) |>
  ...
  mutate(
    Variable = case_when(
      Variable == "G" ~ "G (m²/ha)",
      Variable == "N" ~ "N (tiges/ha)",
      Variable == "D" ~ "D moyen (cm)"
    ),
...
# Répertoire des données du package (inst/extdata après installation)
field_dir <- system.file("extdata", "aba.model", "field", package = "nemeton")
laz_dir <- system.file("extdata", "aba.model", "ALS", "plots.norm.laz", package = "nemeton")
if (field_dir == "") {
  # Fallback pour développement local
  field_dir <- "/home/pascal/Téléchargements/nemeton/inst/extdata/aba.model/field"
  laz_dir <- "/home/pascal/Téléchargements/nemeton/inst/extdata/aba.model/ALS/plots.norm.laz"
}
...
# Lister les fichiers LAZ par pattern matching (7 clusters × 4 placettes)
laz_files <- dir(laz_dir,
                 pattern = "Verc-(02|05|06|C1|C2|S4|S7)-[1-4]\\.laz$",
                 full.names = TRUE)
...
# Charger arbres depuis *_ArbresTerrain.csv par pattern matching
files_t <- dir(field_dir,
               pattern = "Verc-(02|05|06|C1|C2|S4|S7)-[1-4]_ArbresTerrain\\.csv$",
               full.names = TRUE)

stand_params <- files_t |>
  map_dfr(~ {
    df <- read.table(.x, sep = ";", header = TRUE, stringsAsFactors = FALSE)
    df$plot_id <- sub("_ArbresTerrain\\.csv$", "", basename(.x))
    df[, c("Dia_cm", "plot_id")]
  }) |>
  dplyr::filter(!is.na(Dia_cm), Dia_cm >= 7.5) |>
  dplyr::group_by(plot_id) |>
  dplyr::summarise(G_m2_ha = sum(pi * (Dia_cm / 200)^2) / plot_area_ha, .groups = "drop")

# Fusionner terrain + LiDAR
plots_aba <- stand_params |>
  dplyr::inner_join(metrics, by = "plot_id")
...
Le **BABA (Buffered Area-Based Approach)** est une amélioration de l'ABA classique avec plusieurs avantages :

1. **Décorrélation résolution / fenêtre de calcul** : pixels fins (10m) mais métriques calculées sur grandes fenêtres (30m)
2. **Compatibilité placettes terrain** : fenêtre 30×30m (900 m²) proche des placettes de rayon 15m (707 m²)
3. **Lissage spatial + haute résolution** : métriques stables sur grandes fenêtres, cartes à résolution fine
4. **Réduction des effets de bord** : fenêtres chevauchantes → transitions douces entre pixels
5. **Performance lasR** : calcul C++ ultra-rapide, gestion automatique des buffers, parallélisation native

```{r ex-6-3-setup}
# Mêmes données LAZ normalisées que l'exercice 6.2
laz_dir <- system.file("extdata", "aba.model", "ALS", "plots.norm.laz", package = "nemeton")
if (laz_dir == "") {
  laz_dir <- "/home/pascal/Téléchargements/nemeton/inst/extdata/aba.model/ALS/plots.norm.laz"
}
...
cat("AVANTAGES DU BABA vs ABA classique:\n")
cat("─────────────────────────────────────────────────────\n")
...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  # Mêmes 28 placettes que l'exercice 6.2 (7 clusters × 4 placettes)
  laz_files <- dir(laz_dir,
                   pattern = "Verc-(02|05|06|C1|C2|S4|S7)-[1-4]\\.laz$",
                   full.names = TRUE)
  cat("Fichiers LAZ (mêmes que 6.2):", length(laz_files), "\n\n")
  ...
  cat("ABA classique (exercice 6.2):\n")
  cat("  - Métriques calculées par nuage (1 placette = 1 valeur)\n")
  cat("  - clouds_metrics() avec aba_metrics()\n\n")

  cat("BABA (cet exercice):\n")
  cat("  - Métriques rasterisées avec fenêtre glissante\n")
  cat("  - res = c(10, 30) : pixel 10m, fenêtre 30m\n\n")

  # Pipeline BABA
  output_file <- file.path(output_dir, "metriques_baba_28plots.tif")
  ...
  # Exécution
  cat("\n--- Exécution BABA sur 28 placettes ---\n")
  ...
  if (file.exists(output_file)) {
    ...
    cat("\nRésultat BABA:\n")
    cat("  Dimensions:", ncol(baba), "×", nrow(baba), "pixels\n")
    cat("  Résolution:", terra::res(baba)[1], "m\n")
    cat("  Couches:", paste(names(baba), collapse = ", "), "\n")

    # Visualisation comparative
    par(mfrow = c(2, 2))
    plot(baba[[1]], main = "Zmax (m)", col = hcl.colors(50, "Greens", rev = TRUE))
    plot(baba[[2]], main = "Zmean (m)", col = hcl.colors(50, "Greens", rev = TRUE))
    plot(baba[[4]], main = "Zp95 (m)", col = hcl.colors(50, "Greens", rev = TRUE))
    plot(baba[[5]], main = "Couverture > 2m", col = hcl.colors(50, "YlGn", rev = TRUE))

    cat("\nConclusion:\n")
    cat("- BABA produit une carte raster (pixels 10m)\n")
    cat("- ABA produit des valeurs ponctuelles par placette\n")
    cat("- BABA permet la cartographie continue du massif\n")
  }
}
```
