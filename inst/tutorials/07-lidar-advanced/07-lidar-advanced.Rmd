...
### 6.1 Import field inventory data

#### 6.1.1 Tree-level inventory
...
#### 6.1.3 Compute tree coordinates
...
data("quatre_montagnes", package = "lidaRtRee", envir = environment())
plots <- quatre_montagnes[grepl(paste0("^", pattern_zone, "-"), quatre_montagnes$plotId), ]
plots_sf <- sf::st_as_sf(plots, coords = c("X", "Y"), crs = 2154)
plots_sf <- cbind(plots_sf, X = plots$X, Y = plots$Y)

# Répertoire des LAZ normalisés (données utilisateur)
laz_dir_normalized <- "/home/pascal/.local/share/nemeton/tutorial_data/result_laz_normalized"

# Créer un LAScatalog pour extraire les placettes
...
# Extraire les nuages circulaires aux coordonnées des placettes
llas_height <- list()
for (i in 1:nrow(plots)) {
  tryCatch({
    las <- lidR::clip_circle(ctg_normalized, plots$X[i], plots$Y[i], p_radius)
    if (!lidR::is.empty(las)) {
      las$Z[las$Z < 0] <- 0  # Correction hauteurs négatives
      llas_height[[plots$plotId[i]]] <- las
    }
  }, error = function(e) NULL)
}
```

```{r ex-6-5, exercise=TRUE, exercise.timelimit=180, exercise.setup="ex-6-5-setup"}
# === 6.2.1 : Extraction des nuages de points normalisés ===

cat("=== Extraction des nuages depuis les tuiles LiDAR HD ===\n\n")
cat("Répertoire LAZ normalisés:", laz_dir_normalized, "\n\n")

# Informations sur le catalogue
cat("Catalogue LAZ:\n")
cat("  - Nombre de tuiles:", length(ctg_normalized$filename), "\n")
cat("  - CRS: Lambert-93 (EPSG:2154)\n\n")

# Résultat de l'extraction
cat("=== Nuages de points extraits ===\n")
cat("Nombre de placettes:", nrow(plots), "\n")
cat("Nuages extraits:", length(llas_height), "\n\n")

if (length(llas_height) > 0) {
  cat("Premier nuage:", names(llas_height)[1], "\n")
  print(llas_height[[1]])
} else {
  cat("ERREUR: Aucun nuage extrait.\n")
  cat("Vérifiez que les fichiers LAZ existent dans:", laz_dir_normalized, "\n")
}
```

#### 6.2.2 Computation of terrain statistics
...
# Répertoire des LAZ ORIGINAUX (non-normalisés) pour métriques terrain
laz_dir_original <- "/home/pascal/.local/share/nemeton/tutorial_data/lidar_hd/NUALHD_1-0__LAZ_LAMB93_PM_2025-03-25"
...
for (i in 1:nrow(plots)) {
  tryCatch({
    las <- lidR::clip_circle(ctg_original, plots$X[i], plots$Y[i], p_radius)
    if (!lidR::is.empty(las)) {
      llas_original[[plots$plotId[i]]] <- las
    }
  }, error = function(e) NULL)
}
```
...
for (i in seq_along(llas_original)) {
  ...
  plot_row <- which(plots$plotId == plot_name)
...
### 6.3 Check field inventory data

#### 6.3.1 Consistency with the inventory protocol

### Exercice 6.7 : Vérification des données d'inventaire
...
# Import arbres (simplifié - utiliser les données déjà préparées)
# Dans le tutoriel complet on chargerait depuis les CSV
# Ici on simule avec les données disponibles
```

```{r ex-6-7, exercise=TRUE, exercise.timelimit=60, exercise.setup="ex-6-7-setup"}
# === 6.3.1 : Vérification de la cohérence avec le protocole ===
...
#### 6.3.2 Height / diameter allometry
...
#### 6.3.3 Trees positions and remote sensing data

### Exercice 6.9 : Superposition CHM et données terrain
...
# Extraire les nuages circulaires
llas_height <- list()
for (i in 1:nrow(plots)) {
  tryCatch({
    las <- lidR::clip_circle(ctg_normalized, plots$X[i], plots$Y[i], p_radius)
...
# === 6.3.6 : Superposition CHM et positions arbres ===
...
terra::plot(chm,
            ...
            main = paste("CHM placette", plot_test))

# Ajouter le cercle de la placette
plot_coords <- plots[plots$plotId == plot_test, c("X", "Y")]
...
cat("CHM calculé - résolution:", terra::res(chm)[1], "m\n")
cat("Hauteur max:", round(terra::global(chm, "max", na.rm = TRUE)$max, 1), "m\n")
```

### 6.4 Stand level parameters

#### 6.4.1 Compute stand level forest parameters
...
### 6.5 Summary of prepared data

### Exercice 6.11 : Résumé des données préparées
...
# LAZ normalisés pour llas_height
laz_dir_normalized <- "/home/pascal/.local/share/nemeton/tutorial_data/result_laz_normalized"
ctg_normalized <- lidR::readLAScatalog(laz_dir_normalized)
lidR::projection(ctg_normalized) <- 2154
lidR::opt_progress(ctg_normalized) <- FALSE
...
# LAZ originaux pour métriques terrain
laz_dir_original <- "/home/pascal/.local/share/nemeton/tutorial_data/lidar_hd/NUALHD_1-0__LAZ_LAMB93_PM_2025-03-25"
ctg_original <- lidR::readLAScatalog(laz_dir_original)
lidR::projection(ctg_original) <- 2154
lidR::opt_progress(ctg_original) <- FALSE

# Synchroniser
common_ids <- intersect(plots$plotId, names(llas_height))
plots <- plots[plots$plotId %in% common_ids, ]
llas_height <- llas_height[plots$plotId]

# Calculer métriques terrain depuis LAZ originaux
metrics_terrain <- list()
for (i in 1:nrow(plots)) {
  tryCatch({
    las_orig <- lidR::clip_circle(ctg_original, plots$X[i], plots$Y[i], p_radius)
    if (!lidR::is.empty(las_orig)) {
      center_xy <- as.numeric(plots[i, c("X", "Y")])
      metrics_terrain[[plots$plotId[i]]] <- lidaRtRee::terrain_points_metrics(las_orig, center_xy, p_radius)
    }
  }, error = function(e) NULL)
}
...
## Section 7 : Calibration des Modèles de Prédiction
...
cat(paste(names(metrics_points)[1:15], collapse = ", "), "...\n")
```
...
aba_point_metrics_fun <- ~ lidaRtRee::aba_metrics(Z, Intensity, ReturnNumber,
                                                  Classification, 2)
llas_height_plot_extent <- lapply(llas_height, function(x) x)
metrics_points <- lidaRtRee::clouds_metrics(llas_height_plot_extent,
                                            aba_point_metrics_fun)

metrics_trees <- lidaRtRee::clouds_tree_metrics(
  llas_height, plots[, c("X", "Y")], plot_radius, res = aba_res_chm,
  func = function(x) {
    lidaRtRee::std_tree_metrics(x, area_ha = pi * plot_radius^2 / 10000)
  }
)
...
# Combinaison des trois types de métriques
metrics <- cbind(
  metrics_points[plots$plot_id, ],
  metrics_trees[plots$plot_id, ],
  metrics_terrain[plots$plot_id, 1:3]  # altitude, pente, aspect
)
...
print(round(metrics[1:5, c("zmax", "zmean", "zq95", "Tree_meanH", "altitude_mean")], 2))
```
...
metrics_terrain <- mapply(function(x, y) {
  lidaRtRee::terrain_points_metrics(x, plots[y, c("X", "Y")], p_radius)
}, x = llas_height, y = as.list(1:length(llas_height)), SIMPLIFY = FALSE)
metrics_terrain <- do.call(rbind, metrics_terrain)
rownames(metrics_terrain) <- names(llas_height)
...
cat("=== Calibration du modèle pour", variable, "===\n\n")
...
cor_residuals <- round(cor(cbind(
  ...
  metrics_terrain[subsample, 1:3]
)), 2)[1, ]
print(cor_residuals)

# Test de corrélation résidus vs variable observée
cat("\nTest corrélation résidus ~", variable, ":\n")
print(cor.test(model_aba$values$residual, plots[subsample, variable]))
```
...
variable <- "G_m2_ha"
model_aba <- lidaRtRee::aba_build_model(
  plots[, variable], metrics, transform = "boxcox", nmax = 4,
  xy = plots[, c("X", "Y")]
)
```
...
```{r ex-7-8, exercise=TRUE, exercise.timelimit=180, exercise.setup="ex-7-8-setup"}
# === 7.8 : Modèles multiples (G, D, N) ===
...
for (i in c("G_m2_ha", "D_mean_cm", "N_ha")) {
  cat("Calibration de", i, "...\n")
  models_aba[[i]] <- lidaRtRee::aba_build_model(
    plots[, i],
    metrics,
    transform = "boxcox",
    nmax = 4,
...
cat("\n=== RÉSUMÉ DES MODÈLES ===\n\n")
print(round(model_stats, 3))

cat("\n\nINTERPRÉTATION:\n")
cat("- adjR2: R² ajusté du modèle calibré\n")
cat("- CV-R2: R² en validation croisée leave-one-out\n")
cat("- CV-RMSE: erreur quadratique moyenne en CV\n")
cat("- RMSE%: coefficient de variation des erreurs\n")
...
```{r ex-7-9, exercise=TRUE, exercise.timelimit=120, exercise.setup="ex-7-9-setup"}
...
if (!strat %in% names(plots)) {
  ...
  # Création d'une variable de stratification basée sur le cluster
  plots$stratum <- factor(ifelse(grepl("^Verc-(02|05|06)", plots$plotId),
                                  "cluster_A", "cluster_B"))
}
...
for (i in levels(plots[, strat])) {
  ...
  cat("\nStrate", i, ":", length(subsample), "placettes\n")

  if (length(subsample) >= 10) {  # Minimum pour la démonstration
    ...
    cat("  CV-R²:", round(model_aba_stratified[[i]]$stats$`CV-R2`, 3), "\n")
  } else {
    cat("  Trop peu de placettes pour la calibration\n")
  }
}

# Combinaison des modèles stratifiés
if (length(model_aba_stratified) > 1) {
  model_aba_stratified_combined <- lidaRtRee::aba_combine_strata(
    model_aba_stratified, plots$plot_id
  )

  cat("\n=== COMPARAISON ===\n")
  cat("Modèle global CV-R²:", round(model_aba$stats$`CV-R2`, 3), "\n")
  cat("Modèle stratifié CV-R²:",
      round(model_aba_stratified_combined$stats$`CV-R2`, 3), "\n")
}

cat("\nNOTE: Avec 28 placettes, la stratification n'est pas statistiquement robuste.\n")
...
model_aba <- lidaRtRee::aba_build_model(
  plots[, "G_m2_ha"], metrics, transform = "boxcox", nmax = 4,
  xy = plots[, c("X", "Y")]
)

if (requireNamespace("rappdirs", quietly = TRUE)) {
  output_dir <- file.path(rappdirs::user_data_dir("nemeton"), "tutorial_data")
} else {
  output_dir <- file.path(path.expand("~"), "nemeton_tutorial_data")
}
...
cat("  - model_aba: modèle calibré pour G_m2_ha\n")
cat("  - aba_point_metrics_fun: fonction de calcul des métriques\n")
cat("  - aba_res_chm: résolution du MNC\n")
cat("  - class_points: classes de points à conserver\n")
cat("  - h_points: hauteur maximale\n")
cat("  - plot_radius: rayon des placettes\n\n")
...
*Approche surfacique (ABA) - Application des modèles pour la cartographie wall-to-wall*

Ce code présente un workflow pour appliquer les modèles de prédiction calibrés dans la section précédente à l'ensemble de la zone couverte par les données ALS, en utilisant l'approche surfacique (ABA).

### 8.1 Chargement des modèles et configuration du LAScatalog

Les modèles calibrés et les paramètres de configuration sont chargés. Le LAScatalog est configuré avec les filtres appropriés pour le traitement par tuiles.

```{r ex-8-1-setup, message=FALSE, warning=FALSE}
# Paramètres de la section 7
pattern_zone <- "Verc-(02|05|06|C1|C2|S4|S7)"
class_points <- c(0, 1, 2, 3, 4, 5, 12)
h_points <- 60
aba_res_chm <- 0.5
plot_radius <- 15
resolution <- 25  # Résolution de la carte de sortie

# LAZ normalisés (données utilisateur)
laz_dir_normalized <- "/home/pascal/.local/share/nemeton/tutorial_data/result_laz_normalized"

# Reconstruction du modèle (normalement chargé depuis le fichier)
data("quatre_montagnes", package = "lidaRtRee", envir = environment())
plots <- quatre_montagnes[grepl(paste0("^", pattern_zone, "-"), quatre_montagnes$plotId), ]
plots$plot_id <- plots$plotId
...
for (i in 1:nrow(plots)) {
  tryCatch({
    las <- lidR::clip_circle(ctg_normalized, plots$X[i], plots$Y[i], plot_radius)
    if (!lidR::is.empty(las)) {
      las$Z[las$Z < 0] <- 0
      llas_height[[plots$plotId[i]]] <- las
    }
...
for (i in 1:nrow(plots)) {
  tryCatch({
    las_orig <- lidR::clip_circle(ctg_original, plots$X[i], plots$Y[i], plot_radius)
    if (!lidR::is.empty(las_orig)) {
      center_xy <- as.numeric(plots[i, c("X", "Y")])
      metrics_terrain[[plots$plotId[i]]] <- lidaRtRee::terrain_points_metrics(las_orig, center_xy, plot_radius)
    }
  }, error = function(e) NULL)
}
metrics_terrain <- as.data.frame(do.call(rbind, metrics_terrain))
...
cat("=== Configuration pour la cartographie wall-to-wall ===\n\n")

# Répertoire des tuiles normalisées
tiles_dir <- laz_dir_normalized

cat("Répertoire des tuiles:", tiles_dir, "\n")

# Filtre pour le LAScatalog
filter_cata_height <- paste(c("-keep_class", class_points,
                              "-drop_z_above", h_points), collapse = " ")

cat("\nFiltre LAScatalog:\n")
cat(" ", filter_cata_height, "\n")

# Création du LAScatalog
cata_height <- lidR::readALSLAScatalog(tiles_dir,
                                        progress = FALSE,
                                        select = "xyzirnc",
                                        filter = filter_cata_height)

# Configuration de la projection
lidR::projection(cata_height) <- 2154  # Lambert-93

cat("\nLAScatalog créé:\n")
cat("  - Nombre de fichiers:", length(cata_height$filename), "\n")
cat("  - CRS: EPSG:2154 (Lambert-93)\n")
cat("  - Résolution de sortie:", resolution, "m\n\n")

# Afficher l'emprise
cat("Emprise spatiale:\n")
print(sf::st_bbox(cata_height))
```

### 8.2 Calcul des métriques raster - Points

Les métriques du nuage de points sont calculées sur une grille régulière à la résolution spécifiée (25 m).
...
# Charger les placettes pour extraire des nuages de points
data("quatre_montagnes", package = "lidaRtRee", envir = environment())
plots <- quatre_montagnes[grepl(paste0("^", pattern_zone, "-"), quatre_montagnes$plotId), ]
...
# === 8.2 : Métriques raster du nuage de points ===

cat("=== Calcul des métriques points sur grille ===\n\n")
cat("Résolution:", resolution, "m\n")
cat("Surface pixel:", resolution^2, "m²\n\n")

# Extraire des nuages de points depuis le LAScatalog
point_clouds <- list()
for (i in 1:min(4, nrow(plots))) {
  tryCatch({
    las <- lidR::clip_circle(ctg_normalized, plots$X[i], plots$Y[i], p_radius)
    if (!lidR::is.empty(las)) {
      point_clouds[[i]] <- las
...
if (length(point_clouds) > 0) {
  # Fusionner les nuages
  point_cloud <- do.call(rbind, point_clouds)

  # Filtrage
  point_cloud <- lidR::filter_poi(point_cloud,
                                   is.element(Classification, class_points) &
                                     Z <= h_points)
  point_cloud$Z[point_cloud$Z < 0] <- 0

  cat("Nuage de points chargé:\n")
  cat("  - Points:", nrow(point_cloud@data), "\n")
  cat("  - Emprise:", round(diff(range(point_cloud$X))), "x",
      round(diff(range(point_cloud$Y))), "m\n\n")

  # Calcul des métriques pixel_metrics
  cat("Calcul pixel_metrics avec aba_metrics()...\n")
  metrics_points_raster <- lidR::pixel_metrics(point_cloud,
                                                aba_point_metrics_fun,
                                                res = resolution)
  ...
  cat("  - Premières couches:", paste(names(metrics_points_raster)[1:5],
                                      collapse = ", "), "...\n")

  # Visualisation
  par(mfrow = c(2, 2), mar = c(2, 2, 3, 1))
  terra::plot(metrics_points_raster[["zmax"]], main = "Zmax (m)",
              col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
  terra::plot(metrics_points_raster[["zmean"]], main = "Zmean (m)",
              col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
  terra::plot(metrics_points_raster[["zq95"]], main = "Zq95 (m)",
              col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
  terra::plot(metrics_points_raster[["pzabove2"]], main = "Couverture > 2m",
              col = grDevices::hcl.colors(50, "YlGn", rev = TRUE))
  par(mfrow = c(1, 1))
} else {
  cat("Aucun nuage de points extrait depuis:", laz_dir_normalized, "\n")
}
```

### 8.3 Calcul des métriques raster - Arbres

Les métriques basées sur la détection d'arbres sont calculées via segmentation du MNC et agrégation par pixel.
...
# Extraire plusieurs nuages de points
point_clouds <- list()
for (i in 1:min(4, nrow(plots))) {
  tryCatch({
    las <- lidR::clip_circle(ctg_normalized, plots$X[i], plots$Y[i], p_radius)
...
```{r ex-8-3, exercise=TRUE, exercise.timelimit=300, exercise.setup="ex-8-3-setup"}
# === 8.3 : Métriques raster basées sur les arbres ===

cat("=== Segmentation et métriques arbres ===\n\n")

if (exists("point_cloud") && !is.null(point_cloud)) {

  # 1. Créer le MNC
  cat("1. Création du MNC (résolution:", aba_res_chm, "m)...\n")
  chm <- lidR::rasterize_canopy(point_cloud, res = aba_res_chm,
                                 algorithm = lidR::p2r())
  chm[is.na(chm) | chm < 0] <- 0

  cat("   Dimensions:", terra::ncol(chm), "x", terra::nrow(chm), "\n\n")
  ...
  cat("Métriques arbres calculées:\n")
  ...
  cat("Nuage de points non disponible\n")
}
```

### 8.4 Calcul des métriques raster - Terrain

Les métriques de terrain (altitude, pente, aspect) sont calculées à partir des points classifiés sol.
...
# LAZ originaux (non-normalisés) pour les métriques de terrain
laz_dir_original <- "/home/pascal/.local/share/nemeton/tutorial_data/lidar_hd/NUALHD_1-0__LAZ_LAMB93_PM_2025-03-25"
...
# Extraire plusieurs nuages de points originaux
point_clouds <- list()
for (i in 1:min(4, nrow(plots))) {
  tryCatch({
    las <- lidR::clip_circle(ctg_original, plots$X[i], plots$Y[i], p_radius)
...
```{r ex-8-4, exercise=TRUE, exercise.timelimit=120, exercise.setup="ex-8-4-setup"}
# === 8.4 : Métriques terrain ===
...
if (exists("point_cloud") && !is.null(point_cloud)) {
  ...
  # Extraction des points sol (classes 2 et 9)
  point_cloud_ground <- lidR::filter_poi(point_cloud, Classification %in% c(2, 9))

  cat("Points sol:", nrow(point_cloud_ground@data), "\n\n")

  if (nrow(point_cloud_ground@data) > 100) {
    # Calcul des métriques terrain
    cat("Calcul pixel_metrics terrain...\n")
    metrics_terrain_raster <- lidR::pixel_metrics(point_cloud_ground,
                                                   ~ f_terrain(X, Y, Z),
                                                   res = resolution)

    # Retirer adjR2_plane si présent
    ...
    cat("Trop peu de points sol pour le calcul\n")
  }
} else {
  cat("Nuage de points non disponible\n")
}
...
La fonction `aba_predict()` applique le modèle calibré aux métriques raster pour produire une carte de prédiction.
...
# Données quatre_montagnes pour le modèle
data("quatre_montagnes", package = "lidaRtRee", envir = environment())
plots <- quatre_montagnes[grepl(paste0("^", pattern_zone, "-"), quatre_montagnes$plotId), ]
plots$plot_id <- plots$plotId
...
metrics_trees <- lidaRtRee::clouds_tree_metrics(
  llas_height, plots[, c("X", "Y")], p_radius, res = aba_res_chm,
  func = function(x) {
    lidaRtRee::std_tree_metrics(x, area_ha = pi * p_radius^2 / 10000)
  }
)
...
# Créer métriques raster pour la prédiction
point_clouds <- list()
for (i in 1:min(4, nrow(plots))) {
  tryCatch({
    las <- lidR::clip_circle(ctg_normalized, plots$X[i], plots$Y[i], p_radius)
...
metrics_points_raster <- lidR::pixel_metrics(point_cloud, aba_point_metrics_fun,
                                              res = resolution)

chm <- lidR::rasterize_canopy(point_cloud, res = aba_res_chm, algorithm = lidR::p2r())
chm[is.na(chm) | chm < 0] <- 0
segms <- lidaRtRee::tree_segmentation(chm)
trees <- lidaRtRee::tree_extraction(segms)

metrics_trees_raster <- lidaRtRee::raster_metrics(
  trees, res = resolution,
...
point_cloud_ground <- lidR::filter_poi(point_cloud, Classification %in% c(2, 9))
metrics_terrain_raster <- lidR::pixel_metrics(point_cloud_ground, ~ f_terrain(X, Y, Z),
                                               res = resolution)
if ("adjR2_plane" %in% names(metrics_terrain_raster)) {
  metrics_terrain_raster <- terra::subset(metrics_terrain_raster,
    which(names(metrics_terrain_raster) != "adjR2_plane"))
}
...
cat("=== Application du modèle ABA ===\n\n")

cat("Modèle utilisé:\n")
print(model_aba$model)
cat("\n")

cat("Métriques disponibles:", terra::nlyr(metrics_map), "couches\n")
...
cat("  - Mean:", round(terra::global(prediction_map, "mean", na.rm = TRUE)[[1]], 1), "m²/ha\n\n")
...
llas_height <- lapply(llas_height, function(x)
  lidR::filter_poi(x, is.element(Classification, class_points) & Z <= h_points))
llas_height <- lapply(llas_height, function(x) { x$Z[x$Z < 0] <- 0; x })

aba_point_metrics_fun <- ~ lidaRtRee::aba_metrics(Z, Intensity, ReturnNumber,
                                                  Classification, 2)
metrics_points <- lidaRtRee::clouds_metrics(llas_height, aba_point_metrics_fun)
metrics_trees <- lidaRtRee::clouds_tree_metrics(llas_height, plots[, c("X", "Y")], 15,
  res = aba_res_chm, func = function(x) lidaRtRee::std_tree_metrics(x, pi * 15^2 / 10000))
metrics_terrain <- mapply(function(x, y)
  lidaRtRee::terrain_points_metrics(x, plots[y, c("X", "Y")], 15),
  x = llas_height, y = as.list(1:length(llas_height)), SIMPLIFY = FALSE)
metrics_terrain <- do.call(rbind, metrics_terrain)
rownames(metrics_terrain) <- names(llas_height)

metrics <- cbind(metrics_points[plots$plot_id, ], metrics_trees[plots$plot_id, ],
                 metrics_terrain[plots$plot_id, 1:3])
model_aba <- lidaRtRee::aba_build_model(plots[, "G_m2_ha"], metrics,
  transform = "boxcox", nmax = 4, xy = plots[, c("X", "Y")])

point_cloud <- do.call(rbind, lapply(laz_files[1:min(4, length(laz_files))], lidR::readLAS))
point_cloud <- lidR::filter_poi(point_cloud, is.element(Classification, class_points) & Z <= h_points)
point_cloud$Z[point_cloud$Z < 0] <- 0

metrics_points_raster <- lidR::pixel_metrics(point_cloud, aba_point_metrics_fun, res = resolution)
chm <- lidR::rasterize_canopy(point_cloud, res = aba_res_chm, algorithm = lidR::p2r())
chm[is.na(chm) | chm < 0] <- 0
segms <- lidaRtRee::tree_segmentation(chm)
trees <- lidaRtRee::tree_extraction(segms)
metrics_trees_raster <- lidaRtRee::raster_metrics(trees, res = resolution,
  fun = function(x) lidaRtRee::std_tree_metrics(x, resolution^2 / 10000), output = "raster")
metrics_trees_raster[!is.finite(metrics_trees_raster)] <- 0

f_terrain <- function(x, y, z) as.list(lidaRtRee::terrain_points_metrics(data.frame(x, y, z)))
point_cloud_ground <- lidR::filter_poi(point_cloud, Classification %in% c(2, 9))
metrics_terrain_raster <- lidR::pixel_metrics(point_cloud_ground, ~ f_terrain(X, Y, Z), res = resolution)
if ("adjR2_plane" %in% names(metrics_terrain_raster))
  metrics_terrain_raster <- terra::subset(metrics_terrain_raster, which(names(metrics_terrain_raster) != "adjR2_plane"))
...
if ("pzabove2" %in% names(metrics_map)) {
  ...
  cat("  - Pixels forêt:", sum(terra::values(forest_mask), na.rm = TRUE), "\n")
  cat("  - Pixels total:", terra::ncell(forest_mask), "\n\n")
} else {
  forest_mask <- NULL
...
cat("Après nettoyage:\n")
cat("  - Pixels valides:", sum(!is.na(terra::values(prediction_clean))), "\n")
...
terra::plot(prediction_map, main = "Avant nettoyage",
            col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
terra::plot(prediction_clean, main = "Après nettoyage",
            col = grDevices::hcl.colors(50, "Greens", rev = TRUE))
par(mfrow = c(1, 1))
```
...
Les cartes de prédiction sont exportées au format GeoTIFF et les statistiques de synthèse sont calculées.
...
metrics_trees <- lidaRtRee::clouds_tree_metrics(llas_height, plots[, c("X", "Y")], 15,
  res = aba_res_chm, func = function(x) lidaRtRee::std_tree_metrics(x, pi * 15^2 / 10000))
metrics_terrain <- mapply(function(x, y) lidaRtRee::terrain_points_metrics(x, plots[y, c("X", "Y")], 15),
  x = llas_height, y = as.list(1:length(llas_height)), SIMPLIFY = FALSE)
metrics_terrain <- do.call(rbind, metrics_terrain)
rownames(metrics_terrain) <- names(llas_height)
metrics <- cbind(metrics_points[plots$plot_id, ], metrics_trees[plots$plot_id, ],
...
cat("  - CV-R²:", round(model_aba$stats$`CV-R2`, 3), "\n")
cat("  - CV-RMSE:", round(model_aba$stats$`CV-RMSE`, 2), "m²/ha\n")
cat("  - RMSE%:", round(model_aba$stats$`RMSE%`, 1), "%\n\n")
...
```{r ex-9-1-setup}
# LAZ normalisés (données utilisateur)
laz_dir <- "/home/pascal/.local/share/nemeton/tutorial_data/result_laz_normalized"
