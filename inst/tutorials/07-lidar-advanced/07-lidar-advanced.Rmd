...
if (length(fichiers_laz) == 0) {
  ...
  cat("- chm_*.tif : tuiles CHM\n")
  cat("- c_filled_*.tif : tuiles CHM rempli\n")
...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  # 2. CHM à 0.5m de résolution (lasR:: pour éviter conflit avec terra)
  chm <- lasR::rasterize(0.5, del)

  # 3. Remplissage des puits
  chm_filled <- lasR::pit_fill(chm)

  # 4. Détection des cimes (fenêtre de 3m, hauteur min 5m)
  seeds <- lasR::local_maximum_raster(chm_filled, ws = 3, min_height = 5)

  # 5. Segmentation par croissance de région
  crowns <- lasR::region_growing(chm_filled, seeds)

  # 6. Callback pour extraction des métriques
  tree_metrics <- lasR::callback(
  ...
  # Pipeline complet
  pipeline_itd <- lasR::reader_las() + del + chm + chm_filled + seeds + crowns + tree_metrics
  ...
  if (!is.null(ctg) && length(fichiers_laz) > 0) {
    cat("\n=== Exécution sur", length(fichiers_laz), "dalles LiDAR ===\n")
    cat("Stratégie: concurrent_files(", N_CORES, ")\n\n")
    ...
    ans <- lasR::exec(pipeline_itd, on = ctg,
                      ...
                      ncores = lasR::concurrent_files(N_CORES))
    ...
    # ==========================================================================
    cat("\n=== Fusion des métriques d'arbres ===\n")
    gpkg_files <- list.files(result_dir, pattern = "tree_metrics.*\\.gpkg$",
                             full.names = TRUE)
    cat("Fichiers générés:", length(gpkg_files), "\n")

    if (length(gpkg_files) > 0) {
      # Fusionner tous les gpkg en un seul
      trees_list <- lapply(gpkg_files, sf::st_read, quiet = TRUE)
      trees_all <- do.call(rbind, trees_list)
      sf::st_write(trees_all, file.path(data_dir, "tree_metrics_complet.gpkg"),
                   delete_dsn = TRUE, quiet = TRUE)
      cat("Métriques fusionnées:", file.path(data_dir, "tree_metrics_complet.gpkg"), "\n")

      cat("\n=== Résultats : Métriques des arbres ===\n")
      cat("Nombre d'arbres détectés:", nrow(trees_all), "\n")
      ...
      cat("\n=== Premiers arbres ===\n")
      print(head(sf::st_drop_geometry(trees_all[, c("id", "height", "dom_radius",
                                                     "surface", "volume")])))
    }
  } else {
    cat("\nAucun fichier LAZ trouvé. Testez avec vos propres données :\n")
    cat("ctg <- readLAScatalog('chemin/vers/fichiers/*.laz')\n")
    cat("ans <- lasR::exec(pipeline_itd, on = ctg,\n")
    cat("                  with = list(tree_metrics = list(\n")
    cat("                    chm = chm_filled, crowns = crowns, seeds = seeds)),\n")
    cat("                  ncores = lasR::concurrent_files(4))\n")
  }
}
```
