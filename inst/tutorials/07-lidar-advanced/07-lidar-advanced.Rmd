...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  pipeline_complet <- reader_las() +
    ...
    rasterize(res = 30,
              ...
              ofile = file.path(result, "lasr_metriques_30m_*.tif"))
  ...
  if (length(mnh_tiles) > 0) {
    mnh_vrt <- vrt(mnh_tiles, file.path(data_dir, "mnh_complet.vrt"))
    cat("VRT MNH créé:", file.path(data_dir, "mnh_complet.vrt"), "\n")
  }

  if (length(met_tiles) > 0) {
    met_vrt <- vrt(met_tiles, file.path(data_dir, "metriques_complet.vrt"))
    cat("VRT Métriques créé:", file.path(data_dir, "metriques_complet.vrt"), "\n")
  }
  ...
  writeRaster(mnt_merge, file.path(data_dir, "mnt_complet.tif"))
  ...
  writeRaster(mnh_merge, file.path(data_dir, "mnh_complet.tif"))
  cat("MNH complet écrit:", file.path(data_dir, "mnh_complet.tif"), "\n")
  
  cat("\n=== Fusion physique des métriques (optionnel) ===\n")
  met_merge <- terra::merge(sprc(met_tiles))
  writeRaster(met_merge, file.path(data_dir, "met_complet.tif"))
  cat("MET complet écrit:", file.path(data_dir, "met_complet.tif"), "\n")
...
cat("1. triangulate()         - Maillage des premiers retours\n")
cat("2. rasterize()           - CHM à 0.5m de résolution\n")
cat("3. pit_fill()            - Remplissage des trous\n")
cat("4. local_maximum_raster()- Détection des cimes\n")
cat("5. region_growing()      - Segmentation des houppiers\n\n")
...
if (!requireNamespace("lasR", quietly = TRUE)) {
  ...
  if (!dir.exists(result_itd)) dir.create(result_itd, recursive = TRUE)

  # ==========================================================================
  # PIPELINE ITD lasR (basé sur le tutoriel officiel)
  # ==========================================================================

  # 1. Triangulation des premiers retours (base pour le CHM)
  del <- triangulate(filter = keep_first())

  # 2. CHM à 0.5m de résolution (haute précision pour ITD)
  chm <- rasterize(0.5, del,
                   ofile = file.path(result_itd, "chm_*.tif"))

  # 3. Remplissage des puits et NA (améliore la qualité du CHM)
  chm_filled <- pit_fill(chm)

  # 4. Détection des maxima locaux (cimes des arbres)
  ...
  # Utilise le CHM rempli et les seeds comme points de départ
  crowns <- region_growing(chm_filled, seeds,
                           ofile = file.path(result_itd, "crowns_*.tif"))

  # Pipeline complet
  pipeline_itd <- del + chm + chm_filled + seeds + crowns
  ...
  # Exécution parallèle
  ans <- exec(pipeline_itd, on = fichiers_laz,
              ncores = concurrent_files(N_CORES))

  print(ans)

  # ==========================================================================
  # FUSION DES RÉSULTATS EN VRT
  # ==========================================================================
  cat("\n=== Création des rasters virtuels ===\n")
  ...
  if (length(chm_tiles) > 0) {
    chm_vrt <- vrt(chm_tiles, file.path(result_itd, "chm_complet.vrt"))
    cat("CHM VRT créé:", nrow(chm_vrt), "x", ncol(chm_vrt), "pixels\n")
  }

  if (length(crown_tiles) > 0) {
    crowns_vrt <- vrt(crown_tiles, file.path(result_itd, "crowns_complet.vrt"))
    cat("Crowns VRT créé:", nrow(crowns_vrt), "x", ncol(crowns_vrt), "pixels\n")
  }
  ...
  if (length(seed_files) > 0) {
    ...
    sf::st_write(seeds_all, file.path(result_itd, "seeds_complet.gpkg"),
  ...
  # Visualisation du CHM avec cimes
  if (exists("chm_vrt") && exists("seeds_all")) {
    par(mfrow = c(1, 2))
    plot(chm_vrt, main = "CHM (0.5m)", col = viridis::viridis(50))
    plot(crowns_vrt %% 8, main = "Segments", col = rainbow(8), legend = FALSE)
  }
}
```
