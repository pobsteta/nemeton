...
plots_extracted_norm <- "/home/pascal/.local/share/nemeton/tutorial_data/plots_extracted_norm"
plots_extracted_orig <- "/home/pascal/.local/share/nemeton/tutorial_data/plots_extracted_orig"
...
for (i in seq_along(llas_original)) {
  ...
  if (length(plot_row) > 0) {
    tryCatch({
      center_xy <- as.numeric(plots[plot_row, c("X", "Y")])
      metrics_terrain[[plot_name]] <- lidaRtRee::terrain_points_metrics(llas_original[[i]], center_xy, plot_radius)
    }, error = function(e) NULL)
  }
}
metrics_terrain <- as.data.frame(do.call(rbind, metrics_terrain))
...
# Utiliser les 4 premiers nuages pré-extraits
point_clouds <- llas_height[1:min(4, length(llas_height))]
...
# Utiliser les 4 premiers nuages
point_clouds <- llas_height[1:min(4, length(llas_height))]
...
# Utiliser les 4 premiers nuages
point_clouds <- llas_original[1:min(4, length(llas_original))]
...
point_cloud <- do.call(rbind, llas_height[1:min(4, length(llas_height))])
