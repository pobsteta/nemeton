...
# === Calcul de G, N, D par essence ===
cat("\n=== Statistiques par essence ===\n")

# Surface de la placette (m²) et facteur de conversion vers hectare
plot_area_m2 <- pi * p_radius^2
ha_factor <- 10000 / plot_area_m2

# Calculer G, N, D par essence (toutes placettes confondues)
stats_by_species <- aggregate(
  diameter_cm ~ species,
  data = trees,
  FUN = function(d) {
    c(
      G_m2_ha = sum(pi * (d/200)^2) * ha_factor,  # surface terrière
      N_ha = length(d) * ha_factor,                # densité
      D_mean_cm = mean(d)                          # diamètre moyen
    )
  }
)
...
stats_sp <- data.frame(
  species = stats_by_species$species,
  G_m2_ha = stats_by_species$diameter_cm[, "G_m2_ha"],
  N_ha = stats_by_species$diameter_cm[, "N_ha"],
  D_mean_cm = stats_by_species$diameter_cm[, "D_mean_cm"]
)

print(stats_sp)

# Scatterplot matriciel G, N, D coloré par essence
cat("\n=== Scatterplot G, N, D par essence ===\n")
sp_colors <- rainbow(nrow(stats_sp))
pairs(stats_sp[, c("G_m2_ha", "N_ha", "D_mean_cm")],
      pch = 19, cex = 1.5,
      col = sp_colors,
      main = "Relations G, N, D par essence",
      labels = c("G (m²/ha)", "N (tiges/ha)", "D moyen (cm)"))

# Légende séparée
plot.new()
legend("center", legend = stats_sp$species,
       col = sp_colors, pch = 19, cex = 0.9,
       title = "Essences", ncol = 3)
```
