...
## Section 4 : Indicateurs de risque (R1, R2, R3)
...
- data.gouv.fr : Tableaux de chasse départementaux (OFB/ONCFS)
- Espèces : Chevreuil (50%), Cerf (35%), Sanglier (15%)
...
```{r ex-4-4, exercise=TRUE, exercise.lines=150, exercise.setup="ex-4-4-setup", exercise.timelimit=900}
# === PRESSION GIBIER (R4) avec données de chasse ===
...
parcelles <- st_read(parcelles_path, quiet = TRUE)
cat("=== Indicateur R4 : Pression Gibier ===\n\n")

# ============================================================
# 1. TÉLÉCHARGEMENT DES DONNÉES DE CHASSE (data.gouv.fr)
# ============================================================

cat("--- 1. Données de chasse départementales ---\n")

pressure_data <- NULL

if (requireNamespace("nemeton", quietly = TRUE)) {
  tryCatch({
    # Utiliser les fonctions nemeton
    pressure_data <- nemeton::compute_game_pressure_index()
    cat("Indice de pression calculé pour", nrow(pressure_data), "départements\n")

    # Afficher les 5 départements avec la plus forte pression
    cat("\nTop 5 départements (pression gibier) :\n")
    top5 <- head(pressure_data[order(-pressure_data$pressure_index), ], 5)
    for (i in seq_len(nrow(top5))) {
      cat(sprintf("  %s (%s): %.1f\n",
                  top5$nom_dept[i], top5$code_dept[i], top5$pressure_index[i]))
    }
  }, error = function(e) {
...
# ============================================================
# 2. CALCUL DE LA PALATABILITÉ PAR PARCELLE
# ============================================================

cat("\n--- 2. Palatabilité des essences ---\n")

# Table de palatabilité (appétence pour le gibier)
      ...
      if (nrow(inter) > 0) {
        ...
        for (j in seq_len(nrow(palatabilite))) {
          if (grepl(palatabilite$pattern[j], essence)) {
            parcelles$R4_palatabilite[i] <- palatabilite$score[j]
            break
          }
        }
      }
...
# Utiliser les métriques LiDAR si disponibles
...
if (file.exists(lidar_path)) {
  ...
  if ("zmean" %in% names(lidar)) {
    # Joindre par géométrie ou ID
    lidar_heights <- st_drop_geometry(lidar)[, c("zmean")]
    # Vulnérabilité: <2m = 100, 2-10m = décroissant, >10m = 0
...
parcelles$R4_lisiere <- sapply(seq_len(nrow(parcelles)), function(i) {
  ...
  if (!is.null(inner) && !st_is_empty(inner)) {
    ...
    100  # Petite parcelle entièrement en lisière
  }
})
...
# ============================================================
# 5. DENSITÉ GIBIER PAR DÉPARTEMENT
# ============================================================

cat("\n--- 5. Densité gibier locale ---\n")

parcelles$R4_densite <- 50  # Valeur par défaut

if (!is.null(pressure_data) && nrow(pressure_data) > 0) {
  # Déterminer le département de chaque parcelle
  # (simplifié: utiliser le centroïde)

  if (requireNamespace("happign", quietly = TRUE)) {
    tryCatch({
      # Télécharger les limites départementales
      ...
      if (!is.null(depts) && nrow(depts) > 0) {
        ...
        # Identifier le code département pour chaque parcelle
        dept_col <- intersect(names(depts), c("code_insee", "insee_dep", "code_dept"))
        if (length(dept_col) > 0) {
          centroids <- st_centroid(parcelles)
          join <- st_join(centroids, depts[, dept_col[1]])
          parcelles$code_dept <- join[[dept_col[1]]]

          # Joindre l'indice de pression
          for (i in seq_len(nrow(parcelles))) {
            dept <- parcelles$code_dept[i]
            if (!is.na(dept)) {
              idx <- which(pressure_data$code_dept == dept)
              if (length(idx) > 0) {
                parcelles$R4_densite[i] <- pressure_data$pressure_index[idx[1]]
              }
            }
          }
          cat("Densité gibier assignée depuis tableaux de chasse\n")
        }
      }
    }, error = function(e) {
...
```{r ex-6-1, exercise=TRUE, exercise.lines=140, exercise.setup="ex-6-1-setup", exercise.timelimit=900}
...
if (file.exists(mnt_path) && file.exists(parcelles_path)) {
  ...
  if (requireNamespace("climateR", quietly = TRUE) && requireNamespace("SPEI", quietly = TRUE)) {
    ...
    # Télécharger données climatiques (TerraClimate ou PRISM)
  ...
  if (requireNamespace("geodata", quietly = TRUE)) {
    ...
    # Télécharger les données SoilGrids pour la zone
    bbox_wgs84 <- st_bbox(st_transform(parcelles, 4326))

    soil_data <- tryCatch({
      # Variables SoilGrids disponibles:
      # "clay", "sand", "silt", "cfvo" (coarse fragments)
      # "nitrogen", "phh2o", "soc" (soil organic carbon)
      # "bdod" (bulk density), "cec", "ocd" (organic carbon density)

      # pH du sol (0-30cm)
      ph <- soil_world(var = "phh2o", depth = 5, path = tempdir())
      ph_crop <- crop(ph, ext(bbox_wgs84))

      # Carbone organique (fertilité)
      soc <- soil_world(var = "soc", depth = 5, path = tempdir())
      soc_crop <- crop(soc, ext(bbox_wgs84))

      # Texture - argile (rétention eau)
      clay <- soil_world(var = "clay", depth = 5, path = tempdir())
...
```{r ex-7-2, exercise=TRUE, exercise.lines=40, exercise.setup="ex-7-2-setup"}
...
indicateurs <- c("W1", "W2", "W3", "R1", "R2", "R3", "S1", "S2", "S3", "P2", "F1")
