...
if (file.exists(mnt_path) && file.exists(parcelles_path)) {
  ...
  output_path <- file.path(data_dir, "metriques_erosion.gpkg")
...
if (file.exists(mnt_path)) {
  ...
  if (file.exists(parcelles_path)) {
    ...
    output_path <- file.path(data_dir, "metriques_twi.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
if (file.exists(parcelles_path)) {
  ...
  if (file.exists(cours_eau_path)) {
    ...
    if (nrow(cours_eau) > 0) {
      ...
      # Sauvegarder
      output_path <- file.path(data_dir, "metriques_densite_hydro.gpkg")
      st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
      st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
if (file.exists(parcelles_path)) {
  ...
  if (file.exists(twi_path)) {
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_couverture_zone_humide.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
if (file.exists(mnt_path) && file.exists(parcelles_path) && file.exists(bdforet_path)) {
  ...
  if (requireNamespace("fireexposuR", quietly = TRUE)) {
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_exposition_feu.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
if (file.exists(mnt_path) && file.exists(parcelles_path)) {
  ...
  # Sauvegarder
  output_path <- file.path(data_dir, "metriques_exposition_tempete.gpkg")
  st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
  st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
if (file.exists(parcelles_path)) {
  ...
  if (file.exists(mnt_path)) {
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_risque_secheresse.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
# Sauvegarder
output_path <- file.path(data_dir, "metriques_pression_gibier.gpkg")
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
if (file.exists(parcelles_path) && file.exists(mnt_path)) {
  ...
  if (file.exists(routes_path)) {
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_access_route.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
    cat("\n=== Métriques accessibilité route exportées ===\n")
    cat("Fichier:", output_path, "\n")
    cat("Parcelles:", nrow(parcelles), " lignes et ", ncol(parcelles), " colonnes\n\n")
  } else {
...
if (file.exists(parcelles_path) && file.exists(mnt_path)) {
  ...
  if (file.exists(batiments_path)) {
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_distance_batiment.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
    cat("\n=== Métriques distance au batiment exportées ===\n")
    cat("Fichier:", output_path, "\n")
    cat("Parcelles:", nrow(parcelles), " lignes et ", ncol(parcelles), " colonnes\n\n")
  } else {
...
if (file.exists(parcelles_path) && file.exists(mnt_path)) {
  ...
  if (file.exists(sentiers_path)) {
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_densite_sentiers.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
    cat("\n=== Métriques densité sentiers exportées ===\n")
    cat("Fichier:", output_path, "\n")
    cat("Parcelles:", nrow(parcelles), " lignes et ", ncol(parcelles), " colonnes\n\n")
  } else {
...
## Section 6 : Fertilité de station (P2)

### Exercice 6.1 : Indice de fertilité station (P2)
...
```{r ex-6-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=160, exercise.setup="ex-6-1-setup", exercise.timelimit=900}
...
if (file.exists(mnt_path) && file.exists(parcelles_path)) {
  ...
  # Sauvegarder
  output_path <- file.path(data_dir, "metriques_fertilite_station.gpkg")
  st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
  st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
  cat("\n=== Métriques fertilité station exportées ===\n")
  cat("Fichier:", output_path, "\n")
  cat("Parcelles:", nrow(parcelles), " lignes et ", ncol(parcelles), " colonnes\n\n")
...
```{r ex-7-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=60, exercise.setup="ex-7-1-setup"}
...
```{r ex-7-2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=65, exercise.setup="ex-7-2-setup"}
...
# Calculé en exercice 4.4 et sauvegardé dans metriques_pression_gibier.gpkg
r4_path <- file.path(data_dir, "metriques_pression_gibier.gpkg")
...
```{r ex-7-3, exercise=TRUE, exercise.eval=FALSE, exercise.lines=55, exercise.setup="ex-7-3-setup"}
...
### Exercice 7.4 : Familles P (Production) et F (Sol)
...
```{r ex-7-4, exercise=TRUE, exercise.eval=FALSE, exercise.lines=45, exercise.setup="ex-7-4-setup"}
# === FAMILLES P (PRODUCTION) et F (SOL) ===
output_path <- file.path(data_dir, "indicateurs_terrain.gpkg")
parcelles <- st_read(output_path, quiet = TRUE)
...
# P2 : Fertilité station (Section 6)
# Indice composite : pente + aspect + TPI + TWI + climat
...
if (file.exists(fertilite_path)) {
  ...
  cat("P2 (fertilité station):", round(mean(parcelles$P2, na.rm = TRUE), 3), "\n")
  cat("   Score 0-1 : 0-0.2=contraignant, 0.2-0.4=faible,\n")
  cat("              0.4-0.6=moyen, 0.6-0.8=bon, 0.8-1=excellent\n\n")
}
...
# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamilles P/F sauvegardées:", output_path, "\n")
```
...
```{r ex-7-5, exercise=TRUE, exercise.eval=FALSE, exercise.lines=70, exercise.setup="ex-7-5-setup"}
# === SYNTHÈSE FINALE - TOUS LES INDICATEURS TERRAIN ===
output_path <- file.path(data_dir, "indicateurs_terrain.gpkg")
parcelles <- st_read(output_path, quiet = TRUE)

cat("╔══════════════════════════════════════════════════════════════╗\n")
cat("║          SYNTHÈSE INDICATEURS TERRAIN (Tutorial 03)         ║\n")
cat("╚══════════════════════════════════════════════════════════════╝\n\n")

cat("Fichier:", output_path, "\n")
cat("Parcelles:", nrow(parcelles), "\n\n")

# Définition des indicateurs avec leurs unités et légendes
indicateurs_info <- list(
  W1 = list(nom = "Densité hydro", unite = "m/ha", legende = "0=aucun, >50=dense"),
  W2 = list(nom = "Zones humides", unite = "%", legende = "% surface TWI>12"),
  W3 = list(nom = "TWI moyen", unite = "", legende = "<6=sec, 9-12=moyen, >12=humide"),
  R1 = list(nom = "Risque feu", unite = "0-1", legende = "0=faible, 1=fort"),
  R2 = list(nom = "Risque tempête", unite = "0-1", legende = "0=abrité, 1=exposé"),
  R3 = list(nom = "Risque sécheresse", unite = "0-1", legende = "0=humide, 1=sec"),
  R4 = list(nom = "Pression gibier", unite = "0-1", legende = "0=faible, 1=forte"),
  S1 = list(nom = "Dist routes", unite = "m", legende = "<100=accessible, >500=isolé"),
  S2 = list(nom = "Dist bâtiments", unite = "m", legende = "<200=proche, >1000=rural"),
  S3 = list(nom = "Densité sentiers", unite = "m/ha", legende = "0=aucun, >20=récréatif"),
  P2 = list(nom = "Fertilité station", unite = "0-1", legende = "0=contraignant, 1=fertile"),
  F1 = list(nom = "Érosion RUSLE", unite = "0-1", legende = "0=faible, 1=fort risque")
)

cat("┌────────┬──────────────────┬─────────┬─────────┬─────────┬────────────────────────┐\n")
cat("│ Indic. │ Description      │   Min   │   Moy   │   Max   │ Interprétation         │\n")
cat("├────────┼──────────────────┼─────────┼─────────┼─────────┼────────────────────────┤\n")

for (ind in names(indicateurs_info)) {
  if (ind %in% names(parcelles)) {
    info <- indicateurs_info[[ind]]
    vals <- parcelles[[ind]]
    cat(sprintf("│ %-6s │ %-16s │ %7.2f │ %7.2f │ %7.2f │ %-22s │\n",
                ind, substr(info$nom, 1, 16),
                min(vals, na.rm = TRUE),
                mean(vals, na.rm = TRUE),
                max(vals, na.rm = TRUE),
                substr(info$legende, 1, 22)))
  }
}
...
cat("\nTotal:", sum(names(indicateurs_info) %in% names(parcelles)), "/12 indicateurs calculés\n")
```
...
| **P** | P2 | Fertilité station | MNT + climateR | 0-1 | pente+TWI+aspect+TPI+climat |
...
|---------|---------|-------------|
| `indicateurs_terrain.gpkg` | Synthèse complète | W1-3, R1-4, S1-3, P2, F1 |
| `metriques_twi.gpkg` | Humidité topographique | W3 |
| `metriques_pression_gibier.gpkg` | Pression gibier détaillée | R4 + composantes |
| `metriques_erosion.gpkg` | Érosion RUSLE | F1 |
| `metriques_fertilite_station.gpkg` | Fertilité station | P2 |
| `twi.tif` | Raster TWI | - |
| `erosion_f1.tif` | Raster érosion RUSLE | - |
| `fertilite_station.tif` | Raster fertilité | - |
