...
# R4 : Pression gibier (score 0-1)
game_path <- file.path(data_dir, "game_pressure.tif")
if (file.exists(game_path)) {
  parcelles$R4 <- extract_mean(rast(game_path), parcelles)
  cat("R4 (pression gibier):", round(mean(parcelles$R4, na.rm = TRUE), 3), "\n")
  cat("   Score 0-1 (0=faible, 1=forte) - données chasse\n")
} else {
  # Valeur par défaut si pas de données
  parcelles$R4 <- 0.5
  cat("R4 (pression gibier): 0.5 (valeur par défaut)\n")
  cat("   Données chasse non disponibles\n")
}
