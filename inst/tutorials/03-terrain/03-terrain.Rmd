...
# === SYNTHÈSE FINALE ===
# Consolidation de tous les indicateurs terrain dans parcelles.gpkg

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== CONSOLIDATION INDICATEURS TERRAIN ===\n\n")

# Fusionner les colonnes des fichiers metriques_*.gpkg
metriques_files <- list.files(data_dir, pattern = "^metriques_.*\\.gpkg$", full.names = TRUE)
for (f in metriques_files) {
  tryCatch({
    metrics <- st_read(f, quiet = TRUE)
    new_cols <- setdiff(names(st_drop_geometry(metrics)), names(st_drop_geometry(parcelles)))
    if (length(new_cols) > 0) {
      parcelles <- cbind(parcelles, st_drop_geometry(metrics)[new_cols])
    }
  }, error = function(e) NULL)
}

# Liste des indicateurs par famille (12 indicateurs)
familles <- list(
  "W (Eau)" = c("W1", "W2", "W3"),
...
# Sauvegarder indicateurs_terrain.gpkg (fichier spécifique)
output_path <- file.path(data_dir, "indicateurs_terrain.gpkg")
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)

# Sauvegarder parcelles.gpkg avec tous les indicateurs consolidés
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Indicateurs terrain consolidés dans parcelles.gpkg\n")
```
