...
```{r ex-6-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=50, exercise.setup="ex-6-1-setup"}
# === FAMILLE W (Eau) : W1, W2, W3 ===

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== Consolidation Famille W (Eau) ===\n\n")

# Fonction pour importer un indicateur
import_indicator <- function(file_name, indicator_col) {
  file_path <- file.path(data_dir, file_name)
  if (file.exists(file_path)) {
    data <- st_read(file_path, quiet = TRUE)
    if (indicator_col %in% names(data)) {
      cat("  ✓", indicator_col, "importé depuis", file_name, "\n")
      return(as.numeric(unlist(data[[indicator_col]])))
    }
  }
  cat("  ✗", indicator_col, "non trouvé\n")
  return(NULL)
}

# Import des indicateurs W
w1 <- import_indicator("metriques_densite_hydro_w1.gpkg", "W1")
if (!is.null(w1)) parcelles$W1 <- w1

w2 <- import_indicator("metriques_couverture_zone_humide_w2.gpkg", "W2")
if (!is.null(w2)) parcelles$W2 <- w2

w3 <- import_indicator("metriques_twi_w3.gpkg", "W3")
if (!is.null(w3)) parcelles$W3 <- w3

# Statistiques
cat("\n=== Statistiques Famille W ===\n")
for (ind in c("W1", "W2", "W3")) {
  if (ind %in% names(parcelles)) {
    vals <- parcelles[[ind]]
    cat(sprintf("%s: min=%.2f, moy=%.2f, max=%.2f\n",
      ind, min(vals, na.rm=TRUE), mean(vals, na.rm=TRUE), max(vals, na.rm=TRUE)))
  }
}

# Sauvegarder
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille W consolidée (3 indicateurs)\n")
```
...
cat("=== Consolidation Famille R (Risques) ===\n\n")
...
# Import des indicateurs R
r1 <- import_indicator("metriques_exposition_feu_r1.gpkg", "R1")
if (!is.null(r1)) parcelles$R1 <- r1

r2 <- import_indicator("metriques_exposition_tempete_r2.gpkg", "R2")
if (!is.null(r2)) parcelles$R2 <- r2

r3 <- import_indicator("metriques_risque_secheresse_r3.gpkg", "R3")
if (!is.null(r3)) parcelles$R3 <- r3

r4 <- import_indicator("metriques_pression_gibier_r4.gpkg", "R4")
if (!is.null(r4)) parcelles$R4 <- r4

# Statistiques
cat("\n=== Statistiques Famille R ===\n")
for (ind in c("R1", "R2", "R3", "R4")) {
  if (ind %in% names(parcelles)) {
    vals <- parcelles[[ind]]
    cat(sprintf("%s: min=%.2f, moy=%.2f, max=%.2f\n",
      ind, min(vals, na.rm=TRUE), mean(vals, na.rm=TRUE), max(vals, na.rm=TRUE)))
...
# Sauvegarder
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille R consolidée (4 indicateurs)\n")
```
...
```{r ex-6-3, exercise=TRUE, exercise.eval=FALSE, exercise.lines=50, exercise.setup="ex-6-3-setup"}
# === FAMILLE S (Social) : S1, S2, S3 ===

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== Consolidation Famille S (Social) ===\n\n")
...
# Import des indicateurs S
s1 <- import_indicator("metriques_access_route_s1.gpkg", "S1")
if (!is.null(s1)) parcelles$S1 <- s1

s2 <- import_indicator("metriques_distance_batiment_s2.gpkg", "S2")
if (!is.null(s2)) parcelles$S2 <- s2

s3 <- import_indicator("metriques_densite_sentiers_s3.gpkg", "S3")
if (!is.null(s3)) parcelles$S3 <- s3

# Statistiques
cat("\n=== Statistiques Famille S ===\n")
for (ind in c("S1", "S2", "S3")) {
  if (ind %in% names(parcelles)) {
    vals <- parcelles[[ind]]
    cat(sprintf("%s: min=%.2f, moy=%.2f, max=%.2f\n",
      ind, min(vals, na.rm=TRUE), mean(vals, na.rm=TRUE), max(vals, na.rm=TRUE)))
...
# Sauvegarder
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille S consolidée (3 indicateurs)\n")
```
...
cat("=== Consolidation Famille F (Sol) ===\n\n")
...
# Import des indicateurs F
f1 <- import_indicator("metriques_erosion_f1.gpkg", "F1")
if (!is.null(f1)) parcelles$F1 <- f1

f2 <- import_indicator("metriques_fertilite_sol_f2.gpkg", "F2")
if (!is.null(f2)) parcelles$F2 <- f2

# Statistiques
cat("\n=== Statistiques Famille F ===\n")
for (ind in c("F1", "F2")) {
  if (ind %in% names(parcelles)) {
    vals <- parcelles[[ind]]
    cat(sprintf("%s: min=%.2f, moy=%.2f, max=%.2f\n",
...
# Sauvegarder
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille F consolidée (2 indicateurs)\n")
```
...
```{r ex-6-5, exercise=TRUE, exercise.eval=FALSE, exercise.lines=50, exercise.setup="ex-6-5-setup"}
# === SYNTHÈSE FINALE ===

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== Synthèse des indicateurs terrain ===\n\n")
...
total <- sum(unlist(familles) %in% names(parcelles))
cat("\nTotal:", total, "/12 indicateurs consolidés\n")

# Statistiques complètes
cat("\n=== Statistiques des indicateurs ===\n")
cat("Indicateur    Min      Moy      Max\n")
cat("----------------------------------\n")
for (ind in unlist(familles)) {
  if (ind %in% names(parcelles)) {
    vals <- parcelles[[ind]]
    cat(sprintf("%-10s %7.2f %8.2f %8.2f\n",
      ind, min(vals, na.rm=TRUE), mean(vals, na.rm=TRUE), max(vals, na.rm=TRUE)))
  }
}
