...
if (file.exists(mnt_path) && file.exists(parcelles_path)) {
  ...
  if (requireNamespace("climateR", quietly = TRUE) && requireNamespace("SPEI", quietly = TRUE)) {
    ...
    # Cache pour données climatiques
    climate_cache <- file.path(cache_dir, "climate_data.rds")

    climate_data <- NULL
    if (file.exists(climate_cache)) {
      cat("Chargement données climatiques depuis cache...\n")
      climate_data <- readRDS(climate_cache)
    } else {
      ...
      climate_data <- tryCatch({
        ...
        saveRDS(data, climate_cache)
        cat("Données climatiques mises en cache\n")
        data
      }, error = function(e) {
  ...
  if (requireNamespace("geodata", quietly = TRUE)) {
    ...
    # Cache pour données SoilGrids
    soil_cache <- file.path(cache_dir, "soilgrids_data.rds")
    bbox_wgs84 <- st_bbox(st_transform(parcelles, 4326))

    soil_data <- NULL
    if (file.exists(soil_cache)) {
      cat("Chargement données SoilGrids depuis cache...\n")
      soil_data <- readRDS(soil_cache)
    } else {
      soil_data <- tryCatch({
        ...
        data <- list(ph = ph_crop, soc = soc_crop, clay = clay_crop)
        # Sauvegarder en cache
        saveRDS(data, soil_cache)
        cat("Données SoilGrids mises en cache\n")
        data
      }, error = function(e) {
