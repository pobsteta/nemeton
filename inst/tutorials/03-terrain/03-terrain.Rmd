---
title: "Tutorial 03 : Terrain — Familles W, R, S, F (12 indicateurs)"
output:
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
description: >
  ...
  S1-3 (accessibilité sociale), F1-2 (sol : érosion et fertilité).
---
...
- **F** (Sol) : F1 (risque érosion), F2 (fertilité)
...
if (file.exists(mnt_path) && file.exists(parcelles_path)) {
  ...
  cat("Fichier principal:", parcelles_path, "\n")
...
## Section 2 : TWI et Fertilité (W3, F2)
...
### Exercice 2.2 : Fertilité du sol (F2)

L'indicateur **F2** évalue la fertilité potentielle du sol basée sur :
- **TWI** : Accumulation d'eau et nutriments (plus humide = plus fertile)
- **Pente** : Érosion des nutriments (pente forte = moins fertile)
...
```{r ex-2-2-f2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=70, exercise.setup="ex-2-2-f2-setup"}
# === FERTILITÉ DU SOL (F2) ===
...
if (file.exists(parcelles_path) && file.exists(mnt_path)) {
  ...
  cat("=== Calcul de F2 (Fertilité du sol) ===\n\n")
  ...
  # Charger pente depuis le fichier sauvegardé (Section 1)
  pente_path <- file.path(data_dir, "pente.tif")

  # 1. TWI moyen par parcelle
  if (file.exists(twi_path)) {
    twi <- rast(twi_path)
    parcelles$twi_mean <- extract_mean(twi, parcelles)
    cat("TWI moyen:", round(mean(parcelles$twi_mean, na.rm = TRUE), 2), "\n")
  } else {
    # Approximation TWI depuis pente
    if (file.exists(pente_path)) {
      pente_rad <- rast(pente_path) * pi / 180  # Convertir en radians
    } else {
      pente_rad <- terrain(mnt, v = "slope", unit = "radians")
    }
    twi_approx <- log(1 / (tan(pente_rad) + 0.001))
    twi_approx <- clamp(twi_approx, lower = 0, upper = 20)
    parcelles$twi_mean <- extract_mean(twi_approx, parcelles)
    cat("TWI approximé depuis MNT\n")
  }

  # 2. Pente moyenne par parcelle
  if (file.exists(pente_path)) {
    slope_deg <- rast(pente_path)
  } else {
    slope_deg <- terrain(mnt, v = "slope", unit = "degrees")
  }
  parcelles$slope_mean <- extract_mean(slope_deg, parcelles)
  cat("Pente moyenne:", round(mean(parcelles$slope_mean, na.rm = TRUE), 1), "°\n")

  # 3. Calcul F2
  # Normalisation TWI (5-15 -> 0-100)
  twi_norm <- function(twi) pmax(pmin((twi - 5) / 10 * 100, 100), 0)

  # Normalisation pente (0-45° -> 100-0)
  slope_norm <- function(slope) pmax(pmin(100 - (slope / 45) * 100, 100), 0)

  parcelles$F2 <- round((twi_norm(parcelles$twi_mean) + slope_norm(parcelles$slope_mean)) / 2, 1)

  cat("\n=== Résultats F2 ===\n")
  cat("F2 min:", round(min(parcelles$F2, na.rm = TRUE), 1), "\n")
  cat("F2 moy:", round(mean(parcelles$F2, na.rm = TRUE), 1), "/ 100\n")
  cat("F2 max:", round(max(parcelles$F2, na.rm = TRUE), 1), "\n")
  cat("Interprétation: 0 = sol pauvre, 100 = sol fertile\n")

  # Sauvegarder
  output_path <- file.path(data_dir, "metriques_fertilite_sol_f2.gpkg")
  st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
  st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)

  cat("\n=== Métriques F2 fertilité exportées ===\n")
  cat("Fichier:", output_path, "\n")
  cat("Fichier principal:", parcelles_path, "\n")
  cat("Parcelles:", nrow(parcelles), " lignes et ", ncol(parcelles), " colonnes\n\n")
} else {
...
| Score F2 | Classe | Description |
|----------|--------|-------------|
| 0 - 30 | Pauvre | Crêtes, pentes fortes, sol mince |
| 30 - 50 | Modéré | Versants, drainage moyen |
| 50 - 70 | Fertile | Mi-pente, bon drainage |
| 70 - 100 | Très fertile | Bas de pente, accumulation |
...
if (file.exists(parcelles_path)) {
  ...
  if (file.exists(cours_eau_path)) {
    ...
    if (nrow(cours_eau) > 0) {
      ...
      parcelles$W1 <- nemeton::smart_map(
...
if (file.exists(mnt_path) && file.exists(parcelles_path)) {
  ...
  if (is.null(R2_raster)) {
    ...
    # Charger pente et exposition depuis les fichiers sauvegardés (Section 1)
    pente_path <- file.path(data_dir, "pente.tif")
    expo_path <- file.path(data_dir, "exposition.tif")

    if (file.exists(expo_path)) {
      aspect <- rast(expo_path)
    } else {
      aspect <- terrain(mnt, v = "aspect", unit = "degrees")
    }

    if (file.exists(pente_path)) {
      pente <- rast(pente_path)
    } else {
      pente <- terrain(mnt, v = "slope", unit = "degrees")
    }
...
# 0. CONFIGURATION
# ============================================================
# Utilisation de nemeton::smart_map pour le mapping adaptatif
# La fonction choisit automatiquement séquentiel/parallèle selon le nombre d'éléments
...
if (file.exists(bd_foret_path)) {
  ...
  if (length(essence_col) > 0) {
    ...
    parcelles$R4_palatabilite <- nemeton::smart_map(
      seq_len(nrow(parcelles)), calc_palatabilite,
      complexity = "low", .type = "dbl"
    )
  }
}
...
parcelles$R4_lisiere <- nemeton::smart_map(
  seq_len(nrow(parcelles)), calc_lisiere,
...
if (file.exists(parcelles_path) && file.exists(mnt_path)) {
  ...
  if (file.exists(routes_path)) {
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_access_route_s1.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
if (file.exists(parcelles_path) && file.exists(mnt_path)) {
  ...
  if (file.exists(batiments_path)) {
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_distance_batiment_s2.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
# NOTE (pédagogique) — l'appel `osmdata` direct ci-dessous est là pour montrer
# la mécanique d'une requête Overpass. Ce n'est PAS le chemin de production :
# celui-ci passe par le client canonique de `foretaccess`, `osm_overpass()`
# pour le transport et `acquire_desserte_osm()` pour la desserte. Le client
# borne le pire cas, tourne sur plusieurs instances, distingue un refus
# Overpass d'un résultat vide, et cache avec provenance horodatée — trois
# propriétés qu'`osmdata` n'offre pas (une instance saturée le fait attendre en
# backoff sans rendre la main). Voir `specs/BRIEF-osm-overpass-unification.md`.
...
if (file.exists(parcelles_path) && file.exists(mnt_path)) {
  ...
  if (file.exists(sentiers_path)) {
    ...
    # Utiliser smart_map avec complexité "medium" (intersection spatiale ~20-100ms/parcelle)
    parcelles$S3 <- nemeton::smart_map(
      seq_len(nrow(parcelles)),
      calc_s3,
      complexity = "medium",
      .type = "dbl"
    ...
    # Sauvegarder
    output_path <- file.path(data_dir, "metriques_densite_sentiers_s3.gpkg")
    st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
    st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
```{r ex-6-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=55, exercise.setup="ex-6-1-setup"}
...
# W1 : Densité réseau hydrographique (calculé en section 3)
w1_path <- file.path(data_dir, "metriques_densite_hydro_w1.gpkg")
if (file.exists(w1_path)) {
  w1_data <- st_read(w1_path, quiet = TRUE) |> st_drop_geometry()
  if ("W1" %in% names(w1_data)) {
    parcelles$W1 <- as.numeric(unlist(w1_data$W1))
    cat("W1 (densité hydro):", round(mean(parcelles$W1, na.rm = TRUE), 1), "m/ha\n")
    cat("   0 = pas de cours d'eau, valeurs élevées = dense réseau hydro\n\n")
  }
}

# W2 : Zones humides (calculé en section 3)
w2_path <- file.path(data_dir, "metriques_couverture_zone_humide_w2.gpkg")
if (file.exists(w2_path)) {
  w2_data <- st_read(w2_path, quiet = TRUE) |> st_drop_geometry()
  if ("W2" %in% names(w2_data)) {
    parcelles$W2 <- as.numeric(unlist(w2_data$W2))
    cat("W2 (zones humides):", round(mean(parcelles$W2, na.rm = TRUE), 1), "%\n")
    cat("   0% = pas de zone humide, 100% = entièrement humide\n\n")
  }
}

# W3 : TWI moyen (calculé en section 2)
w3_path <- file.path(data_dir, "metriques_twi_w3.gpkg")
if (file.exists(w3_path)) {
  w3_data <- st_read(w3_path, quiet = TRUE) |> st_drop_geometry()
  if ("W3" %in% names(w3_data)) {
    parcelles$W3 <- as.numeric(unlist(w3_data$W3))
    cat("W3 (TWI moyen):", round(mean(parcelles$W3, na.rm = TRUE), 1), "\n")
    cat("   valeurs faibles = sec (crête), valeurs élevées = humide (fond de vallée)\n")
  }
}

# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Famille W consolidée:", basename(output_path), "\n")
```
...
# R1 : Risque incendie (calculé en section 4)
r1_path <- file.path(data_dir, "metriques_exposition_feu_r1.gpkg")
if (file.exists(r1_path)) {
  r1_data <- st_read(r1_path, quiet = TRUE) |> st_drop_geometry()
  if ("R1" %in% names(r1_data)) {
    parcelles$R1 <- as.numeric(unlist(r1_data$R1))
    cat("R1 (risque incendie):", round(mean(parcelles$R1, na.rm = TRUE), 1), "\n")
    cat("   0 = faible risque, 100 = risque très élevé\n\n")
  }
}

# R2 : Risque tempête (calculé en section 4)
r2_path <- file.path(data_dir, "metriques_exposition_tempete_r2.gpkg")
if (file.exists(r2_path)) {
  r2_data <- st_read(r2_path, quiet = TRUE) |> st_drop_geometry()
  if ("R2" %in% names(r2_data)) {
    parcelles$R2 <- as.numeric(unlist(r2_data$R2))
    cat("R2 (risque tempête):", round(mean(parcelles$R2, na.rm = TRUE), 1), "\n")
    cat("   0 = faible exposition, 100 = très exposé au vent\n\n")
  }
}

# R3 : Risque sécheresse (calculé en section 4)
r3_path <- file.path(data_dir, "metriques_risque_secheresse_r3.gpkg")
if (file.exists(r3_path)) {
  r3_data <- st_read(r3_path, quiet = TRUE) |> st_drop_geometry()
  if ("R3" %in% names(r3_data)) {
    parcelles$R3 <- as.numeric(unlist(r3_data$R3))
    cat("R3 (risque sécheresse):", round(mean(parcelles$R3, na.rm = TRUE), 1), "\n")
    cat("   0 = faible risque, 100 = risque très élevé\n\n")
  }
}

# R4 : Pression gibier (calculé en section 4)
r4_path <- file.path(data_dir, "metriques_pression_gibier_r4.gpkg")
if (file.exists(r4_path)) {
  r4_data <- st_read(r4_path, quiet = TRUE) |> st_drop_geometry()
  if ("R4" %in% names(r4_data)) {
    parcelles$R4 <- as.numeric(unlist(r4_data$R4))
    cat("R4 (pression gibier):", round(mean(parcelles$R4, na.rm = TRUE), 1), "\n")
    cat("   0 = faible pression, 100 = forte pression d'abroutissement\n")
  }
}

# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Famille R consolidée:", basename(output_path), "\n")
```
...
# S1 : Distance aux routes (calculé en section 5)
s1_path <- file.path(data_dir, "metriques_access_route_s1.gpkg")
if (file.exists(s1_path)) {
  s1_data <- st_read(s1_path, quiet = TRUE) |> st_drop_geometry()
  if ("S1" %in% names(s1_data)) {
    parcelles$S1 <- as.numeric(unlist(s1_data$S1))
    cat("S1 (distance routes):", round(mean(parcelles$S1, na.rm = TRUE), 0), "m\n")
    cat("   0 = sur route, valeurs élevées = éloigné des routes\n\n")
  }
}

# S2 : Distance aux bâtiments (calculé en section 5)
s2_path <- file.path(data_dir, "metriques_distance_batiment_s2.gpkg")
if (file.exists(s2_path)) {
  s2_data <- st_read(s2_path, quiet = TRUE) |> st_drop_geometry()
  if ("S2" %in% names(s2_data)) {
    parcelles$S2 <- as.numeric(unlist(s2_data$S2))
    cat("S2 (distance bâtiments):", round(mean(parcelles$S2, na.rm = TRUE), 0), "m\n")
    cat("   0 = sur bâtiment, valeurs élevées = éloigné de l'habitat\n\n")
  }
}

# S3 : Densité de sentiers (calculé en section 5)
s3_path <- file.path(data_dir, "metriques_densite_sentiers_s3.gpkg")
if (file.exists(s3_path)) {
  s3_data <- st_read(s3_path, quiet = TRUE) |> st_drop_geometry()
  if ("S3" %in% names(s3_data)) {
    parcelles$S3 <- as.numeric(unlist(s3_data$S3))
    cat("S3 (densité sentiers):", round(mean(parcelles$S3, na.rm = TRUE), 1), "m/ha\n")
    cat("   0 = pas de sentier, valeurs élevées = forte fréquentation potentielle\n")
  }
}

# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Famille S consolidée:", basename(output_path), "\n")
```
...
```{r ex-6-4, exercise=TRUE, exercise.eval=FALSE, exercise.lines=45, exercise.setup="ex-6-4-setup"}
# === FAMILLE F (SOL) : F1, F2 ===
# Consolidation des indicateurs déjà calculés
...
# F1 : Risque érosion (calculé en section 1)
f1_path <- file.path(data_dir, "metriques_erosion_f1.gpkg")
if (file.exists(f1_path)) {
  f1_data <- st_read(f1_path, quiet = TRUE) |> st_drop_geometry()
  if ("F1" %in% names(f1_data)) {
    parcelles$F1 <- as.numeric(unlist(f1_data$F1))
    cat("F1 (risque érosion):", round(mean(parcelles$F1, na.rm = TRUE), 1), "\n")
    cat("   0 = faible risque, 100 = risque très élevé (RUSLE simplifié)\n\n")
  }
}

# F2 : Fertilité sol (calculé en section 2)
f2_path <- file.path(data_dir, "metriques_fertilite_sol_f2.gpkg")
if (file.exists(f2_path)) {
  f2_data <- st_read(f2_path, quiet = TRUE) |> st_drop_geometry()
  if ("F2" %in% names(f2_data)) {
    parcelles$F2 <- as.numeric(unlist(f2_data$F2))
    cat("F2 (fertilité sol):", round(mean(parcelles$F2, na.rm = TRUE), 1), "\n")
    cat("   0 = sol pauvre, 100 = sol fertile\n")
  }
}

# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Famille F consolidée:", basename(output_path), "\n")
```
...
```{r ex-6-5, exercise=TRUE, exercise.eval=FALSE, exercise.lines=55, exercise.setup="ex-6-5-setup"}
# === SYNTHÈSE FINALE ===
# Consolidation des indicateurs terrain (T03 uniquement)

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== CONSOLIDATION INDICATEURS TERRAIN (T03) ===\n\n")

# Liste des indicateurs terrain par famille (12 indicateurs T03)
familles <- list(
  "W (Eau)" = c("W1", "W2", "W3"),
  "R (Risques)" = c("R1", "R2", "R3", "R4"),
  "S (Social)" = c("S1", "S2", "S3"),
  "F (Sol)" = c("F1", "F2")
)
indicateurs_t03 <- unlist(familles)
...
cat("Colonnes totales:", ncol(parcelles), "\n")
cat(strrep("=", 40), "\n")

# Référence exercices pour indicateurs manquants
manquants <- indicateurs_calcules[!indicateurs_calcules %in% names(parcelles)]
if (length(manquants) > 0) {
  cat("\n⚠ INDICATEURS MANQUANTS - Exercices à exécuter :\n")
  refs <- c(F1 = "1.1", W3 = "2.1", F2 = "3.1", W1 = "4.1", W2 = "4.2",
            R1 = "5.1", R2 = "5.2", R3 = "5.3", R4 = "5.4",
            S1 = "6.1", S2 = "6.2", S3 = "6.3")
  for (m in manquants) {
    cat(sprintf("  - %s → Exercice %s\n", m, refs[m]))
  }
}

# Sauvegarder indicateurs_terrain.gpkg (uniquement indicateurs T03)
cols_base <- c("id", "code_parcelle", "surface_ha")  # colonnes de base si présentes
cols_base <- cols_base[cols_base %in% names(parcelles)]
cols_t03 <- indicateurs_t03[indicateurs_t03 %in% names(parcelles)]
parcelles_t03 <- parcelles[, c(cols_base, cols_t03)]

output_path <- file.path(data_dir, "indicateurs_terrain.gpkg")
st_write(parcelles_t03, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ indicateurs_terrain.gpkg:", length(cols_t03), "indicateurs T03\n")

# Sauvegarder parcelles.gpkg avec tous les indicateurs (cumulatif)
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("✓ parcelles.gpkg: tous les indicateurs consolidés\n")
```
...
|---------|---------|-------------|
| `indicateurs_terrain.gpkg` | Synthèse complète (12 indicateurs) | Tous |
| `metriques_densite_hydro_w1.gpkg` | Densité hydrographique | W1 |
| `metriques_couverture_zone_humide_w2.gpkg` | Zones humides | W2 |
| `metriques_twi_w3.gpkg` | TWI moyen | W3 |
| `metriques_exposition_feu_r1.gpkg` | Risque incendie | R1 |
...
| **F** | F2 | Fertilité sol | MNT (TWI + pente) | 0-100 | 0=pauvre, 100=fertile |

### Fichiers produits

| Fichier | Contenu | Indicateurs |
|---------|---------|-------------|
| `indicateurs_terrain.gpkg` | Synthèse complète | W1-3, R1-4, S1-3, F1-2 |
...
→ **Tutorial 04 : Écologie** — Familles B, L, T, N (11 indicateurs) : Calculer les indicateurs Biodiversité (B1, B2, B3), Paysage (L1, L2, L3), Temporel (T1, T2) et Naturalité (N1, N2, N3).
