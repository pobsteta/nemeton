...
### Exercice 6.4 : Famille F (Sol) - F1 et F2

La famille F comprend deux indicateurs :
- **F1** : Risque d'érosion (RUSLE simplifié)
- **F2** : Fertilité du sol (TWI + pente)
...
```{r ex-6-4, exercise=TRUE, exercise.eval=FALSE, exercise.lines=90, exercise.setup="ex-6-4-setup"}
# === FAMILLE F (SOL) - F1 et F2 ===
output_path <- file.path(data_dir, "indicateurs_terrain.gpkg")
parcelles <- st_read(output_path, quiet = TRUE)
mnt_path <- file.path(data_dir, "mnt.tif")
twi_path <- file.path(data_dir, "twi.tif")
...
# === F1 : Risque érosion RUSLE simplifié ===
cat("--- F1 : Risque érosion ---\n")
...
if (file.exists(erosion_path)) {
  ...
  cat("   Score 0-1 : 0=très faible, 1=très élevé\n\n")
} else {
  cat("F1: Fichier erosion_f1.tif non trouvé.\n\n")
}

# === F2 : Fertilité du sol ===
cat("--- F2 : Fertilité du sol ---\n")

# 1. TWI (indice d'humidité topographique)
if (file.exists(twi_path)) {
  twi <- rast(twi_path)
  parcelles$twi_mean <- extract_mean(twi, parcelles)
  cat("TWI moyen:", round(mean(parcelles$twi_mean, na.rm = TRUE), 2), "\n")
} else if (file.exists(mnt_path)) {
  mnt <- rast(mnt_path)
  slope_rad <- terrain(mnt, v = "slope", unit = "radians")
  twi_approx <- log(1 / (tan(slope_rad) + 0.001))
...
# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\nFamille F (F1 + F2) sauvegardée:", output_path, "\n")
```
...
cat("\nTotal:", sum(unlist(familles) %in% names(parcelles)), "/12 indicateurs calculés\n")
```
