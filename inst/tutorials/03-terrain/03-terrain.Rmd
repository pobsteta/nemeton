    ...
    tryCatch({
      ...
      if (!is.null(depts) && nrow(depts) > 0) {
        ...
        if (length(dept_col) > 0) {
          # Jointure spatiale vectorisée
          centroids <- st_centroid(parcelles)
          join <- st_join(centroids, depts[, dept_col[1]])
          parcelles$code_dept <- join[[dept_col[1]]]

          # Joindre l'indice de pression (vectorisé avec match)
          idx <- match(parcelles$code_dept, pressure_data$code_dept)
          parcelles$R4_densite <- ifelse(is.na(idx), 50, pressure_data$pressure_index[idx])
