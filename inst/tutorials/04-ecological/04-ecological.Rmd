...
if (file.exists(parcelles_path)) {
  ...
  if (file.exists(zones_path)) {
    ...
    if (requireNamespace("happign", quietly = TRUE) && requireNamespace("purrr", quietly = TRUE)) {
      ...
      # Utiliser smart_map avec complexité "high" (requêtes WFS ~100-500ms/couche)
      all_zones <- nemeton::smart_map(
        patrinat_layers,
        function(layer) download_layer(layer, shape = parcelles),
        complexity = "high"
      )
...
if (file.exists(parcelles_path) && requireNamespace("purrr", quietly = TRUE)) {
  ...
  if (file.exists(zones_path)) {
    ...
    results <- nemeton::smart_map(
      idx_list,
      function(i) calc_b1(i, parcelles_df = parcelles, zones_df = zones, zones_union_geom = zones_union),
      complexity = "medium"
    )
...
if (file.exists(parcelles_path)) {
  ...
  cat("Fichier parcelles.gpkg non trouvé. Exécutez le Tutorial 01.\n")
}
```
...
# -------------------------------------------------------------
# NOTE (pédagogique) — les appels `osmdata` directs ci-dessous montrent la
# mécanique d'une requête Overpass, une par thème. Ce n'est PAS le chemin de
# production : celui-ci passe par le client canonique de `foretaccess`,
# `osm_overpass()`, qui borne le pire cas, tourne sur plusieurs instances,
# distingue un refus Overpass d'un résultat vide et cache avec provenance
# horodatée. En production on **regroupe** aussi les filtres en une seule
# requête au lieu d'en enchaîner quatre comme ici : Overpass limite le nombre
# de requêtes, pas la surface interrogée.
# Voir `specs/BRIEF-osm-overpass-unification.md`.
osm_available <- FALSE
...
if (ocs_source == "ocsge" && !is.null(ocs_data)) {
  ...
  # Utiliser smart_map avec complexité "medium" (intersection spatiale ~20-100ms/parcelle)
  parcelles$L1_contraste <- nemeton::smart_map(
  ...
  # Utiliser smart_map avec complexité "medium" (calcul géométrique ~20-100ms/parcelle)
  parcelles$L1_contraste <- nemeton::smart_map(
    seq_len(nrow(parcelles)),
    calc_osm_i,
    complexity = "medium",
...
# Utiliser smart_map avec complexité "low" (calcul géométrique simple ~5-20ms/parcelle)
exposures_list <- nemeton::smart_map(
  seq_len(nrow(parcelles)),
  calc_exposure_i,
  complexity = "low"
)
...
results_list <- nemeton::smart_map(
  seq_len(nrow(parcelles)),
  function(i) calc_l3_i(i, parcelles_geom, tvb_trans, cours_eau),
  complexity = "medium"
)
...
#
# NOTE (pédagogique) — ce bloc et le suivant (zones urbaines) appellent
# `osmdata` directement pour rester lisibles. Ce n'est PAS le chemin de
# production : `foretaccess::acquire_obstacles()` couvre déjà bâti / eau / voie
# ferrée / falaise en une requête, au-dessus du client canonique
# `osm_overpass()` (pire cas borné, rotation d'instances, refus distingué d'un
# résultat vide, cache horodaté).
# Voir `specs/BRIEF-osm-overpass-unification.md`.
...
results_list <- nemeton::smart_map(
  seq_len(nrow(parcelles)),
  function(i) calc_n2_i(i, parcelles_geom, bd_foret, foret_ancienne),
  complexity = "medium"
)
results <- do.call(rbind, results_list)
...
if (file.exists(bd_foret_path)) {
  ...
  if (!is.null(tfv_field)) {
    ...
    # Utiliser smart_map avec complexité "medium" (intersection spatiale ~20-100ms/parcelle)
    parcelles$T1 <- nemeton::smart_map(
      seq_len(nrow(parcelles)),
      function(i) calc_t1_i(i, parcelles, bd_foret, tfv_field, estimate_age),
      complexity = "medium",
      .type = "dbl"
    )
  } else {
...
# === SYNTHÈSE FINALE ===
# Consolidation des indicateurs écologiques (T04 uniquement)

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
parcelles <- st_read(parcelles_path, quiet = TRUE)

cat("=== CONSOLIDATION INDICATEURS ÉCOLOGIQUES (T04) ===\n\n")

# Liste des indicateurs écologiques par famille (11 indicateurs T04)
familles <- list(
...
indicateurs_t04 <- unlist(familles)
...
if (length(manquants) > 0) {
  ...
  refs <- c(B1 = "1.1", B2 = "2.1", B3 = "2.2",
            L1 = "3.1", L2 = "3.2", L3 = "4.1",
            N1 = "5.1", N2 = "5.1", N3 = "5.1",
            T1 = "6.1", T2 = "6.1")
  for (m in manquants) {
...
# Sauvegarder indicateurs_ecologiques.gpkg (uniquement indicateurs T04)
cols_base <- c("id", "code_parcelle", "surface_ha")  # colonnes de base si présentes
cols_base <- cols_base[cols_base %in% names(parcelles)]
cols_t04 <- indicateurs_t04[indicateurs_t04 %in% names(parcelles)]
parcelles_t04 <- parcelles[, c(cols_base, cols_t04)]

output_path <- file.path(data_dir, "indicateurs_ecologiques.gpkg")
st_write(parcelles_t04, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ indicateurs_ecologiques.gpkg:", length(cols_t04), "indicateurs T04\n")

# Sauvegarder parcelles.gpkg avec tous les indicateurs (cumulatif)
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
cat("✓ parcelles.gpkg: tous les indicateurs consolidés (32 au total)\n")
```
