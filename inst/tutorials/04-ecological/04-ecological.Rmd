...
```{r ex-4-2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=120, exercise.setup="ex-4-2-setup", exercise.timelimit=600}
...
```{r ex-4-3, exercise=TRUE, exercise.eval=FALSE, exercise.lines=150, exercise.setup="ex-4-3-setup", exercise.timelimit=900}
...
## Section 6 : Export Optimisé des 14 Indicateurs Écologiques
...
```{r ex-6-1, exercise=TRUE, exercise.eval=FALSE, exercise.lines=120, exercise.setup="ex-6-1-setup", exercise.timelimit=600}
...
```{r ex-6-2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=150, exercise.setup="ex-6-2-setup", exercise.timelimit=600}
...
# ============================================================
# Récupérer L3 s'il a été calculé dans la section 4
if ("L3" %in% names(parcelles)) {
  cat("L3 (TVB): déjà calculé -", round(mean(parcelles$L3, na.rm = TRUE), 1), "(moy)\n")
} else {
  cat("\nCalcul L3 (Trame Verte et Bleue)...\n")

  # Fonction de calcul L3 pour une parcelle
  calc_L3 <- function(idx, parcelles, bd_foret) {
    buffer_500m <- st_buffer(st_geometry(parcelles)[idx], 500)
    geom <- st_geometry(parcelles)[idx]

    # Trame Verte: diversité forestière dans un rayon de 500m
    foret_buffer <- suppressWarnings(st_intersection(bd_foret, buffer_500m))
    trame_verte <- 0
    if (nrow(foret_buffer) > 0) {
      # Utiliser la diversité des types de peuplement
      if ("tfv" %in% names(foret_buffer)) {
        n_types <- length(unique(foret_buffer$tfv))
        trame_verte <- min(100, n_types * 20)
      } else {
        surface_foret <- as.numeric(sum(st_area(foret_buffer)))
        surface_buffer <- as.numeric(st_area(buffer_500m))
        trame_verte <- min(100, (surface_foret / surface_buffer) * 150)
      }
    }

    # Trame Bleue: proximité à l'eau (score 0-100 basé sur distance)
    # Simulé ici - idéalement utiliser données BDTOPO eau
    trame_bleue <- runif(1, 30, 70)

    # L3 composite: 60% Trame Verte + 40% Trame Bleue
    return(0.6 * trame_verte + 0.4 * trame_bleue)
  }
  ...
  if (exists("bd_foret")) {
    ...
    if (requireNamespace("furrr", quietly = TRUE) &&
      ...
      parcelles$L3 <- future_map_dbl(
        seq_len(n_parcelles),
        ~calc_L3(.x, parcelles, bd_foret),
        .progress = TRUE,
        .options = furrr_options(seed = TRUE)
      )
      ...
      cat("Mode: SÉQUENTIEL (purrr)\n")
      parcelles$L3 <- map_dbl(seq_len(n_parcelles), ~calc_L3(.x, parcelles, bd_foret))
    }
  } else {
    # Fallback si BD Forêt non disponible
    parcelles$L3 <- runif(nrow(parcelles), 40, 80)
  }

  cat("L3 (TVB):", round(mean(parcelles$L3, na.rm = TRUE), 1), "(moy)\n")
}
...
```{r ex-6-2-check}
grade_this({
  pass("Famille L (Paysage: L1, L2, L3) calculée et exportée avec succès !")
})
```

---

### Exercice 6.2b : Famille C (Carbone/Vitalité) — C2
...
```{r ex-6-2b, exercise=TRUE, exercise.eval=FALSE, exercise.lines=80, exercise.setup="ex-6-2b-setup", exercise.timelimit=600}
# === FAMILLE C (CARBONE/VITALITÉ) : C2 ===
# Indicateur de vitalité basé sur NDVI

output_path <- file.path(data_dir, "indicateurs_ecologiques.gpkg")
parcelles <- st_read(output_path, quiet = TRUE)

cat("=== Famille C (Vitalité) — Calcul C2 ===\n\n")

# ============================================================
# C2 : Indice de Vitalité (NDVI)
# ============================================================
# Récupérer C2 s'il a été calculé dans la section 4
if ("C2" %in% names(parcelles) && !all(is.na(parcelles$C2))) {
  cat("C2 (NDVI): déjà calculé -", round(mean(parcelles$C2, na.rm = TRUE), 3), "(moy)\n")
  cat("C2 score:", round(mean(parcelles$C2 * 100, na.rm = TRUE), 1), "/ 100\n")
} else {
  cat("Calcul C2 (Vitalité NDVI)...\n")

  # Vérifier si un raster NDVI est disponible
  ndvi_path <- file.path(data_dir, "ndvi.tif")

  if (file.exists(ndvi_path)) {
    cat("Extraction depuis raster NDVI...\n")
    ...
    if (requireNamespace("exactextractr", quietly = TRUE)) {
      ...
      parcelles$C2 <- terra::extract(ndvi_raster, vect(st_transform(parcelles, crs(ndvi_raster))), fun = mean, na.rm = TRUE)[, 2]
    }
  } else {
    cat("Pas de raster NDVI disponible, estimation depuis BD Forêt...\n")
    ...
    if (file.exists(bd_foret_path)) {
      ...
      # Table des NDVI typiques par type de forêt
      ndvi_by_type <- data.frame(
        pattern = c("futaie.*feuill", "futaie.*resin", "melange", "taillis", "peupleraie"),
        ndvi_base = c(0.75, 0.70, 0.72, 0.65, 0.60)
      )

      parcelles$C2 <- 0.65  # Valeur par défaut

      for (i in seq_len(nrow(parcelles))) {
        foret_inter <- suppressWarnings(st_intersection(bd_foret, st_geometry(parcelles)[i]))
        if (nrow(foret_inter) > 0 && "tfv" %in% names(foret_inter)) {
          type_val <- tolower(as.character(foret_inter$tfv[1]))
          for (j in seq_len(nrow(ndvi_by_type))) {
            if (grepl(ndvi_by_type$pattern[j], type_val)) {
              parcelles$C2[i] <- ndvi_by_type$ndvi_base[j] + rnorm(1, 0, 0.05)
              break
      ...
      parcelles$C2 <- runif(nrow(parcelles), 0.55, 0.80)
    }
  }

  # Borner entre 0 et 1
  parcelles$C2 <- pmax(0, pmin(1, parcelles$C2))
  cat("C2 (NDVI):", round(mean(parcelles$C2, na.rm = TRUE), 3), "(moy)\n")
  cat("C2 score:", round(mean(parcelles$C2 * 100, na.rm = TRUE), 1), "/ 100\n")
}

# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Famille C sauvegardée:", basename(output_path), "\n")
```

```{r ex-6-2b-check}
grade_this({
  pass("Famille C (Vitalité: C2 NDVI) calculée et exportée avec succès !")
})
```
...
```{r ex-6-3, exercise=TRUE, exercise.eval=FALSE, exercise.lines=100, exercise.setup="ex-6-3-setup", exercise.timelimit=600}
...
```{r ex-6-4, exercise=TRUE, exercise.eval=FALSE, exercise.lines=100, exercise.setup="ex-6-4-setup", exercise.timelimit=600}
...
```{r ex-6-5, exercise=TRUE, exercise.eval=FALSE, exercise.lines=130, exercise.setup="ex-6-5-setup", exercise.timelimit=600}
