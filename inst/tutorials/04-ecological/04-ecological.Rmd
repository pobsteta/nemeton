...
## Section 4 : Paysage et Sylvosphère (L1, L2)
...
L'indicateur **C2** mesure la **vitalité de la végétation** via l'indice NDVI (Normalized Difference Vegetation Index) calculé à partir d'images satellite Sentinel-2.

| Bande | Longueur d'onde | Utilisation |
|-------|-----------------|-------------|
| B4 | Rouge (665 nm) | Absorption chlorophylle |
| B8 | NIR (842 nm) | Réflexion végétation |
...
# ============================================================
# 1. TÉLÉCHARGEMENT SENTINEL-2 via rstac/openeo
# ============================================================

# Méthode 1: rstac (STAC API Copernicus)
if (requireNamespace("rstac", quietly = TRUE)) {
  library(rstac)

  tryCatch({
    cat("Tentative via rstac (Copernicus STAC)...\n")
    ...
    if (length(items$features) > 0) {
      ...
      # Télécharger la première image (bandes B4 et B8)
      best_item <- items$features[[1]]
      cat("Image sélectionnée:", best_item$id, "\n")
      cat("Couverture nuageuse:", best_item$properties$`eo:cloud_cover`, "%\n")

      # Note: Le téléchargement réel nécessite une authentification
      # Pour le tutoriel, on simule le NDVI
    }
  }, error = function(e) {
...
# Méthode 2: geodata (données globales simplifiées)
if (!ndvi_available && requireNamespace("geodata", quietly = TRUE)) {
  tryCatch({
    cat("\nTentative via geodata (MODIS NDVI)...\n")

    # geodata fournit des données MODIS NDVI globales
    # Résolution plus faible (250m) mais accessible sans authentification

    # Note: geodata::pixel_ndvi() ou similar si disponible
    cat("geodata disponible pour données MODIS\n")
  }, error = function(e) {
    cat("Erreur geodata:", e$message, "\n")
  })
}
...
# ============================================================
# 2. CALCUL DU NDVI (simulation basée sur données disponibles)
# ============================================================

cat("\n=== Calcul NDVI ===\n")

# Si pas de données satellite, estimation basée sur:
# - BD Forêt (type de peuplement)
# - Métriques LiDAR (couverture)
...
### Exercice 6.1 : Famille B (Biodiversité)
...
```{r ex-6-1, exercise=TRUE, exercise.lines=80, exercise.setup="ex-6-1-setup", exercise.timelimit=600}
...
# Valeur en % de la surface de la parcelle couverte
...
for (i in seq_len(nrow(parcelles))) {
  taux_protection <- 0
  parcelle_geom <- st_geometry(parcelles)[i]
  surface_parcelle <- as.numeric(st_area(parcelle_geom))

  # ZNIEFF
  if (file.exists(znieff_path)) {
    znieff <- st_read(znieff_path, quiet = TRUE)
    znieff <- st_transform(znieff, st_crs(parcelles))
    inter_znieff <- suppressWarnings(st_intersection(znieff, parcelle_geom))
    if (nrow(inter_znieff) > 0) {
      taux_protection <- taux_protection + as.numeric(sum(st_area(inter_znieff))) / surface_parcelle * 100
    }
  }
  ...
  if (file.exists(natura_path)) {
    ...
    natura <- st_transform(natura, st_crs(parcelles))
    inter_natura <- suppressWarnings(st_intersection(natura, parcelle_geom))
    if (nrow(inter_natura) > 0) {
      taux_protection <- taux_protection + as.numeric(sum(st_area(inter_natura))) / surface_parcelle * 50
    }
  }

  parcelles$B1[i] <- min(100, taux_protection)  # Plafonner à 100%
}
...
lidar_path <- file.path(data_dir, "metriques_lidar.gpkg")
if (file.exists(lidar_path)) {
  lidar_metrics <- st_read(lidar_path, quiet = TRUE)
  lidar_metrics <- st_drop_geometry(lidar_metrics)

  # Joindre par id_parcelle ou spatiale
  if ("id_parcelle" %in% names(lidar_metrics) && "id_parcelle" %in% names(parcelles)) {
    ...
    # B2 = fonction de l'écart-type des hauteurs et entropie
    parcelles$B2 <- ifelse(!is.na(parcelles$zsd),
                           pmin(100, (parcelles$zsd / 15) * 50 +
                                     ifelse(!is.na(parcelles$entropy), parcelles$entropy * 25, 25)),
                           50)  # Valeur par défaut si pas de LiDAR
  } else {
    parcelles$B2 <- 50
  }
} else {
  # Estimation basée sur BD Forêt
  parcelles$B2 <- 50
}
cat("B2 (structure):", round(mean(parcelles$B2, na.rm = TRUE), 1), "(moy)\n")

# B3 : Connectivité écologique (calculé dans ex-3-2, on récupère ou recalcule)
# Valeur composite sur 100
...
```{r ex-6-1-check}
grade_this({
  pass("Famille B (Biodiversité) exportée !")
})
```

---

### Exercice 6.2 : Famille L (Paysage)
...
```{r ex-6-2, exercise=TRUE, exercise.lines=70, exercise.setup="ex-6-2-setup", exercise.timelimit=600}
...
# L1 : Indice de lisière (sylvosphère)
# Ratio périmètre / surface (forme factor)
# Plus le ratio est élevé, plus la parcelle est exposée aux effets de lisière
parcelles$L1 <- sapply(seq_len(nrow(parcelles)), function(i) {
  geom <- st_geometry(parcelles)[i]
  perimetre <- as.numeric(st_length(st_cast(geom, "MULTILINESTRING")))
  surface <- as.numeric(st_area(geom))

  # Indice de forme: 100 * périmètre / (2 * sqrt(pi * surface))
  # Cercle = 100, formes complexes > 100
  if (surface > 0) {
    indice_forme <- 100 * perimetre / (2 * sqrt(pi * surface))
    # Normaliser sur 0-100 (100 = cercle parfait = peu de lisière)
    # On inverse: plus de lisière = valeur plus élevée
...
# L2 : Fragmentation du paysage
# Basé sur la BD Forêt: nombre de taches forestières dans un buffer
bd_foret_path <- file.path(data_dir, "bd_foret.gpkg")
...
if (file.exists(bd_foret_path)) {
  ...
  parcelles$L2 <- sapply(seq_len(nrow(parcelles)), function(i) {
    # Buffer de 500m autour de la parcelle
    buffer <- st_buffer(st_geometry(parcelles)[i], 500)

    # Compter les taches forestières dans le buffer
    foret_buffer <- suppressWarnings(st_intersection(bd_foret, buffer))

    if (nrow(foret_buffer) > 0) {
      n_taches <- nrow(foret_buffer)
      # Plus de taches = plus fragmenté = indice plus élevé
      # Normaliser: 1 tache = 0, 20+ taches = 100
      return(min(100, max(0, (n_taches - 1) * 5)))
    }
    return(50)  # Valeur par défaut
  })
} else {
  # Estimation par la compacité des parcelles
  parcelles$L2 <- parcelles$L1 * 0.8  # Corrélé à L1
}
...
```{r ex-6-2-check}
grade_this({
  pass("Famille L (Paysage) exportée !")
})
```

---

### Exercice 6.3 : Famille T (Temporel)
...
```{r ex-6-3, exercise=TRUE, exercise.lines=70, exercise.setup="ex-6-3-setup", exercise.timelimit=600}
...
# Basé sur la BD Forêt et les métriques LiDAR (hauteur dominante)
bd_foret_path <- file.path(data_dir, "bd_foret.gpkg")
lidar_path <- file.path(data_dir, "metriques_lidar.gpkg")

parcelles$T1 <- 50  # Valeur par défaut (50 ans)

if (file.exists(bd_foret_path)) {
  bd_foret <- st_read(bd_foret_path, quiet = TRUE)
  bd_foret <- st_transform(bd_foret, st_crs(parcelles))

  # Chercher la colonne d'essence ou de type
  ...
  for (i in seq_len(nrow(parcelles))) {
    ...
    if (nrow(foret_inter) > 0) {
      # Estimation de l'âge basée sur le type de peuplement
      # Futaie régulière feuillus: 80-120 ans, Futaie résineux: 50-80 ans
      # Taillis: 20-40 ans, Mélange: 40-60 ans
      if (length(essence_col) > 0 && essence_col[1] %in% names(foret_inter)) {
        type <- as.character(foret_inter[[essence_col[1]]][1])
        if (grepl("futaie|feuill", type, ignore.case = TRUE)) {
          parcelles$T1[i] <- sample(80:120, 1)
        } else if (grepl("resin|conif|pin|sapin", type, ignore.case = TRUE)) {
          parcelles$T1[i] <- sample(50:80, 1)
        } else if (grepl("taillis", type, ignore.case = TRUE)) {
          parcelles$T1[i] <- sample(20:40, 1)
        } else {
          parcelles$T1[i] <- sample(40:70, 1)
        }
      }
    }
  }
...
# Négatif = perte de couvert, Positif = gain
# Basé sur comparaison BD Forêt anciennes vs actuelle ou analyse NDVI
...
if (file.exists(bd_foret_anciennes_path)) {
  ...
  parcelles$T2 <- sapply(seq_len(nrow(parcelles)), function(i) {
    # Taux de changement simulé basé sur l'ancienneté
    if (!is.na(parcelles$N2[i]) && parcelles$N2[i] > 80) {
      return(runif(1, 0, 0.5))  # Forêt ancienne stable
    } else {
      return(runif(1, -1, 1))  # Forêt récente, plus variable
    }
  })
} else {
  # Estimation sans données historiques
...
```{r ex-6-3-check}
grade_this({
  pass("Famille T (Temporel) exportée !")
})
```

---

### Exercice 6.4 : Familles A/F (Air/Fertilité)
...
```{r ex-6-4, exercise=TRUE, exercise.lines=80, exercise.setup="ex-6-4-setup", exercise.timelimit=600}
# === FAMILLES A (AIR) et F (FERTILITÉ) : A2, F2 ===

output_path <- file.path(data_dir, "indicateurs_ecologiques.gpkg")
parcelles <- st_read(output_path, quiet = TRUE)

# Fonction d'extraction raster
extract_mean <- function(r, poly) {
  if (requireNamespace("exactextractr", quietly = TRUE)) {
    exactextractr::exact_extract(r, poly, fun = "mean")
  } else {
    terra::extract(r, vect(poly), fun = mean, na.rm = TRUE)[, 2]
  }
}

cat("=== Familles A/F (Air/Fertilité) ===\n\n")

# A2 : Qualité de l'air forestier
# Basé sur: densité du couvert, distance aux routes, altitude
# Score 0-100 (100 = excellente qualité)

mnt_path <- file.path(data_dir, "mnt.tif")
dist_routes_path <- file.path(data_dir, "distance_routes.tif")
lidar_path <- file.path(data_dir, "metriques_lidar.gpkg")

parcelles$A2 <- 70  # Valeur par défaut (forêt = bon air)

for (i in seq_len(nrow(parcelles))) {
  score_air <- 70  # Base forêt

  # Bonus altitude (>500m = air plus pur)
  if (file.exists(mnt_path)) {
    mnt <- rast(mnt_path)
    altitude <- extract_mean(mnt, parcelles[i, ])
    if (!is.na(altitude) && altitude > 500) {
      score_air <- score_air + min(15, (altitude - 500) / 100 * 5)
    }
  }

  # Bonus distance routes (>500m = moins de pollution)
  if (file.exists(dist_routes_path)) {
    dist_routes <- rast(dist_routes_path)
    dist <- extract_mean(dist_routes, parcelles[i, ])
    if (!is.na(dist) && dist > 500) {
      score_air <- score_air + min(15, (dist - 500) / 200 * 5)
    }
  }

  parcelles$A2[i] <- min(100, score_air)
}
cat("A2 (qualité air):", round(mean(parcelles$A2, na.rm = TRUE), 1), "(moy)\n")

# F2 : Fertilité du sol
# Basé sur: TWI (humidité), pente, altitude, type de sol si disponible
twi_path <- file.path(data_dir, "twi.tif")
pente_path <- file.path(data_dir, "pente.tif")

parcelles$F2 <- 60  # Valeur par défaut

for (i in seq_len(nrow(parcelles))) {
  score_fertilite <- 50  # Base neutre

  # TWI: zones humides = plus fertiles (TWI > 10)
  if (file.exists(twi_path)) {
    twi <- rast(twi_path)
    twi_val <- extract_mean(twi, parcelles[i, ])
    if (!is.na(twi_val)) {
      # TWI optimal entre 8 et 14
      if (twi_val >= 8 && twi_val <= 14) {
        score_fertilite <- score_fertilite + 25
      } else if (twi_val > 14) {
        score_fertilite <- score_fertilite + 10  # Trop humide
      } else {
        score_fertilite <- score_fertilite + 15  # Sec
      }
    }
  }

  # Pente: faible pente = meilleure fertilité
  if (file.exists(pente_path)) {
    pente <- rast(pente_path)
    pente_val <- extract_mean(pente, parcelles[i, ])
    if (!is.na(pente_val)) {
      score_fertilite <- score_fertilite + max(0, 25 - pente_val)
    }
  }

  parcelles$F2[i] <- min(100, max(0, score_fertilite))
}
...
```{r ex-6-4-check}
grade_this({
  pass("Familles A/F (Air/Fertilité) exportées !")
})
```

---

### Exercice 6.5 : Famille N (Naturalité) + Synthèse
...
```{r ex-6-5, exercise=TRUE, exercise.lines=100, exercise.setup="ex-6-5-setup", exercise.timelimit=600}
...
# N1 : Distance aux infrastructures (routes, bâtiments, zones urbaines)
# Score 0-100 (100 = très éloigné = naturel)
dist_routes_path <- file.path(data_dir, "distance_routes.tif")
...
parcelles$N1 <- 50  # Valeur par défaut

for (i in seq_len(nrow(parcelles))) {
  dist_min <- 1000  # Distance minimale par défaut

  if (file.exists(dist_routes_path)) {
    dist_r <- extract_mean(rast(dist_routes_path), parcelles[i, ])
    if (!is.na(dist_r)) dist_min <- min(dist_min, dist_r)
  }

  if (file.exists(dist_bat_path)) {
    dist_b <- extract_mean(rast(dist_bat_path), parcelles[i, ])
    if (!is.na(dist_b)) dist_min <- min(dist_min, dist_b)
  }

  # Normalisation: 0m = 0, 2000m+ = 100
  parcelles$N1[i] <- min(100, max(0, dist_min / 20))
}
cat("N1 (distance infra):", round(mean(parcelles$N1, na.rm = TRUE), 1), "(moy)\n")

# N2 : Continuité forestière (ancienneté)
# Score 0-100 basé sur BD Forêt Anciennes
bd_foret_anciennes_path <- file.path(data_dir, "bd_foret_anciennes.gpkg")
bd_foret_path <- file.path(data_dir, "bd_foret.gpkg")

parcelles$N2 <- 30  # Valeur par défaut (forêt récente)

if (file.exists(bd_foret_anciennes_path)) {
  foret_anc <- st_read(bd_foret_anciennes_path, quiet = TRUE)
  foret_anc <- st_transform(foret_anc, st_crs(parcelles))
  ...
  for (i in seq_len(nrow(parcelles))) {
    ...
    if (nrow(inter) > 0) {
      ...
      # Forêt ancienne (1818-1866): score 60-100
      parcelles$N2[i] <- 60 + min(40, taux_ancien * 0.4)
    }
  }
} else if (file.exists(bd_foret_path)) {
  # Estimation basée sur le type de peuplement
...
# N3 : Indice composite de naturalité
# Moyenne pondérée de N1 et N2 avec bonus biodiversité
parcelles$N3 <- (parcelles$N1 * 0.4 + parcelles$N2 * 0.4 +
                 ifelse(!is.na(parcelles$B1), parcelles$B1 * 0.2, 10))
...
indicateurs_eco <- c("B1", "B2", "B3", "L1", "L2", "T1", "T2", "A2", "F2", "N1", "N2", "N3")

cat("Indicateurs par famille:\n\n")
cat("FAMILLE B (Biodiversité):\n")
for (ind in c("B1", "B2", "B3")) {
...
cat("Nombre d'indicateurs:", sum(indicateurs_eco %in% names(parcelles)), "/", length(indicateurs_eco), "\n")
```

```{r ex-6-5-check}
grade_this({
  pass("Tous les indicateurs écologiques exportés !")
})
```
