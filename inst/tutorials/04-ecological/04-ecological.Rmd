---
title: "Tutorial 04 : Écologie — Familles B, L, T, F, N (12 indicateurs)"
output:
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
description: >
  Calcul de 12 indicateurs écologiques : B1-3 (biodiversité), L1-3 (paysage/TVB),
  T1-2 (temporel), F2 (fertilité), N1-3 (naturalité).
---
...
> **Note** : Les indicateurs C2 (vitalité) et A2 (qualité air) sont calculés dans le Tutorial 02.
...
parcelles_path <- file.path(data_dir, "parcelles.gpkg")
routes_path <- file.path(data_dir, "routes.gpkg") # Téléchargé en tutoriel 01
...
# Charger les routes depuis le fichier téléchargé en tutoriel 01
if (file.exists(routes_path)) {
  routes <- st_read(routes_path, quiet = TRUE)
  routes <- st_transform(routes, st_crs(parcelles))
...
# Plus le coefficient est élevé, plus la route pollue
...
if (!is.null(routes) && nrow(routes) > 0) {
  ...
  if (!is.null(nature_field)) {
    ...
    routes$pollution_weight <- 0.5 # Défaut
  ...
  if (requireNamespace("furrr", quietly = TRUE) && requireNamespace("future", quietly = TRUE)) {
    ...
    plan(multisession, workers = max(1, availableCores() - 1))
    cat("   Mode parallèle (furrr,", nbrOfWorkers(), "workers)...\n")
    pollution_scores <- future_map_dbl(seq_len(nrow(parcelles)), calc_fn, .progress = TRUE)
    plan(sequential)
  } else if (requireNamespace("purrr", quietly = TRUE)) {
    library(purrr)
    cat("   Mode séquentiel (purrr)...\n")
    pollution_scores <- map_dbl(seq_len(nrow(parcelles)), calc_fn)
  } else {
    cat("   Mode séquentiel (base R)...\n")
    pollution_scores <- sapply(seq_len(nrow(parcelles)), calc_fn)
  }
...
# Normalisation du score pollution (0-100)
# Score pollution élevé = mauvaise qualité air = A2 faible
# Score pollution faible = bonne qualité air = A2 élevé
...
if (max_pollution > 0) {
  # Transformation log pour compresser la distribution
  pollution_norm <- log1p(parcelles$pollution_score) / log1p(max_pollution)
  # Inverser : 0 = très pollué, 100 = air pur
  parcelles$A2 <- round((1 - pollution_norm) * 100, 1)
} else {
  # Pas de pollution = air pur
  parcelles$A2 <- 100
}
