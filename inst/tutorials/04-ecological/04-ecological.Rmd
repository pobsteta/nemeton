...
## Section 4 : Paysage et Vitalité (L1-3, C2)
...
if (requireNamespace("landscapemetrics", quietly = TRUE)) {
  ...
  r_template <- rast(ext(zone_buffer), resolution = 10, crs = st_crs(parcelles)$wkt)
  ...
  metrics <- tryCatch({
    lsm_results <- calculate_lsm(r_foret,
                                  what = c("lsm_l_np",      # Nombre de patches
...
### Exercice 4.4 : Indice de Vitalité NDVI (C2)
...
```{r ex-4-4, exercise=TRUE, exercise.eval=FALSE, exercise.lines=150, exercise.setup="ex-4-4-setup", exercise.timelimit=900}
...
if (requireNamespace("happign", quietly = TRUE)) {
  ...
  tryCatch({
    ...
    irc_layers <- wms_layers$Identifier[grepl("irc|infrarouge|ortho.*ir",
                                        wms_layers$Identifier, ignore.case = TRUE)]
...
if (ndvi_available && exists("ndvi_raster") && !is.null(ndvi_raster)) {
  ...
  if (requireNamespace("exactextractr", quietly = TRUE)) {
    ...
    if (requireNamespace("furrr", quietly = TRUE) && requireNamespace("future", quietly = TRUE)) {
      ...
      plan(multisession, workers = max(1, availableCores() - 1))
      cat("Mode parallèle (furrr,", nbrOfWorkers(), "workers)...\n")
      parcelles$C2 <- future_map_dbl(seq_len(nrow(parcelles)), extract_ndvi_i,
                                      .progress = TRUE)
      plan(sequential)
    } else if (requireNamespace("purrr", quietly = TRUE)) {
      library(purrr)
      cat("Mode séquentiel (purrr)...\n")
      parcelles$C2 <- map_dbl(seq_len(nrow(parcelles)), extract_ndvi_i)
    } else {
      cat("Mode séquentiel (base R)...\n")
      parcelles$C2 <- sapply(seq_len(nrow(parcelles)), extract_ndvi_i)
    }
  }
} else {
...
if (file.exists(bd_foret_path)) {
  ...
  # Fonction de calcul NDVI par BD Forêt
  ...
  if (requireNamespace("furrr", quietly = TRUE) && requireNamespace("future", quietly = TRUE)) {
    ...
    plan(multisession, workers = max(1, availableCores() - 1))
    cat("Mode parallèle (furrr,", nbrOfWorkers(), "workers)...\n")
    parcelles$C2 <- future_map_dbl(seq_len(nrow(parcelles)), calc_ndvi_bdforet_i,
                                    parcelles_geom = parcelles_geom,
                                    bd_foret = bd_foret,
                                    ndvi_by_type = ndvi_by_type,
                                    .progress = TRUE,
    ...
    library(purrr)
    cat("Mode séquentiel (purrr)...\n")
    parcelles$C2 <- map_dbl(seq_len(nrow(parcelles)), calc_ndvi_bdforet_i,
                            parcelles_geom = parcelles_geom,
                            bd_foret = bd_foret,
                            ndvi_by_type = ndvi_by_type)
  } else {
    cat("Mode séquentiel (base R)...\n")
    parcelles$C2 <- sapply(seq_len(nrow(parcelles)), function(i) {
      calc_ndvi_bdforet_i(i, parcelles_geom, bd_foret, ndvi_by_type)
    })
  }
}
...
calc_n2_i <- function(i, parcelles_geom, bd_foret, foret_ancienne) {
  ...
  return(c(boisement = taux_boisement * 100,
...
## Section 6 : Indicateurs Complémentaires (T1, T2, A2, F2)
...
cat("  Min:", round(min(parcelles$T1, na.rm = TRUE), 0), "/ Max:",
...
parcelles$T_classe <- cut((parcelles$T1 + parcelles$T2) / 2,
                          ...
                          labels = c("Jeune/Instable", "En développement",
...
### Exercice 6.2 : Qualité de l'air (A2)

L'indicateur **A2** évalue la **qualité de l'air** dans le contexte forestier, basée sur l'éloignement des sources de pollution.

| Facteur | Description | Impact |
|---------|-------------|--------|
| **Distance routes** | Éloignement des axes routiers | Pollution atmosphérique |
| **Densité forestière** | Couverture arborée locale | Filtration des polluants |
| **Distance industrie** | Éloignement zones industrielles | Sources ponctuelles |
...
```{r ex-6-2-a, exercise=TRUE, exercise.eval=FALSE, exercise.lines=100, exercise.setup="ex-6-2-a-setup", exercise.timelimit=300}
# === INDICATEUR A2 (QUALITÉ AIR FORESTIER) ===

parcelles_path <- file.path(data_dir, "parcelles.gpkg")
bd_topo_routes_path <- file.path(data_dir, "bd_topo_routes.gpkg")
...
# ============================================================
cat("1. Distance aux routes...\n")

routes <- NULL

# Essayer de charger les routes depuis BD TOPO
if (file.exists(bd_topo_routes_path)) {
  routes <- st_read(bd_topo_routes_path, quiet = TRUE)
  cat("   Routes BD TOPO:", nrow(routes), "segments\n")
}

# Fallback : télécharger via happign ou OSM
if (is.null(routes) || nrow(routes) == 0) {
  if (requireNamespace("happign", quietly = TRUE)) {
    library(happign)
    cat("   Téléchargement routes via happign...\n")

    zone_buffer <- st_buffer(st_union(parcelles), 2000)

    tryCatch({
      routes <- get_wfs(
        x = zone_buffer,
        layer = "BDTOPO_V3:route",
        spatial_filter = "intersects"
      )
      if (!is.null(routes) && nrow(routes) > 0) {
        cat("   Routes téléchargées:", nrow(routes), "\n")
      }
    }, error = function(e) {
      cat("   Erreur téléchargement routes:", e$message, "\n")
    })
  }
}

# Calcul de la distance minimale aux routes pour chaque parcelle
if (!is.null(routes) && nrow(routes) > 0) {
  routes <- st_transform(routes, st_crs(parcelles))
  routes_union <- st_union(routes)

  parcelles$dist_routes <- as.numeric(st_distance(st_centroid(parcelles), routes_union))
  cat("   Distance moyenne aux routes:", round(mean(parcelles$dist_routes, na.rm = TRUE)), "m\n")
} else {
  # Fallback : distance par défaut (zone éloignée)
  parcelles$dist_routes <- 500
  cat("   Routes non disponibles - valeur par défaut: 500m\n")
}

# ============================================================
# 2. CALCUL DE L'INDICE A2
# ============================================================
cat("\n2. Calcul de l'indice A2...\n")

# Normalisation de la distance (0-100)
# Plus la distance est grande, meilleure est la qualité de l'air
# Seuils : 0m = 0, 100m = 50, 500m = 80, >1000m = 100

normalize_distance <- function(d) {
  score <- ifelse(d < 100, d / 2,
           ifelse(d < 500, 50 + (d - 100) * 0.075,
           ifelse(d < 1000, 80 + (d - 500) * 0.04, 100)))
  return(pmin(score, 100))
}

parcelles$A2 <- round(normalize_distance(parcelles$dist_routes), 1)
...
# Sauvegarder
output_path <- file.path(data_dir, "metriques_air_a2.gpkg")
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
### Exercice 6.3 : Fertilité du sol (F2)

L'indicateur **F2** évalue la **fertilité potentielle du sol** forestier à partir d'indices topographiques et de proxys disponibles.

| Facteur | Description | Source |
|---------|-------------|--------|
| **TWI** | Indice d'humidité topographique | MNT (calculé en Tutorial 03) |
| **Pente** | Déclivité du terrain | MNT |
| **Exposition** | Orientation des versants | MNT |
| **Type de sol** | Proxy via végétation | BD Forêt |
...
```{r ex-6-3-f, exercise=TRUE, exercise.eval=FALSE, exercise.lines=120, exercise.setup="ex-6-3-f-setup", exercise.timelimit=300}
# === INDICATEUR F2 (FERTILITÉ SOL) ===
...
cat("=== INDICATEUR F2 : Fertilité du sol ===\n\n")

# ============================================================
# 1. INDICE D'HUMIDITÉ TOPOGRAPHIQUE (TWI)
# ============================================================
cat("1. Indice d'humidité topographique (TWI)...\n")

twi_mean <- NULL

if (file.exists(twi_path)) {
  twi <- rast(twi_path)
  cat("   TWI chargé:", basename(twi_path), "\n")

  if (requireNamespace("exactextractr", quietly = TRUE)) {
    parcelles$twi_mean <- exactextractr::exact_extract(twi, parcelles, "mean")
    twi_mean <- mean(parcelles$twi_mean, na.rm = TRUE)
    cat("   TWI moyen:", round(twi_mean, 2), "\n")
  }
} else {
  cat("   TWI non disponible - calcul depuis MNT...\n")

  if (file.exists(mnt_path)) {
    mnt <- rast(mnt_path)

    # Calcul simplifié du TWI
    slope <- terrain(mnt, v = "slope", unit = "radians")
    slope_deg <- terrain(mnt, v = "slope", unit = "degrees")

    # TWI = ln(aire_drainage / tan(pente))
    # Simplification : utiliser la pente seule
    twi_approx <- log(1 / (tan(slope) + 0.001))
    twi_approx <- clamp(twi_approx, lower = 0, upper = 20)

    if (requireNamespace("exactextractr", quietly = TRUE)) {
      parcelles$twi_mean <- exactextractr::exact_extract(twi_approx, parcelles, "mean")
      twi_mean <- mean(parcelles$twi_mean, na.rm = TRUE)
    }
  } else {
    parcelles$twi_mean <- 10  # Valeur neutre
    twi_mean <- 10
  }
}

# ============================================================
# 2. PENTE (facteur limitant pour la fertilité)
# ============================================================
cat("\n2. Pente (facteur limitant)...\n")
...
if (file.exists(mnt_path)) {
  ...
  if (requireNamespace("exactextractr", quietly = TRUE)) {
    parcelles$slope_mean <- exactextractr::exact_extract(slope_deg, parcelles, "mean")
    cat("   Pente moyenne:", round(mean(parcelles$slope_mean, na.rm = TRUE), 1), "°\n")
  }
} else {
  parcelles$slope_mean <- 10  # Pente modérée par défaut
}

# ============================================================
# 3. CALCUL DE L'INDICE F2
# ============================================================
cat("\n3. Calcul de l'indice F2...\n")

# Normalisation TWI (5-15 -> 0-100)
# TWI élevé = accumulation d'eau = meilleure fertilité (jusqu'à un certain point)
twi_norm <- function(twi) {
  score <- (twi - 5) / 10 * 100
  return(pmax(pmin(score, 100), 0))
}

# Normalisation pente (0-45° -> 100-0)
# Pente faible = meilleure fertilité (moins d'érosion)
slope_norm <- function(slope) {
  score <- 100 - (slope / 45) * 100
  return(pmax(pmin(score, 100), 0))
}

# Combiner les facteurs (pondération égale)
parcelles$F2_twi <- twi_norm(parcelles$twi_mean)
parcelles$F2_slope <- slope_norm(parcelles$slope_mean)
parcelles$F2 <- round((parcelles$F2_twi + parcelles$F2_slope) / 2, 1)

cat("\n=== Résultats F2 ===\n")
cat("F2 moyen:", round(mean(parcelles$F2, na.rm = TRUE), 1), "/ 100\n")
cat("F2 min:", round(min(parcelles$F2, na.rm = TRUE), 1), "\n")
cat("F2 max:", round(max(parcelles$F2, na.rm = TRUE), 1), "\n")
cat("Interprétation: 0 = sol pauvre/érodé, 100 = sol fertile\n")

# Classification
parcelles$F2_classe <- cut(parcelles$F2,
                           breaks = c(0, 30, 50, 70, 100),
                           labels = c("Pauvre", "Modéré", "Fertile", "Très fertile"),
                           include.lowest = TRUE)
cat("\nDistribution:\n")
print(table(parcelles$F2_classe))

# Sauvegarder
output_path <- file.path(data_dir, "metriques_fertilite_f2.gpkg")
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
st_write(parcelles, parcelles_path, delete_dsn = TRUE, quiet = TRUE)
...
### Exercice 7.2 : Familles L (Paysage) et C (Vitalité)
...
```{r ex-7-2, exercise=TRUE, exercise.eval=FALSE, exercise.lines=70, exercise.setup="ex-7-2-setup"}
# === FAMILLES L (PAYSAGE) et C (VITALITÉ) ===
# Consolidation des indicateurs déjà calculés
...
# C2 : Vitalité NDVI (calculé en exercice 4.4)
...
# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Familles L et C consolidées:", basename(output_path), "\n")
```


---

### Exercice 7.3 : Familles T (Temporel), A (Air) et F (Fertilité)
...
```{r ex-7-3, exercise=TRUE, exercise.eval=FALSE, exercise.lines=70, exercise.setup="ex-7-3-setup"}
# === FAMILLES T, A, F ===
# Consolidation des indicateurs déjà calculés
...
# A2 : Qualité air (calculé en exercice 6.2)
a2_path <- file.path(data_dir, "metriques_air_a2.gpkg")
if (file.exists(a2_path)) {
  a2_data <- st_read(a2_path, quiet = TRUE) |> st_drop_geometry()
  if ("A2" %in% names(a2_data)) {
    parcelles$A2 <- a2_data$A2
    cat("A2 (qualité air):", round(mean(parcelles$A2, na.rm = TRUE), 1), "\n")
    cat("   0 = proche pollution, 100 = air pur\n")
  }
}

cat("\n=== Famille F (Fertilité) ===\n\n")

# F2 : Fertilité sol (calculé en exercice 6.3)
f2_path <- file.path(data_dir, "metriques_fertilite_f2.gpkg")
if (file.exists(f2_path)) {
  f2_data <- st_read(f2_path, quiet = TRUE) |> st_drop_geometry()
  if ("F2" %in% names(f2_data)) {
    parcelles$F2 <- f2_data$F2
...
# Sauvegarder
st_write(parcelles, output_path, delete_dsn = TRUE, quiet = TRUE)
cat("\n✓ Familles T, A et F consolidées:", basename(output_path), "\n")
```
...
# Liste des indicateurs par famille (14 indicateurs)
familles <- list(
...
|---------|---------|-------------|
| `indicateurs_ecologiques.gpkg` | Synthèse complète (14 indicateurs) | Tous |
...
| `metriques_vitalite_c2.gpkg` | NDVI | C2 |
| `metriques_temporel_t1t2.gpkg` | Dynamique temporelle | T1, T2 |
| `metriques_air_a2.gpkg` | Qualité air | A2 |
| `metriques_fertilite_f2.gpkg` | Fertilité sol | F2 |
