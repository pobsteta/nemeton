...
5. **Pas de plafond de temps par défaut** : `.onf_wfs_read()` borne le
   transfert à 30 s et le test de fumée réel est **opt-in**
   (`NEMETON_TEST_ONF_LIVE=true`) — hygiène pour un vieux MapServer derrière un
   pare-feu, et non le correctif d'un incident observé.
