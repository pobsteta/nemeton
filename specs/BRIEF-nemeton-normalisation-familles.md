...
> **SOLDÉ le 2026-08-16/17.** Archivé ici tel qu'il a été reçu — le corps du
> texte n'a pas été retouché. Ce qui a été vérifié puis livré est journalisé
> dans `PLAN.md` (entrées v0.172.0 → v0.175.0) :
>
> - **CA-1 : prémisse écartée.** `create_family_index()` **normalise déjà** ce
>   qu'il agrège, et le fait depuis la v0.10.0 (`e2285fce0`). Les colonnes
>   `_norm` sont ignorées *parce que* la fonction normalise les brutes
>   elle-même. Ne pas ré-implémenter ce que le §2 demande.
> - **CA-2, CA-3, CA-5** — livrés en **v0.174.0**, avec deux défauts trouvés en
>   chemin que le brief ne visait pas : les codes courts (`C1`, `S3`) n'étaient
>   pas normalisés comme les noms longs, et la famille S comptait quatre fois la
>   même population (motif `^S[0-9]` non ancré).
> - **CA-4** — entièrement côté app (le cœur reçoit un raster NDVI, il n'en
>   choisit pas la source) ; câblé le 2026-08-16 dans `nemetonshiny`.
> - **CA-6** — **v0.173.1** (trois champs STAC à la racine au lieu de `access`).
> - **CA-7** — **v0.172.1** puis **v0.173.0** ; R1 vaut désormais 68,8 à 87 sur
>   Fordead.
